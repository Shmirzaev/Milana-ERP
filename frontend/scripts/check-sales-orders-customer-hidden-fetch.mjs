...
const customerKey = "/api/customers";
...
assert.match(
  ...
  "the customer directory must depend on a loaded order that references a customer",
);
assert.match(
  source,
  /customerMap\.get\(o\.customer_id\) \?\? t\("sales\.unknownCustomer"\)/,
  "the customer-name fallback must remain unchanged",
);
...
assert.equal(active.requests.filter((key) => key === customerKey).length, 1);
assert.match(textContent(active.tree), /SO-12/);
assert.match(textContent(active.tree), /Customer Nine/);

const branded = renderCase({ rows: [{ ...clientOrder, id: 13, order_no: "SO-13", customer_id: null, order_type: "branded_stock_sale" }], total: 1 });
assert.equal(branded.requests.filter((key) => key === customerKey).length, 0);
...
assert.equal(deniedOrEmpty.requests.filter((key) => key === customerKey).length, 0);
...
console.log("Sales-order customers: unused/denied key 1 -> 0; referenced-customer key remains exactly 1.");
