...
const form = { item_id: 1, batch_no: "roll-entry", color: "white", quantity: 35.5, unit: "kg", cost_per_unit: 0, warehouse_id: 1, qc_status: "passed", supplier_id: 0, old_code: "", color_code: "", color_status: "", order_no: "", gsm: "", length_m: "", piece_count: 2, roll_weights_kg: ["15.5", "20"], weight_entry: "rolls", processes: "", image_url: "" };
...
for (const lang of ["en", "ru", "uz"]) {
  ...
  for (const key of ["fabricRollEntry.totalKg", "fabricRollEntry.individual", "materialLength.optional", "passportBatch.add", "sewingReport.deleteConfirm"]) assert.ok(runtime.includes(`"${key}"`), `${lang} runtime missing ${key}`);
}
