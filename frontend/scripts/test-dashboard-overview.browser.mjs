...
function fixture(days, factory = 'ALL') {
  ...
  return { start: daily[0].date, end: daily.at(-1).date, timezone: 'Asia/Tashkent', updated_at: end.toISOString(), factory,
    ...
    sewing_reports: {
      totals: Object.fromEntries(perFactory.map(f => [f.code, factory === 'ALL' || factory === f.code ? f.totals.sewing : 0])),
      daily: daily.map((p, i) => ({date: p.date, values: Object.fromEntries(perFactory.map(f => [f.code, factory === 'ALL' || factory === f.code ? f.daily[i].sewing : 0]))})),
    },
...
(async () => {
  ...
  await context.route('**/api/**', async route => {
    ...
    if (url.pathname === '/api/dashboard/overview') {
      ...
      if (mode === 'empty') { data.active_orders = 0; data.late_orders = 0; data.planned_quantity = 0; data.by_status = {}; data.orders = []; for (const k in data.totals) data.totals[k] = 0; data.daily.forEach(p => { for (const k in data.totals) p[k] = 0; }); for(const code in data.sewing_reports.totals) data.sewing_reports.totals[code] = 0; data.sewing_reports.daily.forEach(p => { for(const code in p.values) p.values[code] = 0; }); data.factories.forEach(f => { f.active_orders = 0; f.planned_quantity = 0; for(const k in f.totals) f.totals[k] = 0; }); }
      return send(data);
    }
  ...
  await page.getByRole('heading', { name: 'Daily sewing reports' }).waitFor();
  ...
  await page.getByRole('heading', { name: 'Daily sewing reports' }).locator('..').locator('..').screenshot({ path: path.join(out, 'activity-chart.png') });
  ...
  for (const [code, name] of [['MIL', 'Milana'], ['BST', 'Besttex'], ['ECO', 'Eco Cotton']]) {
    ...
    await page.getByRole('group', { name: 'Select factory', exact: true }).getByRole('button', { name, exact: true }).click();
    await response;
    await page.getByRole('heading', { name: 'Daily sewing reports' }).waitFor();
    const expected = snapshot?.[`7:${code}`] || fixture(7, code);
    ...
    assert.equal(await page.getByRole('group', { name: 'Select factory', exact: true }).getByRole('button', { name, exact: true }).getAttribute('aria-pressed'), 'true');
  ...
  await page.getByRole('button', { name: 'Export sewing reports' }).click();
  ...
  assert.equal(scopedCsv.split('\r\n')[0], 'date,factory,BST');
  assert.equal(scopedCsv.split('\r\n').slice(1).reduce((sum, row) => sum + Number(row.split(',')[2]), 0), (snapshot?.['7:BST'] || fixture(7, 'BST')).sewing_reports.totals.BST);
  assert(scopedCsv.split('\r\n').slice(1).every(row => row.split(',')[1] === 'BST'));
  await page.getByRole('button', { name: 'All factories', exact: true }).click();
  await page.getByRole('heading', { name: 'Factory comparison' }).waitFor();
  const periodResponse = page.waitForResponse(r => r.url().includes('/api/dashboard/overview') && r.ok());
  await page.getByLabel('Selected period', { exact: true }).selectOption('30');
  await periodResponse;
  await page.getByRole('heading', { name: 'Daily sewing reports' }).waitFor();
  await page.getByRole('heading', { name: 'Daily sewing reports' }).locator('..').locator('..').screenshot({ path: path.join(out, 'activity-chart-30days.png') });
  await page.getByLabel('Selected period', { exact: true }).selectOption('7');
  await page.getByRole('heading', { name: 'Daily sewing reports' }).waitFor();
  await page.getByRole('button', { name: 'Stage records', exact: true }).click();
  ...
  const stageCsv = fs.readFileSync(path.join(out, 'output.csv'), 'utf8').split('\r\n');
  assert.equal(stageCsv.length, 8);
  assert.equal(stageCsv[0], 'date,factory,cutting,printing,sewing,packaging');
  assert.equal(stageCsv.slice(1).reduce((sum, row) => sum + Number(row.split(',')[4]), 0), (snapshot?.['7:ALL'] || fixture(7)).totals.sewing);
  for (const lang of ['ru', 'uz', 'en']) {
    await page.evaluate(lang => localStorage.setItem('erp_lang', lang), lang);
    await page.reload();
    await page.getByRole('heading', { name: lang === 'ru' ? 'Ежедневные отчёты пошива' : lang === 'uz' ? 'Kunlik tikish hisobotlari' : 'Daily sewing reports' }).waitFor();
  ...
  const widerResponse = page.waitForResponse(r => r.url().includes('/api/dashboard/overview') && r.ok());
  await page.getByRole('button', { name: 'Show last 30 days', exact: true }).click();
  await widerResponse;
  assert.equal(await page.getByLabel('Selected period', { exact: true }).inputValue(), '30');
  mode = 'error'; await page.reload();
  await page.getByText('Dashboard data could not be loaded. Please retry.', { exact: false }).waitFor();
  assert.equal(await page.getByRole('heading', { name: 'Daily sewing reports' }).count(), 0);
  ...
  await browser.close();
  console.log('PASS: separate report/stage sources and CSV, seven-day default, three factory filters/comparison, desktop/mobile, EN/RU/UZ, period, series toggles, search, CSV, empty/error and restricted permissions; zero page exceptions.');
