...
export default function ModelsPage() {
  ...
  const [edit, setEdit] = useState({ code: "", name: "", category: "", description: "", status: "draft" });
  const [editMsg, setEditMsg] = useState("");

  async function create(e: React.FormEvent) {
    e.preventDefault();
    setErr("");
    try { await api.post("/api/models", form); setForm({ code: "", name: "", category: "" }); mutate(); }
    catch (e: any) { setErr(e.message); }
  }
  async function approve(id: number) { await api.post(`/api/models/${id}/approve`); mutate(); }
  function openEdit(m: Model) {
    setEditing(m);
    setEdit({ code: m.code, name: m.name, category: m.category ?? "", description: m.description ?? "", status: m.status });
    setEditMsg("");
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={create} className="card p-4 mb-6 grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th>{t("field.status")}</th><th>{t("field.created")}</th><th>{t("field.actions")}</th>
            </tr>
          </thead>
          <tbody>
            {data?.map((m) => (
