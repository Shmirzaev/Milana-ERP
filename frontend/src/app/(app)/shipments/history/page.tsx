          ...
          <div className="divide-y divide-[#ded9ca] md:hidden">
            {filteredHistory.map((shipment) => (
              <article key={shipment.id} className="p-4">
                ...
                {shipment.shipment_type === "manual" && <Link className="btn" href={`/shipments?shipment_id=${shipment.id}`}>{shipmentReviewText[lang].review}</Link>}{canTraceability ? <Link className="btn mt-3 h-8 px-2.5 text-[11px]" href={`/traceability?shipment=${encodeURIComponent(shipment.shipment_no || shipment.id)}`}>{t("page.shipments.traceability")}</Link> : null}
              </article>
            ))}
