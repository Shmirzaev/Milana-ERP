...
export default function InventoryPage() {
  ...
  const { data: stockPage } = useSWR<any>(`/api/inventory/stock?include_total=true&page=${page}&page_size=${pageSize}`, fetcher);
  const stock: any[] = stockPage?.rows || [];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="card p-4"><div className="text-xs text-slate-500">{t("page.inventory.linesTracked")}</div><div className="text-2xl font-semibold">{stockPage?.total ?? 0}</div></div>
      </div>
      <div className="card overflow-x-auto">
        ...
        <PaginationControls
          page={page}
          pageSize={pageSize}
          total={Number(stockPage?.total || rows.length)}
          count={stock.length}
