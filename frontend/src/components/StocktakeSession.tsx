  ...
  return <section className="space-y-4">
    ...
    {data && <>
      ...
      {pending > 0 && <div role="status" className="space-y-2"><span>{text.pending}: {pending}</span>{failure && <>
        ...
        <button className="btn" onClick={async () => { if (saving.current || !await ask(text.discardConfirm)) return; queue.current = []; persistQueue(); failed.current = false; setFailure(""); }}>{text.discard}</button></div>
      </>}</div>}
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <label className="flex flex-col gap-1">{text.result}<select className="input" value={filter} onChange={e => { setFilter(e.target.value); setOffset(0); }}>
