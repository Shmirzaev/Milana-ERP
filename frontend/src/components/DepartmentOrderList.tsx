...
type QueueKind = "incoming" | "pending" | "in_progress" | "completed";
...
function rowTone(kind: QueueKind) {
  if (kind === "completed") return "bg-green-50 hover:bg-green-100";
  if (kind === "in_progress") return "bg-yellow-50 hover:bg-amber-100";
  return "bg-white hover:bg-[#fdf3eb]";
}

export default function DepartmentOrderList({
  rows, kind, title, emptyLabel, t, startingWorkOrderId, onMoveToInProgress,
}: {
  ...
  return (
    <section className="card min-w-0 overflow-hidden">
      ...
      {groups.length ? groups.map(({ key, orders }) => {
        ...
        const quantity = orders.reduce((sum, row) => sum + Number(kind === "incoming" ? row.expected_qty || row.ready_qty || 0 : row.planned_output_qty || 0), 0);
        return (
          <div key={key}>
            ...
            <div className="overflow-x-auto">
              <table className="table min-w-[1180px]" aria-label={title}>
                <thead>
                  <tr>
                ...
                <tbody>
                  {orders.map((row) => {
                    ...
                    return (
                      <tr key={`${row.work_order_id || row.id || row.production_order_id}-${row.textile_code || "all"}`} className={rowTone(kind)}>
                        ...
                        <td className="whitespace-nowrap font-medium">{statusLabel((kind === "incoming" ? row.source_status : row.status) || row.status || "new", t)}</td>
                        <td>
