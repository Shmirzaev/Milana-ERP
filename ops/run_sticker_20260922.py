...
REMOTE = '/tmp/sticker-warehouse-20260922'
...
docker exec -e PYTHONPATH=/app:/app/backend {CONTAINER} python {REMOTE}-classifier.py {REMOTE}-candidates.json --expected-database-host 172.16.10.3 --expected-database-name erp'''
            ...
            command = f'python3 {REMOTE}-backup.py 20260922_100000'
        elif phase == 'upload':
...
docker cp {REMOTE} {CONTAINER}:/tmp/'''
            ...
            command = f'docker exec -e PYTHONPATH=/app:/app/backend {CONTAINER} python {REMOTE}/import_completed_packs.py {REMOTE}/manifest.json --photo-root {REMOTE}/photos --expected-manifest-sha256 {sha} --expected-database-host 172.16.10.3 --expected-database-name erp --environment production --mode {phase}'
            if phase == 'apply':
