...
- Continued 2026-09-24 (DB01 legacy batch edit): `3712c9e` rejects quantity deltas and populated-batch warehouse moves that would emit a movement with a batch unit inconsistent with its catalog item. Existing warehouse/reservation validation still precedes this guard, and metadata-only edits of historical mismatches remain possible. The entire 58-case inventory movement suite plus three existing batch-editor cases and Ruff pass. Archived-batch deletion and Eco fabric transfers still need the same writer review; historical mismatches remain uncorrected. DB01 stays partial and the official ledger remains **105 fixed,16 partial,6 open,22 remaining**. Local only; no production contact, push, deployment or merge.
