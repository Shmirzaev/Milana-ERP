...
- Continued 2026-09-25 (OPS09 1C route wiring): the finance sync route now uses named per-client credentials and client-scoped idempotent replay. Production/public requests fail closed unless `INTEGRATION_1C_CLIENTS_JSON` is configured and include `Idempotency-Key`; the legacy shared token remains available only outside strict mode when named clients are absent. Before rollout, provision and coordinate named credentials with every 1C caller. Imported invoice/payment records still use the shared `external_source='1c'` plus `external_id` namespace, so clients must coordinate globally unique external IDs unless a separate data-namespace policy is approved. Focused local checks pass; no production contact, push, merge or deployment.
