...
- Continued 2026-09-25 (DB01 production-order quantity input): `c279b594` bounds create, item-breakdown and update planned quantities to nonnegative signed-SQL-INT values; invalid booleans, fractions, negatives and overflow reject before writes, with unauthenticated 401 precedence retained. A review follow-up preserves the existing numeric-string/integral-float coercion contract (`"42"` and `42.0`) while retaining the new bounds. The integrated focused and adjacent material-bound selection passes 17 tests; broader production workflow regression is being rerun. Other alternate writers and historical inconsistencies remain, so DB01 stays partial. Local only; no production contact, push, deployment or merge.
