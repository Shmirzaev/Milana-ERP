...
- Latest reviewed batches: **69/127 findings fixed;35 open;23 partial**. Cutting reconciliation (`87c1b76`) uses16 SELECTs at1/50/401 scopes;16 independent cases pass. PostgreSQL audit appends serialize at outer commit (`735961d`);12 focused cases independently pass within30 PG regressions. Audit IDs/hashes are transient until commit; READ COMMITTED is required, SQLite unchanged, historical damage unrepaired. CI passed at `92baf3b`; later changes need a combined run. Review audit transaction contract, scan-only payroll roles and manual-status clients before rollout. No deployment/production access; main conflicts remain unresolved.
