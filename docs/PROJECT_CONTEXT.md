...
- Third batch: scoped-grant escalation and linked-passport/utilization factory leaks fixed; accessory returns grouped once. Historical migration 0001 now owns a frozen bootstrap; clean PostgreSQL upgrade/rerun and existing-row preservation pass. Existing installations are not changed. DB08 records 30 unresolved schema-drift entries; no ORM-parity claim. Browser API checks pass; cycle-3 visual UI launch was policy-blocked. CI observes API coverage, not complete correctness or capacity.
