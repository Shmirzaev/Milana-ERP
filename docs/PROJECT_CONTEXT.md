...
## Ready storage pack workflows prepared, not deployed (2026-09-08)

- Worktree `C:/ERP/.codex-work/ready-storage-workflows-20260908`, branch `codex/ready-storage-workflows-20260908`, started from verified `origin/main` `a59a10b692b662708d0396cb9320bfd877741822`. Both production source manifests and slot states matched release `20260908_080847`, blue active / `20260908_065525` green rollback, manifest `7f155e13f214f94044f2beb439641a572cea384fac4e309e6981672ac4a5a2a8`. The difference between GitHub and deployed implementation was baseline/context records only. The legacy checkout was preserved.
...
- Additive migrations `0115_package_print_runs`, `0116_pack_sales_dispatch`, and `0117_package_quantity_evidence` were applied during the authorized deployment preparation. Existing rows are retained, new optional fields are not backfilled with guessed history, and downgrade guards preserve receipt/count evidence. Application traffic remains on `20260908_080847`; database revision is now `0117_package_quantity_evidence`. No production business records or permissions were created by the deployment tests.
- Final validation passed all 725 backend tests (one inherited Starlette/httpx deprecation warning), Ruff, Python compilation, migration roundtrips and single Alembic head, frontend lint/strict TypeScript/all build contracts/optimized build, and isolated browser tests. Browser coverage includes burst scanning, offline/reload/retry, manual receipt/reprint, pack-only and ordinary Sales, shipment quantity/amount forms, confirmation for extra receipts, mobile overflow and EN/RU/UZ invoice print media. Evidence is under the worktree's ignored `outputs` directory. Full-page signed-in QA remains incomplete: automatic approval review rejected starting the local full ERP frontend with only `blocked by policy`; the local disposable backend was stopped. Live PostgreSQL concurrency/performance and deployment QA remain deployment gates.
- The user supplied `Фарход ака 25.07.2026.xlsx` and explicitly authorized matching it and deploying. Invoice printing now follows its Milana Tex / Ombor hisob-fakturasi A4 portrait layout, driver/vehicle/carrier/contact header, model/variant/description/size columns, package/piece/weight/price amounts, blue headers and yellow/green totals. Actual transport details, identities, descriptions and measured weights are frozen at dispatch. Mixed-package rows share package/weight cells without duplicating totals; unknown weights stay unknown. Warehouse amount adjustments remain explicit. Optional transport metadata is editable only before dispatch; its nullable JSON column is included in undeployed migration `0116`. The shipment-creation race is closed by locking and refreshing the Sales Order before idempotency/existing-shipment checks. Reference print and transport create/edit/retry/read-only/permission browser checks pass. Deployment authorization is present; production is still unchanged pending final validation and promotion gates.

- Validation after the reference-invoice follow-up passed all 735 backend tests (one inherited Starlette/httpx deprecation warning), Ruff/compilation, 11 deployment-observer tests, full frontend lint/strict TypeScript/contracts/optimized build, and A4 EN/RU/UZ print rendering. Production PostgreSQL and signed candidate/public QA remain promotion gates.

- Candidate `20260908_112728`, source `20d116910c8ebee559deb123fbe246dd4a8e15bc`, was built and staged but not activated. Its isolated PostgreSQL migration/concurrency checks passed, including duplicate manual receipt retries, exact six-then-four receiving, quantity/extra-receipt reservations, immutable triggers, shipment-creation locking and frozen dispatch. Verified pre-migration backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260908_112728.dump`: 51,274,812 bytes, mode 0600, 1090 restore objects, SHA-256 `23b069891ec54ae440a39af01df465e70ab5cd60f1708ac19fdde00ca83f2945`; restore-list SHA-256 `f1dbf5f568aa15246b600c4c793805026aae8cc9130815da79711970ec7fe218`. Signed read-only candidate QA passed the available warehouse, shipment and pack-Sales controls but found a pre-existing 20-pixel mobile inventory-search overflow with actual stocktake data. Setting a 12rem minimum on the search form makes it wrap below the Result filter and was verified at 390px and 320px. The source fix is one Tailwind class; scanner behavior and backend code are unchanged. Candidate images must be rebuilt and verified before activation. First-candidate evidence is retained under `outputs/deployment`.

- Candidate `20260908_114527` / `31ec11e9623a0174dd6ee1ad7bd112a1203dfdb4` passed signed desktop/mobile, isolated PostgreSQL and paired performance gates but remained inactive. Final print-header review found the global API CSP blocked inline invoice/label styles and the Print button on direct HTML responses; earlier HTML fixtures and the preview proxy did not exercise that policy. The four warehouse print routes now return a scoped CSP allowing styles and only the exact hashed `window.print()` handler, with script elements, forms, framing and other resources blocked. The rest of the API retains its existing policy. Actual middleware response/browser checks cover all four routes and EN/RU/UZ invoices; the preview now preserves upstream security headers. No production label GET is used for QA because the older QR helper may lazily persist a missing QR file/reference. A rebuilt, verified candidate is required before activation; previous evidence is retained.
