...
- Latest reviewed batches: **57/127 findings fixed**. Payroll separates management of payable values from scanning trusted labels; custom scan-only issuers need owner-reviewed permissions before rollout. Rate-store I/O leaves the event loop. Partial fixes cover work-order stage/factory guards, passport defaults/lists and customer invoice-total batching. Next: sewing lock order, flow-utilization queries and payroll-finalization races. Audit/QA record tests and limits. No deployment or production access; main conflicts remain unresolved.
