...
- Continued 2026-09-22 (latest local delta): recruitment, shipment, branded-planning, waste, sewing-flow, customer-history, work-order and print-run reads gained opt-in bounded SQL pages while retaining legacy arrays; traceability and stocktake repeated reads were removed; attendance ordering, sales totals, receipt recovery and finite write vocabularies gained rollback regressions; hidden frontend requests were gated by actual route/modal/reference need. Current ledger is **79 fixed,33 partial,15 open,48 remaining**. All work remains local and unpushed; no migration, merge, deployment or production contact.
