...
## Eco Cotton process tracking deployed (2026-08-31)

- Active backend/frontend release is `20260831_100414` in the blue slot, built from merge commit `e4648812315ff5439053c2045df077aad1574574`. Its 662-file source-manifest SHA-256 is `254ac6a153a8529b15139f5f3689496d398bb42311f5d5b55306d7123fb29861`; deterministic archive SHA-256 is `044c5eae802a6dd71777b60d968c39f37f4fc478d4ef4b615a1b3361c1d4734a`. Backend image digest is `sha256:33619b946ad05a7e6ca56fbb536dd3d2175e39418cfd3017e615e887b958e0da`; frontend image digest is `sha256:4d98daa537e4ecdbf37a6c181dc76dc5eaec5646fe2315b35803704e7dbf248e`. Release `20260829_112849` remains running in the green rollback slot.
- Eco Cotton now has a dedicated Process Tracking sidebar section linking to `/processes?factory=ECO`. The shared page shows an Eco-specific title/subtitle and retains the existing search, status/date filters, sorting, pagination, stage detail, audit, and print/export behavior. Backend tracking applies the authoritative Eco sewing-factory route, excludes Usluga orders from this standard-production view, and rejects a session logged into another factory.
- GitHub Actions run `33380696431` passed Ruff, Python compilation, all 500 backend tests, frontend lint, strict TypeScript, every production build contract including the new Eco tracking contract, and the optimized 82-route Next.js build. The immutable release artifact and both GHCR images were built once from the merged commit.
- Verified pre-deployment backup: `/opt/milana-erp/shared/backups/milana_erp_pre_20260831_100414.dump`, mode `0600`, 48,252,161 bytes and 1,067 restore objects; dump SHA-256 `db045f35301f9bbf62a995310a6b42e456b221031cf0792a2def2bcae8ac3a24`; restore-list SHA-256 `187270289f83b0a89a093de652ac1d2f4e91eb05f2a1d40cc00ab809d27eb480`. The migration was a no-op and PostgreSQL remains at `0112_price_calc_requests`.
- The first warmed comparison stopped before cutover when two low-latency medians varied above budget (`global_search` +13.70%, `model_groups` +15.28%); payloads were identical and p95 values remained within budget. The repeat warmed gate passed with identical rows/payload bytes: medians ranged from -2.88% to +9.25% and p95 from -9.28% to +4.62% across all five searches. The post-cutover benchmark also passed.
- Candidate read-only QA returned the one active Eco order, independently verified every returned production order against the authoritative Eco factory condition, rejected a MIL query under the Eco token with HTTP 403, and created or changed no business row. Signed-in Chrome QA switched the Super Admin session to Eco Cotton, verified the new sidebar link, Eco title/subtitle, one correctly labeled Eco order, and zero browser console warnings/errors, then restored the session to Milana.
- Cutover activated backend first and frontend second. Both current symlinks and blue slot states name the new release; public health/login and internal checks returned HTTP 200, HAProxy and both routers validated, both active containers had zero restarts/OOM events and no matched recent critical/HTTP-5xx markers, the backend had exactly two workers, PostgreSQL used 27 of 100 connections, invalid indexes were zero, and backend/frontend disk use was 57%/53%.
- The completed 30-minute observation returned 61/61 public health and 61/61 public login probes at HTTP 200 over 1,814.3 seconds. Closing inspection again verified both active blue and rollback green containers running with zero restarts/OOM events, no matched recent critical/error/HTTP-5xx log markers, valid source manifests and HAProxy configuration, 31 of 100 PostgreSQL connections in use, zero invalid indexes, unchanged 57%/53% disk use, and a successful post-cutover benchmark.
- Deployment added no migration and created, edited, or deleted no production order, work order, bundle, package, inventory, user, role, permission, audit, or other business row. The only data operation was the validated pre-deployment database backup.
