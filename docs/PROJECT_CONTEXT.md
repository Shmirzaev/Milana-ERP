...
## Screenshot-selected active purchase orders deleted (2026-09-10)

- Removed exactly 31 orders / 31 order lines from Active Purchase Orders at the user's explicit request, plus their 31 obsolete historical number aliases. Selected orders: `PUR-0003`–`PUR-0013`, `PUR-0015`–`PUR-0022`, `PUR-0025`–`PUR-0031`, `PUR-0033`–`PUR-0035`, `PUR-0040`, and `PUR-0047`. They comprise 23 Dinar and 8 Samo orders, totaling 19,700 kg ordered and zero received. Duplicate screenshot rows were deduplicated; cropped identifiers were resolved against stored photos, dates, suppliers and quantities. Unmarked partial rows were excluded.
- The locked transaction rechecked the exact reviewed snapshot, zero receipts, no linked stock movements/batches, no sales/production links or tasks, and the complete foreign-key inventory. Full retained-row fingerprints proved all other orders/lines/aliases, purchase requests, request lines, stock batches, stock movements, reservations, items, suppliers and tasks unchanged. Shared photos and historical audit entries were preserved. Audit entry `18770` records the full deleted rows, backup, reason and snapshot hash; its new hash-chain segment validates.
- Fresh post-commit readback proved all 31 targets absent and all 16 retained orders identical: 13 active and 3 received. Remaining active orders are `PUR-0023`, `PUR-0024`, `PUR-0032`, `PUR-0036`–`PUR-0039`, and `PUR-0041`–`PUR-0046`.
- Verified full PostgreSQL backup: `/opt/milana-erp/shared/backups/pre_delete_marked_purchases_20260910_113715.dump`, 52,244,526 bytes, mode 0600, 1,160 restore objects, SHA-256 `6dca6888d5f4e99f8e51e25b31836d5867c3c401748806d8c03a785ea7888443`. Restore-list SHA-256 `f82316fec6a78a4dde8431d69c01c9675f559d5a32b1b3af75671ce01c41c4de`. Both hashes were rechecked immediately before apply. Recovery requires this backup; application rollback does not restore deleted data.
- Application release remains `20260910_093726` / green on both VMs; rollback `20260910_091900` / blue; schema `0122_cutting_material_details`. No application deployment, migration or restart occurred. All four internal/public health/login checks returned HTTP 200. Ruff, Python compilation, guarded production dry-run, transaction fingerprints, audit hash verification and independent post-commit readback passed. No UI code or permission change was made.
- Dedicated worktree: `C:/ERP/.codex-work/remove-marked-purchases-20260910`; branch `codex/remove-marked-purchases-20260910`; execution commit `ba4cd563` pushed. Completion documentation is committed separately on the same branch. Branch is not merged. Evidence is under `outputs/` in that worktree. The legacy checkout was preserved. Production manifests match `origin/main`'s production baseline; the Git tip differs from the release source only by deployment metadata/context.
