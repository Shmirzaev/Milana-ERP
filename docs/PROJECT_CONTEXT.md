...
- Continued 2026-09-23 (closure pass through `534c8bf`): whole-finding review closes PERF13, PERF28, PERF26, PERF30, PERF33 and PERF18. Bundle receipt/acceptance query counts are constant at20/29 SELECTs for1/50/401 bundles with26 focused passes; purchasing request/order/approval/receipt references are chunk-bounded and audit-head reads remain one with140 passes/5 PostgreSQL-only skips. PERF26 reservation candidates use3/3/5 total and2/2/4 stock SELECTs at1/50/401 variants; its legacy metadata repair uses1/1/2 sales-item and1/1/2 collection reads for1/50/401 distinct references, with15 focused and99 adjacent passes. PERF30 clone allocation uses1/1/2 exact code-only reads for1/50/401 occupied codes, with66 compatibility and6 real-PostgreSQL plan/concurrency passes. PERF33 detail and CSV reads remain in one read-only repeatable-read snapshot across the400-row boundary;69 compatibility and3 real-PostgreSQL race/cleanup cases pass. PERF18 packaging/sewing option plans retain top-level limits without subplans or per-scope aggregate repetition at1/50/401;25 focused and6 real-PostgreSQL cases pass. PERF36 bounds notification discovery and removes Data Console's schema-wide counts, but its protected task-broadcast gap remains. PERF39 removes another set of inactive directories while retaining exact active/denied rendering; three dependent chains still require new backend projection/authorization contracts. PERF02 remains blocked on parity-safe database derivation before paging; PERF08 retains complete per-package child arrays; PERF22 retains the legacy unpaged compatibility response. The committed range through `f25b064` is pushed to `feat/ismoiljon`; later closure commits remain local pending separate push approval. Official ledger is **85 fixed,27 partial,15 open,42 remaining**. No migration, merge, deployment or production contact.
