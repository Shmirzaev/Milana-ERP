...
- Continued 2026-09-25 (Cycle 6 local bug fixes): `dced120e`, `4ee9bdbc`, `fa03857f` and `b9827387` bound cutting and expanded production rows; `e6e1823e` corrects a stale 422 test expectation. `1d43b793` and `18c9381b` page Cutting Passport and its production-order picker. `b20411d6`, `da53567f` and `39ca6d8b` preserve item-only stock reservations through Eco dispatch and batch removal; a disposable PostgreSQL deletion race passed. `a98ca5f5` also guards batch quantity edits, warehouse moves and item relinks; its PostgreSQL contention regression passed. `f6a9548e` serializes aggregate stock adjustment with reservations; its PostgreSQL race passed without writes. `8a36ffa8` prevents duplicate ready-pack allocation; `495f299a` bounds shipment notes; `6b6f3f5b` validates generic Production Order type. `6e2ba6bd` and `00c07757` carry currency provenance through new finance records, reports and UI; 51 focused tests and three disposable PostgreSQL checks pass. `6d089382` pages the Production Order summary Sales Order picker; seven backend cases and the UI/TypeScript/lint gates pass. `716cbb19` and `850c18df` preflight the base and delta old-ERP model import plans, including quarantine reconciliation. `def7acbe` preflights receipt-augmented production-migration plans; a final bounded writer check found the known correction/catalog/reconciliation paths guarded too. `70d74a67` rehearses migration 0101 on disposable PostgreSQL. Fresh dependency scans report no package advisories. The canonical ledger remains **106 fixed, 15 partial, six open**: legacy currency/data decisions, actual production row review, infrastructure evidence and release validation remain. No production write/migration, push, merge or deployment occurred.

- Continued 2026-09-25 (Cycle 5 final local follow-ups): `96180b99` caps production-order material/item/batch input lists at 1,000 before writes (26 focused/adjacent tests). `cc574fff` bounds new manual package receipt evidence at 16 KiB before numbering, QR, database or audit writes (three focused tests). `5b245df7` replaces eager purchasing-request material/accessory choices with searchable 50-row pages; its runtime checks, strict TypeScript, lint and full frontend stabilization pass. `c94d467b` adds an explicitly targeted, read-only waste-sale history preview with four synthetic tests; repeated-sale candidates are not automatically duplicates and need source payment/invoice/buyer evidence. The official ledger remains **106 fixed, 15 partial, six open**; mixed-currency and 1C external-ID policy, historical records, approved production access, release rollout and operational validation are still required for the broader findings. No production write, migration, push, merge or deployment occurred.

- Continued 2026-09-25 (Cycle 5 bug fixes and read-only live checks): Inventory reservation release, consumption, accessory issue and packaging now use consistent locking and protect batch-bound plus item-only claims, including warehouse attribution and same-order packaging reconciliation (`21b64fbb`, `024602fb`, `1e0bd95c`, `d19638f7`, `6fbbaf57`, `001f0f74`). Focused SQLite and disposable PostgreSQL contention regressions pass. Purchasing, cutting, stock dimensions, waste replay and one-cent payment history gained focused validation; a combined changed-backend selection passed 287 with five PostgreSQL-only skips, while the later legacy-purchasing and direct-reservation changes passed their own focused suites. Inventory/Planning/Receiving selectors and the Shipments order floor/history are paged; historical shipment deep links recover off-page rows, and strict TypeScript plus full frontend stabilization pass. The owner authorized read-only production checks: public `/health` and `/login` returned 200, but public/backend `/ready` returned 404, and current login headers still allow `unsafe-inline` script without HSTS. This confirms the local readiness/security fixes are not active on the checked service; no production write, migration, push, merge or deployment occurred. The audit ledger remains **106 fixed, 15 partial, six open** because policy, legacy-data and operational acceptance gaps remain.

- Continued 2026-09-25 (DB03 migration compatibility and package payloads): Review of WIP `93b0d07b` found that applying today's 64 KiB/16-container JSON bound inside historical Alembic revisions 0062/0086 could reject valid pre-limit models during restore. Commit `6ee3b032` removes those guards and tests large/deep legacy data, 400 paid operations expanding to 1,200, and downgrade roundtrip. Five focused migration tests pass, including a disposable-PostgreSQL direct-revision rehearsal; the full fresh Alembic chain also passes on disposable PostgreSQL; no production restore or migration was run. Commit `a5e98b11` caps newly submitted package change-request item/allocation arrays at 200 rows while preserving unchanged legacy rows; 22 focused/adjacent tests pass. DB05's existing revision 0126 read-only grant preflight was also rerun with adjacent preflights (25 passing tests). The ledger remains **105 fixed, 16 partial, six open, 22 unresolved**. No production contact, push, merge or deployment occurred.

- Continued 2026-09-25 (DB03 catalog-import details): `import_model_catalog.py` now checks the complete changed `Model.details_json` document against the live 64 KiB/16-container validator before assigning/flagging the ORM field or copying model images. Unknown legacy extensions are preserved rather than trimmed; changed oversized/deep legacy documents and oversized manifest source-row lists fail without model mutation. An exact repeat import retains its original timestamp and does not mark the model details changed, allowing unchanged oversized legacy documents. Five new catalog tests plus four reconciliation tests pass; Ruff and diff checks pass. Historical Alembic 0062/0086 rewrites and the broader editable-JSON audit remain, so DB03 is partial and the ledger remains 105 fixed, 16 partial, six open. No catalog import, migration, production contact, push, merge or deployment occurred.

- Continued 2026-09-25 (DB03 XJ3062/Besttex reconciliation): the reviewed reconciliation script now builds and bounds every changed `Model.details_json` document before mutating any model or sewing line. It uses the live 64 KiB/16-container validator, preserves unrelated extensions, and allows exact unchanged oversized legacy documents. Four new no-database tests plus seven adjacent importer tests pass; Ruff and diff checks pass. Catalog import and historical Alembic JSON rewrites remain, so DB03 stays partial and the ledger remains 105 fixed, 16 partial, six open. The reconciliation, import, migration, production contact, push, merge and deployment were not run.

- Continued 2026-09-25 (DB03 XJ3183 offline paid-operation details): the importer now checks changed `Model.details_json` through the same 64 KiB/16-container validator used by live catalog writes before it assigns any ORM field or audit row. Exact unchanged legacy details remain pass-through, including oversized documents; conflicting existing paid operations keep their prior error precedence. Seven importer-only no-database tests pass; the importer plus adjacent live detail-validation module passes 22. Ruff and diff checks pass. `import_model_catalog.py` remains unmodified because its merge preserves unknown existing extensions and requires a separate compatibility design. DB03 remains partial; no import, migration, data write, production contact, push, merge or deployment occurred.

- Continued 2026-09-25 (DB02 material-reservation command type): `MaterialReservationIn.reservation_type` now advertises and validates the existing model/service codes `material`, `accessory`, and `packaging`, preserving the `material` default, surrounding-whitespace normalization, and the legacy blank/category-derived behavior. Unsupported new values return 422 without reservation, movement, audit, or idempotency writes; an unauthenticated invalid request retains 401. The focused contract and adjacent material-reservation suites pass 33 tests; Ruff and diff checks pass. Other command vocabularies and the unresolved generic Production Order status policy keep DB02 partial. Local only; no production contact, migration, push, merge or deployment.

- Continued 2026-09-25 (DB03 offline model-size measurement schema): the old-ERP size manifest loader now validates imported `ModelSize.measurement_json` against the live strict five-field finite-number schema, covering both the base and delta importers before DB session creation. The reviewed production package validator applies the same check while parsing package rows, also before DB access. Valid values are preserved without normalization; `null` remains accepted. The three focused importer suites pass 83 tests; Ruff and diff checks pass. Other offline `Model.details_json` writers and Alembic 0062/0086 remain unaudited/unbounded, so DB03 stays partial. No import, migration, stored-data write, production contact, push, merge or deployment occurred.

- Continued 2026-09-25 (DB03 legacy stock source-payload bounds): `backend/scripts/import_legacy_ready_stock.py` and `import_sticker_ready_stock_local.py` now reject persisted `LegacyStockReceipt.source_payload` documents above 16 KiB compact UTF-8, over 16 nested containers, or containing non-finite/non-JSON values before opening a database session. Existing row/photo/expectation/target validation ordering remains ahead of the new guard; all six oversized/deep/non-finite no-database tests pass, and Ruff/diff checks pass. Alongside the two old-ERP importers covered earlier, all four repository `LegacyStockReceipt.source_payload` writes found by inventory are now bounded. Direct offline `Model.details_json` import/correction writers and migration revisions 0062/0086 still need compatibility/bounds review. DB03 remains partial. No import, migration, stored-data write, production contact, push, merge or deployment occurred.

- Continued 2026-09-25 (DB02 task-reference command schema): newly supplied task reference types are validated against the existing supported targets and singular/plural aliases before route logic; accepted spelling remains stored unchanged, and legacy unsupported values remain readable. The task input module passes 29; adjacent reference-factory/assignment suites pass 40 with one expected skip; Ruff and diff checks pass. DB02 remains partial: other loose commands and the owner decision for generic Production Order status are still open. No ledger status/count change; local only, no production contact, push, deployment or merge.

- Continued 2026-09-25 (DB03 offline JSON writer audit): both reviewed old-ERP completed-pack importers now reject `LegacyStockReceipt.source_payload` rows above 16 KiB compact UTF-8, over 16 nested containers, or containing non-finite/non-JSON values during manifest validation, before opening a database session. The focused importer selections pass 17 tests; Ruff and diff checks pass. Other offline utilities still persist `LegacyStockReceipt.source_payload` without explicit per-document bounds (`backend/scripts/import_legacy_ready_stock.py`, `backend/scripts/import_sticker_ready_stock_local.py`) and write `Model.details_json` directly from curated/import manifests or correction plans (including `backend/scripts/import_old_erp_models_local.py`, `import_old_erp_model_delta_local.py`, `migrate_reviewed_old_erp_models_production.py`, `correct_old_erp_models_local.py`, `import_model_catalog.py`, `import_xj3183_paid_operations.py`, and `reconcile_xj3062_besttex_20260912.py`). Migrations 0062 and 0086 also rewrite `Model.details_json` without the shared live size/depth validator; 0118 reads the column to populate relational paid-process rows but does not write it. DB03 remains partial pending those writers' contracts and bounds. No import, migration, production contact, push, merge or deployment was run.

- Continued 2026-09-25 (DB01 packaging record quantities): `2e295da3` applies the shared non-boolean integer guard to packaging input, packed and damaged counters, preserving numeric-string/integral-float coercion while rejecting booleans, fractions, negative and SQL-INT-overflow values before writes. The integrated packaging/production quantity selection passes 35 tests; Ruff and diff checks pass. Other writer/reference/state and historical-data work keeps DB01 partial. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-25 (PERF35 accessory Receive/Return order picker): the real accessory inventory receiving screen now searches 50-row production-order pages with exact filtered totals and Load more instead of eagerly requesting the capped 500-order array. Existing production-read authorization is unchanged, and a chosen/deep-linked order remains available across subsequent searches. Five production-order paging/search/auth/parity tests, the frontend contract, strict TypeScript, touched-file ESLint, Ruff and diff checks pass; the frontend dependency junction used for static checks was removed and its target untouched. The stale HR Employees position harness and two renamed script paths were corrected; the full frontend stabilization suite now passes. This is one bounded selector slice, not a whole-finding closure. Other large inventory receive summaries and PERF35 graphs remain. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-25 (DB03 cutting material details JSON): changed `CuttingMaterialUsage.details` writes through the cutting-record details endpoint now have a 16 KiB compact UTF-8 and 16-container-depth limit; exact unchanged oversized/deep legacy JSON passes through. Validation occurs before ORM mutation and audit/commit. The three new focused regressions and adjacent cutting quantity validation pass (71 tests); Ruff and diff checks pass. DB03 remains partial because other editable JSON and offline import/migration paths still need exhaustive review. Local only; no production contact, push, merge or deployment.

- Continued 2026-09-25 (DB01 production-order quantity input): `c279b594` bounds create, item-breakdown and update planned quantities to nonnegative signed-SQL-INT values; invalid booleans, fractions, negatives and overflow reject before writes, with unauthenticated 401 precedence retained. Review follow-up `4791c1f8` preserves the existing numeric-string/integral-float coercion contract (`"42"` and `42.0`) while retaining the new bounds. The integrated focused selection passes 12 tests and the broader production flow/update/pagination selection passes 127. Other alternate writers and historical inconsistencies remain, so DB01 stays partial. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-25 (DB02 branded planning order type): `BrandedPlanningOrderIn.ordered_for_type` now uses the stable `milana`/`eco_cotton`/`besttex`/`customer` vocabulary and retains the route's trim/case normalization for new commands; legacy rows remain readable because output schemas are unchanged. Regressions cover five unsupported-value 422/no order-or-audit writes, authentication 401 precedence, normalized `BESTTEX`/`Eco_Cotton` codes, missing-customer 404 precedence, and OpenAPI enum output; the focused plus adjacent planning selection passes 26 tests. Other command vocabularies and generic Production Order status policy remain open; DB02 stays partial and the official ledger is unchanged. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-25 (integrated local verification): the DB01 batch-unit edit passes 92 inventory movement, stock-adjustment and quantity-bound tests; the DB03 HR/attendance JSON slice passes 22 focused tests; Sewing receive-option pagination passes 13 backend tests with three expected PostgreSQL-only skips, its two Bun contracts, strict TypeScript and targeted ESLint. The Sewing picker now searches and loads exact-total 50-row pages while retaining one row per batch. These are additional PERF35, DB01 and DB03 slices, not whole-finding closures. The official ledger remains 105 fixed, 16 partial, six open. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB03 HR settings and attendance checkpoint JSON): `a52dc5fc` bounds changed factory HR settings and attendance source-checkpoint documents to 16 KiB compact UTF-8 and 16 nested containers; semantically unchanged oversized/deep legacy values remain writable. HR validation follows weekend-day/typed-field rules; attendance checkpoint validation is centralized at its SystemSetting writer and preserves token, identity, source-version, and transaction rollback behavior. The focused HR/attendance settings, workspace, checkpoint, import and vendor selection passes 48 tests; Ruff and diff checks pass. Other editable JSON and offline import/migration paths remain, so DB03 and the official ledger stay partial/unchanged. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-25 (DB01 stock-batch unit edits): changing a stock batch's unit now fails closed when positive stock remains or linked quantity-history units disagree with the selected unit; linked waste records also block relabeling pending their separate unit policy. This prevents a combined unit/quantity PATCH from emitting an adjustment or issue movement in a different unit from the stored balance. Focused inventory movement and stock-adjustment regressions pass with Ruff. Metadata-only edits remain supported for legacy unit drift; no historical rows are changed. DB01 remains partial.

- Continued 2026-09-25 (OPS03 shared-infrastructure review preparation): `docs/OPS03_SHARED_INFRASTRUCTURE_REVIEW.md` adds an authorized read-only owner/topology/limits/schedule inventory and before/during/after-backup measurement record. Repository settings declare a 48-connection blue/green backend budget against a 100-connection PostgreSQL ceiling, but exclude other clients and do not verify live settings or contention; backup manifest code pairs artifacts but does not establish a job or schedule. No host/backup inspection was performed; owners must authorize and supply live evidence. OPS03 remains open; no production contact, backup, migration, push, merge or deployment.

- Continued 2026-09-25 (OPS04 failure-domain checklist): added a read-only owner checklist for VM-to-host, site/power, storage, network and proxy dependencies, with separate recovery/failover evidence and RTO/RPO gates. Deployment docs describe VM roles and blue/green release rollback but do not establish physical independence; the audit's shared-host claim is not independently verified here. OPS04 remains open pending authorized infrastructure inventory and demonstrated recovery/failover. No production contact or infrastructure change.

- Continued 2026-09-25 (DB03 system settings JSON): `e798c94a` bounds changed shared SystemSetting sections to 16 KiB compact UTF-8 and 16 nested containers, preserving semantically unchanged legacy field values. The check runs after existing section, authorization and schema validation and before settings/audit persistence; company-logo updates use the same stored-document guard, with a regression proving an oversized legacy value leaves no orphaned uploaded logo. The integrated settings/security selection passes 52 tests with four expected PostgreSQL-only skips; Ruff and diff checks pass. Factory HR settings, attendance checkpoints, and other writable JSON domains remain outside this slice, so DB03 stays partial and the official ledger is unchanged. Local only; no production contact, push, deployment or merge.
...
- Continued 2026-09-25 (OPS07 capture procedure): added a read-only, per-site checklist for DNS/routes, approved TCP/HTTP phase timing, raw sample retention and time-matched firewall/QoS/proxy evidence. Historical public-path diagnostics support intermittent public DNS/path contribution but do not explain every slow API sample; no factory/branch paths were measured. OPS07 stays open until authorized site captures and effective network-rule review exist. No production/network access or network changes.

- Continued 2026-09-25 (DB01 batch-tracked stock adjustment): `440f1908` and its review follow-up `022f81fe` reject quantity adjustments, including no-op requests, when any positive batch counted in the stock aggregate has a unit different from its catalog item. This includes archived legacy rows because the aggregate includes them. The 75-case adjustment/movement-integrity selection, Ruff and diff checks pass in the isolated worktree. Historical unit drift still needs policy-approved reconciliation, so DB01 remains partial; no production contact, migration, push, merge or deployment.

- Continued 2026-09-25 (FN07 exact-cent settlement): `f99313d0` keeps invoice paid totals and customer invoice/advance splits in Decimal, preventing a one-cent advance from being skipped and an invoice one cent short from being marked paid. `a875a801` applies the final one-cent balance to its open invoice instead of creating an advance; `c07bcc84` uses exact cents for customer order-history totals and paid status; `720b924e` recomputes 1C invoice state in Decimal. The integrated manual/customer/1C payment, idempotency and history selection passes 97 tests with 14 expected PostgreSQL-only skips. FN07 remains partial: manual payment status currently leaves exactly $0.01 as unpaid while 1C marks it partially paid; owner policy is pending. Purchase and stock-batch unit-cost inputs also accept more decimal places than Numeric(12,4), pending a reject-versus-round policy. Local only; no production contact, migration, push, deployment or merge.

- Continued 2026-09-25 (DB03 model details JSON): `fea4fb18` bounds changed `Model.details_json` documents to 64 KiB compact UTF-8 and 16 nested containers, rejects non-finite/non-JSON values, and preserves exact unchanged legacy documents through iterative comparison. Guards cover create, full and paid-operation PATCH, model clone, variant create/edit, and family identity renames; validation follows established shape/reference/conflict checks and precedes persistence. The focused model-details, costing, catalog/clone/variant, and rename selection passes 77 tests; Ruff and diff checks pass. Other editable JSON and offline import/migration paths remain outside this slice, so DB03 stays partial and the official ledger is unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.
...
- Continued 2026-09-25 (FN07 payment cents and financial input-list caps): manual `/api/finance/payments` preserves Decimal through persistence and rejects sub-cent values before Payment or invoice-status writes. Keyed replays keep the existing JSON-number fingerprint semantics. Sales order items and accessory issue lines are capped at 1,000 with exact-boundary/schema tests; oversized sales-order input preserves 401 precedence and creates no order, item or audit rows. The combined finance, payments, sales, purchasing, shipment and accessory-inventory selection passes **307 tests**, with 18 expected PostgreSQL-only skips; Ruff and diff checks pass. FN07 remains partial: derived/accounting policy and the complete financial state/list inventory are not closed. Local only; no production contact, schema migration, push, merge or deployment.

- Continued 2026-09-25 (PERF35 Production Orders screen): `45f24e8b` moves the real screen from showing only the API's default first 50 rows to opt-in 50-row searchable Load more pages with exact totals; search is escaped and matches production/order numbers and model code/name. Row editing, visible-row model labels and the legacy default API shape remain intact. All five 1/50/401, search, auth and no-write backend cases pass independently in integration, plus Ruff and the Bun screen contract; strict TypeScript and targeted ESLint pass in the isolated worktree. Other broad graphs and production-load evidence keep PERF35 partial and the official **105 fixed,16 partial,6 open,22 remaining** ledger unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (DB02 payroll-period contract): `d5b34867` adds regression evidence for the existing draft/open creation guard, rejecting an unsupported state with the established 400 response before business or audit writes and retaining 401 authentication precedence. Five focused/adjacent cases pass independently in integration, plus Ruff. Tightening the schema to `Literal` would change the established 400 to 422; the broader command-vocabulary and lifecycle-policy finding remains partial, with the official **105 fixed,16 partial,6 open,22 remaining** ledger unchanged. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-25 (PERF35 Finished Goods ready-to-ship search): `bd4e3ec1` adds escaped, case-insensitive order-number/customer/address search to the existing exact-total 50-order reservation-backed page and adds a search field plus loaded/filtered-total count on the real screen. The selected page still hydrates complete reservation/package detail, and the unpaged API remains compatible. All five focused backend pagination cases pass independently in integration, as do Ruff, the two Bun paging contracts, strict TypeScript, targeted ESLint and diff checks. The broader production-load and other PERF35 graph evidence remains open; official **105 fixed,16 partial,6 open,22 remaining** ledger unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (accessory plan Decimal round-trip): `e61c4cb0` normalizes accessory plan required, issued, available, remaining, and shortage quantities to the four-decimal scale accepted by the issue endpoint, computing differences in Decimal before emitting JSON. A new test posts the plan's own 0.06 × 210 remaining quantity unchanged to the issue endpoint. The new test, adjacent batching tests and all seven previously failing production-flow cases pass (9 focused); the full sales/stock/price-calculation/production-flow integration selection now passes **133 tests**. Ruff and diff checks pass. This does not close the broader FN07/DB01 findings or change the official **105 fixed,16 partial,6 open,22 remaining** ledger. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (PERF35 Sewing Production Report order options): `620c0739` adds a factory-scoped, searchable SQL-bounded 50-row order-option page with exact totals, legacy order aliases, MIL/SEW/unassigned bundle routing, and selected-option retention. The real report screen requests those pages with Load more and asks the legacy options endpoint to skip its unbounded order history; default legacy behavior remains compatible. Four adjacent backend tests pass independently in the integration branch, plus Ruff and the Bun screen contract; TypeScript and targeted ESLint pass in the isolated worktree. Other broad graphs and production-load evidence keep PERF35 partial and the official **105 fixed,16 partial,6 open,22 remaining** ledger unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (FN07 sales-order line prices): `3526487d` preserves Decimal for explicit unit prices and rejects sub-cent values before order numbering or writes. Omitted `NUMERIC(14,4)` catalog prices round half up to cents before both line storage and total calculation; pack totals are checked again after physical package allocation. The focused price, total-bound, source-type, ready-pack, storage-handoff, price-calculation and production-flow selection passes 51 tests; Ruff and diff checks pass. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-25 (API05 recommendation factory scope): `feb7415f` limits recommendation reads and exact paged totals to model-attributed rows in factories where the caller effectively has `forecasting.view`; creates and status updates require effective `forecasting.manage` access. Model-less creates fail closed, and inaccessible updates return 404 without writes. `a432e313` aligns newer confidence/source-JSON regressions with factory-attributed fixtures. All 68 forecasting/traceability cases across the combined local selection pass independently, plus Ruff. The dashboard and item-reorder routes still consume unattributed inventory/BOM data and remain unscoped pending an explicit shared-data policy, so API05 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (PERF35 Payroll employee options): `36b8d225` replaces the Payroll screen's eager legacy 500-employee directory with factory-scoped, escaped-search 50-row option pages and retained selected IDs. Paged adjustment rows now carry same-factory employee/department labels, and record rows use their existing department projection. Eight focused backend tests pass independently in the integration branch, as do Ruff and both Bun screen contracts; TypeScript and targeted ESLint pass in the isolated worktree. The repository-wide i18n checker still reports pre-existing missing keys, and this change adds no copy. Other broad graphs and load evidence keep PERF35 partial and the official **105 fixed,16 partial,6 open,22 remaining** ledger unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (DB02 forecast confidence): `0b2b6820` restricts new recommendation confidence to low/medium/high or null after auth/reference checks, preserving historical values on reads and rejecting unsupported writes before recommendation/audit persistence. Forty-six adjacent forecasting tests pass independently, plus Ruff. Other loose commands keep DB02 partial and the official **105 fixed,16 partial,6 open,22 remaining** ledger unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (DB03 forecast provenance JSON): `acab0a9c` bounds new recommendation `source_json` writes to 16 KiB UTF-8 and 16 container levels, rejects non-finite values, and preserves existing permission/reference precedence with no recommendation or audit write on rejection. Forty-two adjacent forecasting tests pass independently, plus Ruff. Existing stored JSON remains readable; other editable JSON keeps DB03 partial and the official **105 fixed,16 partial,6 open,22 remaining** ledger unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (DB03 HR profile and Cutting bundle JSON): `4df74a3f` bounds changed HR employee profile JSON writes to 16 KiB UTF-8 and 16 nested levels after reference/conflict checks; exact unchanged oversized/deep legacy data remains round-trippable. `68a62230` caps raw cutting bundle-plan rows at 1,000, rejects boolean/fractional counts and quantities, and limits color/size to the persisted Bundle widths. The HR profile test passes 20 and the six-module HR/employee selection passes 36; cutting quantity validation passes 68; adjacent production-flow/Usluga/material-bound suites pass 115. Ruff, compile and diff checks pass. Flexible `Model.details_json` extensions remain unbounded, keeping DB03 and the official **105 fixed,16 partial,6 open,22 remaining** ledger unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (FN07 manual invoices): `6304934b` preserves Decimal input and rejects unrepresentable/sub-cent new finance invoice amounts after locked sales-order lookup and existing-invoice replay, before numbering or writes; `68c794cc` asserts the exact stored maximum. The four adjacent finance suites pass 65 with six expected PostgreSQL-only skips, and Ruff passes. Derived order totals are read through the NUMERIC(14,2) mapping and checked defensively. FN07 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. Local only; no production contact, push, deployment or merge into `feat/ismoiljon`.

- Continued 2026-09-25 (OPS09 1C route and proxy review): the finance sync route uses named per-client credentials and client-scoped idempotent replay. Production/public requests fail closed unless `INTEGRATION_1C_CLIENTS_JSON` is configured and include `Idempotency-Key`; the legacy shared token remains available only outside strict mode when named clients are absent. Strict proxy configuration rejects default routes (`0.0.0.0/0` and `::/0`) and noncanonical CIDRs whose host bits would otherwise be silently broadened. The focused SEC09/OPS09 backend selection passes 38 with 1 expected PostgreSQL-only skip; Ruff passes. Before rollout, provision and coordinate named credentials with every 1C caller. Imported invoice/payment records still use the shared `external_source='1c'` plus `external_id` namespace, so clients must coordinate globally unique external IDs unless a separate data-namespace policy is approved. Production proxy/header/credential configuration and caller rollout remain unverified; no production contact, push, merge or deployment.
- Continued 2026-09-25 (PERF35 HR Employees): `9c53b633` pages `/api/employees` at 50 rows for the real HR Employees screen with factory-scoped escaped server search, exact filtered totals/status metrics, and permission-gated exact profile coverage. Paged rows include manager labels through a same-factory SQL join; a separate 50-result manager search retains selected options beyond the visible page. Legacy no-page list shape, field privacy, and employee edits remain compatible. Nine adjacent employee/HR backend modules pass 50 tests (0 skips); Ruff, the Bun HR screen contract, targeted ESLint and TypeScript pass. The repository i18n checker still reports pre-existing missing keys outside HR Employees. Integrated locally on `codex/ismoiljon-cycle2-20260925`; live production manifests/slots were not verified, and no production contact, push, deployment or merge into `feat/ismoiljon` occurred.

- Continued 2026-09-25 (DB03 paid operations): `5a7b3abe` caps new/changed `details_json.paid_operations` lists at 1,000 object rows on model create, full update and the dedicated paid-operations route. Exact unchanged legacy rows remain grandfathered across generic and factory-scoped updates; a post-merge check blocks changed scoped writes whose restored hidden rows exceed the limit. The focused catalog, model-details and factory-scope selection passes 57, as does Ruff. Other editable JSON remains, so DB03 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB01 clone and DB03 HR skills): `6779a1e2` preflights item-linked and batch-only legacy BOM units before cloning a model, preventing a new mismatched clone without altering itemless Usluga descriptions; four focused clone cases and Ruff pass. `941c8cf8` bounds new/changed HR position skill lists to 50 entries of at most 160 characters while allowing exact unchanged oversized legacy lists through unrelated full PATCH; 15 adjacent HR position cases and Ruff pass. Other unit writers, historical data and editable JSON remain, so DB01/DB03 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB03 production attachments): `7767ee3c` bounds new/changed ProductionOrder printing attachments to 50 safe uploaded paths with URL/name/content-type limits across direct create, planning create and PATCH; unchanged legacy attachment JSON remains readable and survives unrelated edits. `aa7f27c2` retains existing planning-order missing/closed error precedence before attachment validation without flushing a new planning order first. The combined attachment, update-integrity, upload-lifecycle and signed-flow selection passes 64, as does Ruff. Other editable JSON remains, so DB03 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB01 item-linked BOM units): `eedfe49b` rejects new or changed ModelBOM item/batch/unit combinations that disagree with the catalog unit after existing reference/numeric checks; it preserves itemless Usluga descriptive rows and unchanged legacy mismatches on unrelated PATCH. `e860d79e` aligns the clone-model regression fixture with its selected catalog item's unit. The combined catalog numeric, catalog and Usluga selection passes 67, as does Ruff. Historical mismatches and other writers remain, so DB01 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB02 quality-check severity): `a9bce608` restricts quality-check creation severity to the four database-documented codes (low, medium, high, critical), retaining the low default and rejecting unsupported codes with 422 before a QualityCheck or audit write. All 24 cases across factory scope, list pagination and projection pass, as does Ruff. Other loose commands remain, so DB02 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB03 item composition): `de3bda68` bounds new/changed inventory item material compositions to 100 nonblank rows and 255 characters per stripped name, while preserving exact unchanged legacy JSON on unrelated full-item PATCH and retaining existing composition-total error precedence. All 54 cases across the new boundary module and two adjacent inventory suites pass, as does Ruff. Other editable JSON remains, so DB03 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB01 production material units and API05 branded forecasts): `a229d40d` checks catalog, stock-batch and submitted material units before creating production-order material rows through both array and legacy fabric-batch paths; all 31 material-reservation cases and Ruff pass. `0e9bb6ba` restricts the branded-stock-suggestions route to factories where the caller effectively has `forecasting.view`, excluding unassigned models; `5709af3c` updates a stale recommendation text-bound fixture. Sixty forecasting/traceability cases and Ruff pass. Unattributed item/inventory forecasts, global dashboard/reorder/recommendation routes and historical unit mismatches remain unresolved, so DB01/API05 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB02/DB03 payroll inputs): `cb0af8e3` constrains payroll adjustment type to Bonus or Deduction after existing case/whitespace normalization, retaining null/blank default behavior. `4f4efcb8`/`3a513e93` cap each open raw payroll employee/work snapshot at 16 KiB and 16 nested levels, including after QR enrichment and before persistence. `063d76ac` keeps an existing test helper's generated role name within the established 64-character limit. The combined eight-module payroll selection passes 88; Ruff and diff checks pass. Other loose commands and editable JSON remain; DB02/DB03 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB01 forecast item units): `efbae11f` rejects explicit item-linked recommendation units that differ from the catalog unit and fills the catalog unit when omitted; missing-item reference errors still take precedence and itemless recommendations are unchanged. The full 22-case forecasting suite and Ruff pass with no recommendation/audit writes on failure. Historical mismatches and other alternate writers remain, so DB01 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (PERF35 core department inbox): `2b2ec74c` through `e2b5067e` add an opt-in canonical `/api/inbox/department-orders` page with exact merged-identity totals, uncapped selected bundle-group hydration and scalar-streamed identity discovery; default legacy responses remain unchanged. `6c412e44` wires non-Cutting department screens to 50-row Load more pages and skips duplicate core graph work in their compatibility inbox request. Thirty-five adjacent backend cases, two explicit auth/factory cases and four production-flow inbox cases pass, as do Ruff, strict TypeScript, targeted ESLint, a new screen contract and the full frontend stabilization harness. The exact global identity count still scans O(N) narrow rows and keeps O(N) keys in memory; FGS ready-to-ship, other graphs and production-load performance remain open. PERF35 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB01/DB03 partial follow-ups): `f224b6f5` validates the catalog item, stock batch and saved planned unit before a legacy Cutting passport synthesizes its primary material row; the full 26-case reservation suite and Ruff pass. `2b79edd3` caps explicit accessory costing prices to the same finite ceiling as other price-calculation inputs; nine focused/workflow cases and Ruff pass, including a 422/no-write API check. Neither change rewrites historical rows. DB01 and DB03 remain partial; the official ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, push, deployment or merge.

- Continued 2026-09-25 (DB01/DB02/DB03 partial safeguards): `9404a6fe` checks the catalog item against a replacement Cutting batch even when an existing reservation bypasses reservation creation; all 22 material-reservation cases pass. `8a874361` strictly bounds sewing-correction size entries before ORM writes; all 22 correction cases pass. `9bdf79c4` rejects lifecycle status changes through generic work-order PATCH while allowing unchanged-status metadata edits; seven focused and three adjacent cases pass. Ruff and diff checks pass. These are local slices, not closure of the broader writer/command/JSON findings; the official ledger remains **105 fixed,16 partial,6 open,22 remaining**. No production contact, push, deployment or merge.

- Continued 2026-09-24 (DB01 legacy batch writes): `3712c9e` rejects quantity deltas and populated-batch warehouse moves that would emit a movement with a batch unit inconsistent with its catalog item. Existing warehouse/reservation validation still precedes this guard, and metadata-only edits of historical mismatches remain possible. `5b037dc` rejects stocked-batch archive/deletion before releasing reservations or creating an inconsistent issue movement. `de8585f` rejects catalog/batch unit drift before Eco fabric send/return movements and before showing a roll as sendable; existing idempotent responses remain intact. `4508120` closes the Cutting passport path that could add a legacy mismatched batch when an existing reservation bypassed new reservation validation. The full 59-case inventory movement suite, three existing batch-editor cases, 12 archive/delete cases, 16 Eco transfer cases, 21 material-reservation cases and Ruff pass. Historical mismatches remain uncorrected and the broader DB01 writer/state audit remains; the official ledger stays **105 fixed,16 partial,6 open,22 remaining**. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-24 (DB05 payroll factory preflight): `f3dd734` makes the 0130 permission preview stop before account inspection on revision drift; 14 local migration-preflight tests and Ruff pass. `75fd0fb` adds a predecessor-gated, read-only 0101 payroll factory-attribution preview for five tables, identifying projected codes, mixed periods, new uniqueness collisions, ambiguous bundle sources, invalid codes and default-to-MIL rows without exposing payroll amounts or business identifiers. Three synthetic 0101 tests, Ruff and CLI help pass; the CLI now accepts `--revision 0101`. This is operator evidence, not migration approval: no production row scan, disposable-PostgreSQL migration rehearsal, verified backup or recovery drill occurred. DB05 remains partial and the official ledger remains **105 fixed,16 partial,6 open,22 remaining**. No push, deployment or merge.

- Continued 2026-09-24 (DB05 Usluga migration review): `dea28bb`/`fd86104` add a read-only preview of revision 0107's BOM link removal, role ranking, bundle-link backfill, cutting-record defaults and permission append; it flags named non-Usluga BOM rows that would fail the global new check constraint. `da20712` fixes an ambiguous item projection, refuses to guess malformed role permission results, enforces read-only SQLite/PG inspection and exposes `--revision 0107` in the CLI. Four local tests, Ruff and CLI help pass. No migration/production query ran; exact affected-row review, backup and 0101 live-data review remain. DB05 stays partial and the official ledger remains **105 fixed,16 partial,6 open,22 remaining**. No push, deployment or merge.

- Continued 2026-09-24 (DB01 historical unit visibility): `5904c4b` adds a read-only report of exact item-unit mismatches across stock batches, reservations, purchasing lines, movements and item-attributed forecast recommendations; `db3a94c` requires an explicit database URL and preserves a pre-existing SQLite read-only mode. Five synthetic local tests and Ruff pass. No production scan or automatic conversion was performed; legacy correction remains a business review, so DB01 and the official **105 fixed,16 partial,6 open,22 remaining** ledger are unchanged. No push, deployment or merge.

- Continued 2026-09-24 (DB05 role overwrite review): `2901212` makes the 0055 read-only preflight stop before business-table reflection on revision drift. `f81cb47` previews 0091/0092 Payroll/Sewing role permission overwrites with exact current snapshots, SQL after-values, case-insensitive role IDs, stage applicability and explicit applied-history limits; the CLI accepts 0091, 0092 and both. Twelve combined local preflight tests, Ruff and CLI help pass. A read-only 0101 audit identified factory attribution, mixed-period, uniqueness, invalid-code and rollback hazards requiring an executable preflight and disposable-PostgreSQL check. DB05 remains partial; no migration or production query ran. Official ledger stays **105 fixed,16 partial,6 open,22 remaining**. No push, deployment or merge.

- Continued 2026-09-24 (DB05 destructive-migration review): `0df2c17` adds a read-only preflight for historical revision 0055, reporting exact direct-delete row IDs/snapshot hashes, migration guard and NULL ambiguities, and declared foreign-key references. The CLI accepts `--revision 0055` while preserving 0130 as default. Eight combined preflight tests, Ruff and CLI help pass. The report is not deletion approval: undeclared references and a verified recovery backup still require operator review; no migration or production query ran. DB05 stays partial and the official ledger remains **105 fixed,16 partial,6 open,22 remaining**. No push, deployment or merge.

- Continued 2026-09-24 (DB01 purchasing units): `0e117fe` validates purchase request/order line units against the referenced catalog item before writes, covering manual, sales-order shortage, direct-order and legacy conversion paths while preserving existing validation precedence. Independent adjacent suite passes 68 with 2 expected PostgreSQL skips; the isolated full purchasing run passes 145 with 5 skips. Historical mismatched records and other writers keep DB01 partial; official ledger stays **105 fixed,16 partial,6 open,22 remaining**. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-24 (reviewed local slices): `5d031ca` and `88c22c6` enforce catalog-item units for direct receipts and material reservations before stock writes (55 movement and 20 reservation tests). `a87466c` validates new/changed model translation JSON while preserving unchanged malformed legacy data (44 catalog/BOM tests). `1a030623` pages the Finished Goods stock and branded-stock screens at 50 rows with exact totals/search; `77be6fd` repairs its runtime harness. Fourteen backend Finished Goods/inbox tests, frontend runtime/static contracts, strict TypeScript, targeted ESLint and Ruff pass. DB01, DB03 and PERF35 remain partial; ledger stays **105 fixed,16 partial,6 open,22 remaining**. Local only on `codex/ismoiljon-cycle1-20260921`; no production contact, push, deployment or merge.

- Continued 2026-09-24 (DB01/DB02/DB03): `91fb323` enforces item units on purchase receipts (58 adjacent local tests, 3 expected PostgreSQL skips); `708475d` validates employee scheduled-hours JSON while preserving unchanged legacy data (25 HR tests); `e267e56` restricts new item-category writes to six model-documented categories (39 inventory tests). Ruff passes. These remain slices of partial findings; closure audits are in progress. Ledger stays **105 fixed,16 partial,6 open,22 remaining**. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-24 (reviewed local slices): `fc450a5` adds effective per-factory permission calculation for API05 (35 adjacent tests), but forecasting routes remain unchanged pending unattributed-data policy. `f691900` guards archived material-batch restore against item-unit mismatch (50 adjacent inventory tests). `9748ead` validates new model statuses and model-costing JSON values while preserving unchanged legacy data (50 combined model tests). `359df00` cleans only previous generated company-logo files after replacement commit (21 settings cases with 4 expected PostgreSQL skips and an independent focused pass). Ruff passes for touched backend files. API05, DB01, DB02, DB03 and PERF40 remain partial; ledger remains **105 fixed,16 partial,6 open,22 remaining**. Local only; no production contact, push, deployment or merge.

- Continued 2026-09-24 (API05 scope foundation): owner chose authorized-factory forecasting visibility. `064be49` adds optional factory filters for model-attributed branded demand/stock/pipeline/trend without changing public API behavior; 36 local forecasting tests and Ruff pass. Effective permission checks and an explicit policy for unassigned models, shared inventory and item-only recommendations remain necessary before routes can safely use the scope. API05 stays partial; no production contact or deployment.

- Continued 2026-09-24 (local follow-up): `54cfe39` validates collection status writes; `d034a12` validates new HR weekend-day JSON while preserving unchanged legacy lists; their combined adjacent suite passes 62. `395b83d` corrects an inventory fixture to use Fabric Storage and `59ea70b` blocks item unit changes when quantity references exist; 82 adjacent inventory tests pass. `051a5a2` pages the Replacement Sewing inbox with exact totals, preserving legacy API behavior; 10 adjacent inbox tests, strict TypeScript, targeted lint and Ruff pass. A separate `9efdb2b` SWR import fix passes strict TypeScript and lint, but the full Next build remains unverified because external Google Fonts are unreachable in this local sandbox. All changes remain local on `codex/ismoiljon-cycle1-20260921`; no production contact, push, deployment or merge. DB01, DB02, DB03 and PERF35 remain partial; official ledger stays **105 fixed,16 partial,6 open,22 remaining**.

## Stabilization fixes prepared — not deployed (2026-09-18)

- Latest reviewed batches: **105 fixed,16 partial,6 open;22 of127 remain**. Partial findings are not counted as resolved. The continuation includes bounded list paging, grouped traceability/stocktake reads, strict finite command vocabularies, cross-tab uncertain-write reconciliation, executable hidden-request regressions, per-user authenticated rate budgets, bounded narrow inbox/forecast reference maps, authorization-equivalent detail-page contexts, task-reference authorization matched to target reads, bounded task/notification fan-out, source-versioned attendance imports, set-based accessory queue paging, fail-closed credential/proxy boundaries, migration-equivalent ORM test metadata, duplicate-free fresh relationship/index creation, cancellation-safe worker-owned uploads, nonce-based frontend CSP and per-client 1C identity/replay groundwork. Validation-only run `35675969462` remains the last complete CI baseline; newer commits have focused local evidence and still require final CI. No deployment/production access; develop conflicts, historical audit damage, runtime lifecycle and load validation remain open.
...
- Continued 2026-09-21: `4067755` adds a bounded PostgreSQL `/ready` probe with generic503 while retaining `/health` as liveness (4 focused passes); HAProxy adoption and Redis policy remain open. `5b302dd` enforces Printing input/output conservation (15 focused plus 3 existing flow passes); Cutting/Sewing remain. `749ea5a` batches placement source reads, reducing isolated SELECTs at1/50/401 packages from12/61/422 to12/12/23 (18 focused plus 3 flow passes); per-package writes/audits remain O(N). SEC08 is deliberately unchanged after a disposable local reproduction proved generic raw edits can corrupt administrator authentication fields: owner must choose read-only mode (recommended) or an exact allowlist. Work used only synthetic local data; no migration, merge, deployment or production contact.

- Continued 2026-09-22: `d87166b` adds Cutting numeric bounds and conservation while preserving bundle-derived output; Sewing remains. `bb93fbc` bounds model-family rename reads and excludes migration-only rows; clone/approval work and real PostgreSQL plan/load evidence remain. `503d068` rolls back partial thumbnail batches and restores pre-existing targets; async SQL, cross-process capacity and broader lifecycle work remain. `4c75160` defers the hidden model-season directory while retaining the saved read-only value; `8589ba4` corrects its mock-name lint error. The backend batch passed 42 local tests, Ruff and compilation; CI run `35672476252` passed the complete backend/frontend/PostgreSQL workflow. The earlier ledger error placing SEC08 reproduction evidence on FN08 is corrected. No migration, merge, deployment or production contact.

- Continued 2026-09-22 (third batch): `2e4dbb1` adds Sewing bounds/conservation and no-side-effect API regressions. Full CI run `35674868826` passed frontend/PostgreSQL but exposed four backend cases proving failed input sits beside good sewn output rather than inside it; `65586e0` preserves that behavior while requiring passed<=sewn and processed<=explicit input. The corrected combined slice passes33, and all four failures pass within a24-test compatibility selection. `4a4d0e6` changes Usluga family approval from per-model BOM counts to one grouped query; clone work remains. `be696e1` defers model BOM metadata until the visible Materials tab while preserving read-only metadata. `8240fc9` caps payroll bulk input at500. Corrected validation run `35675969462` passes backend, frontend and PostgreSQL on `bae444d`; release skipped. Independent review also caught and corrected an initial edit-only BOM gate before commit. No migration, merge, deployment or production contact.

- Continued 2026-09-22 (continuation batch): reviewed commits add bounded or paged list reads, batched lookups, grouped aggregates, deferred hidden directories, request-scoped validation and focused security/workflow corrections across catalog, inventory, finance, HR, production and admin paths. Broad backend selections pass173 tests with13 skips,145 with8, and the latest dirty-module review156 with11; frontend stabilization, strict TypeScript and targeted ESLint pass. Later follow-ups preserve normalized accessory search, batch cutting-passport Item validation and package-order synchronization, preload pricing mutation assets, use Decimal profit intermediates, group 1C status refreshes, page HR Documents and Calendar end-to-end, validate forecast references, and gate backend traffic on /ready while retaining /health liveness warm-up. Packaging BOM reads are batched; payroll matching is indexed; receipt access is not duplicated; inbox image blobs are deferred; forecast BOM batches are eager-loaded; cost breakdown uses exact intermediates; HR employee status codes match the UI; and legacy Usluga responses no longer truncate above500 while the UI remains paged. The owner retained authorized generic completion without stage evidence; `d8cd060` codifies that policy and closes WF02. `e7782ec` limits Data Console writes to audited department-name repair and disables hard delete; soft delete awaits explicit migration approval. `cd6ec8b` reuses bulk package valuation, `2cc66e0` batches branded-stock variants, `a47de56` streams complete stocktake CSV exports, `940acd5` cleans failed logo files, `4a0ee8e` batches packaging-receipt references and `0c51202` pages purchasing requests. Commits through `0c51202` are local; the GitHub push awaits fresh explicit remote approval. Counts are74 fixed,37 partial,16 open,53 remaining. No migration, merge, deployment or production contact.

- Continued 2026-09-22 (latest local delta): recruitment, employee/role/data-table directories, global search, dashboard production, shipment customers/eligible orders/ready packages, planning/branded stock, bundle history, package changes/storage search, material reservations, model BOM items/collection seasons, packaging receipts, waste, sewing flow/reports/assignments/history/utilization, customer order/payment history, work/print/purchase orders, notifications, brands, quality checks, finance invoices/revenue, payroll records/paid processes/adjustments/periods, cutting passports, HR positions, Finished Goods, suppliers, departments, warehouses and forecast recommendations gained opt-in bounded SQL pages while retaining legacy response shapes; repeated traceability and stocktake reads were removed; finite write/JSON shapes gained rollback regressions; hidden frontend requests were gated by actual route/modal/reference need. That historical batch ended at **79 fixed,33 partial,15 open,48 remaining**; no migration, merge, deployment or production contact occurred.

- Continued 2026-09-23 (committed range `78155f0..f378278`): focused database-representability bounds now cover purchasing quantities/costs and adjacent finance, payroll, inventory, production, catalog, HR, stocktake, package and sewing inputs; Decimal-safe payment/waste calculations preserve established precision. PERF35 adds bounded fabric-scan and further list/export pages; PERF40 adds failed-upload cleanup regressions across HR/model/production/sales/attendance files; PERF12 caps package label sheets at500 with 500 accepted and 501 rejected before package reads/rendering; PERF33 retains paged stocktake detail/CSV and bounded IDs. Adjacent query work projects image BLOBs out of read paths, aggregates attendance/forecast data and scopes traceability in SQL. Evidence and remaining limits are recorded in [audit](../audit_ismail.md), [backlog](audit_backlog.json) and [QA](stabilization_qa.md). Official ledger remains **79 fixed,33 partial,15 open,48 remaining**; no uncommitted work is counted. No migration, merge, deployment or production contact.

- Continued 2026-09-23 (committed range `f378278..95111b5`): 23 committed follow-ups add finite/range checks for HR positions, payroll/cutting quantities, stock-batch costs/dimensions and purchasing receipt pieces; project bundle-label image BLOBs out of context reads; reuse shipment warehouse references; remove an unused Process Tracking export BOM load; and reduce stocktake detail hydration and streamed scan-summary memory. Inventory color deduplication is pushed to SQL, attendance photo work/buffering is bounded, and admin/Finished Goods/reservation/sewing references receive range guards. Focused regression modules are committed; broader FN07/PERF12/PERF21/PERF33/PERF35/PERF40 scopes remain partial. Official ledger remains **79 fixed,33 partial,15 open,48 remaining**; this entry covers committed history only, excludes uncommitted changes/API02 scaffold, and records no migration, merge, deployment or production contact.

- Continued 2026-09-23 (closure pass through `a8b1707`): whole-finding review closes PERF13, PERF28, PERF26, PERF30, PERF33, PERF18, PERF09, PERF14, PERF21, PERF20, PERF10, PERF19, PERF12 and PERF38. Bundle receipt/acceptance query counts are constant at20/29 SELECTs for1/50/401 bundles with26 focused passes; purchasing request/order/approval/receipt references are chunk-bounded and audit-head reads remain one with140 passes/5 PostgreSQL-only skips. PERF26 reservation candidates use3/3/5 total and2/2/4 stock SELECTs at1/50/401 variants; its legacy metadata repair uses1/1/2 sales-item and1/1/2 collection reads for1/50/401 distinct references, with15 focused and99 adjacent passes. PERF30 clone allocation uses1/1/2 exact code-only reads for1/50/401 occupied codes, with66 compatibility and6 real-PostgreSQL plan/concurrency passes. PERF33 detail and CSV reads remain in one read-only repeatable-read snapshot across the400-row boundary;69 compatibility and3 real-PostgreSQL race/cleanup cases pass. PERF18 packaging/sewing option plans retain top-level limits without subplans or per-scope aggregate repetition at1/50/401;25 focused and6 real-PostgreSQL cases pass. PERF09 package groups reuse locked/read context and batch numbering, ownership, recipients and print children;34 focused,34 adjacent and2 real-PostgreSQL lock/rollback cases pass. PERF14 keeps reservation helper/full Cutting addition reads at11/15 SELECTs for1/50/401 materials and batches each lock/read/insert transfer to1/1/1;14 local and28 real-PostgreSQL cases pass. PERF21 scopes production-batch work orders and histories in SQL with exact legacy fallback; production-batch/package graphs remain17/21 SELECTs at1/50/401, with63 focused and one real-PostgreSQL plan pass. PERF20 batches cutting/packaging write reads; full1/50/401 endpoint counts remain41/30/27/18, with32 focused,156 adjacent and12 real-PostgreSQL cases passing. PERF10 keeps direct receive/place at12/12/23 SELECTs and reduces full print-run receiving from21/756/6021 to18/18/31 at1/50/401, with23 focused and3 real-PostgreSQL concurrency/rollback cases passing. PERF19 reduces full bundle-edit reads41/41/41→29/29/29 and keeps grouped reconciliation16/16/16 at1/50/401 scopes while filtering irrelevant evidence rows;28 focused and one real-PostgreSQL lock/rollback case pass. PERF12 reduces print-run label reads14/504/2004→14/14/14 and bundle sheets8/57/207→7/7/7 at their200-label cap; package sheets remain12/12/14/14 through500, with29 focused and12 compatibility cases passing. PERF38 gives valid signed users independent budgets behind one office IP, preserves IP buckets for anonymous/invalid/public-auth traffic and makes Redis increment/TTL initialization atomic;25 final focused and29 combined auth/security cases pass. OPS08 commit `06705b9` removes python-jose/ecdsa, adopts PyJWT2.14.0 and Node24 LTS; package audits and19 focused cases pass, while exact built-container OS scanning still needs Docker/Trivy or authorized CI. PERF36 and PERF39 retain the exact protected/backend-contract gaps documented in the audit. PERF02 remains blocked on parity-safe database derivation before paging; PERF08 retains complete per-package child arrays; PERF22 retains the legacy unpaged compatibility response. The committed range through `f25b064` is pushed to `feat/ismoiljon`; later closure commits remain local pending separate push approval. Official ledger is **93 fixed,20 partial,14 open,34 remaining**. No migration, merge, deployment or production contact.

- Continued 2026-09-23 (PERF32 closure at `ed4be77`): inbox material payloads, production context, planned BOM demand and branded forecast analysis now use narrow column projections and400-ID maps. Their1/50/401 SELECT counts are4/4/8,6/6/6,4/4/7 and7/7/10 respectively; the dashboard is15/15/18. Forty-four focused/adjacent cases pass with SQL evidence excluding image BLOBs, composition JSON, production attachments and stock-batch roll/image payloads. Official ledger is **94 fixed,19 partial,14 open,33 remaining**. No migration, merge, deployment or production contact.

- Continued 2026-09-23 (PERF39 closure at `b669bd4`): Production Order, Sales Order and Work Order detail pages now load authorization-equivalent page contexts in one browser request instead of2,2 and4 dependent requests. Measured SQL falls21→15,10→8 and30→20. Three focused backend authorization/query cases, the full frontend stabilization suite, strict TypeScript, targeted ESLint, Ruff and diff checks pass. Official ledger is **95 fixed,18 partial,14 open,32 remaining**. No migration, merge, deployment or production contact.

- Continued 2026-09-23 (UI03 closure at `3532e8e`): manual receipts, print runs, package creation, manual shipments and purchase receipts reconcile uncertain saved keys under the same locks as their writes. Recovery never reposts inventory/shipment mutations; committed results remain owner/fingerprint isolated, deleted/revoked results expose no stored response or identifier, and cancellation tombstones block delayed writes. localStorage, Web Locks and storage events coordinate tabs and preserve newer evidence. Twenty-six focused local backend cases, seven real PostgreSQL races, three frontend recovery contracts, full frontend stabilization, strict types, targeted lint, Ruff and diff checks pass. Official ledger is **96 fixed,17 partial,14 open,31 remaining**. No migration, merge, deployment or production contact.

- Continued 2026-09-24 (API02 closure at `73d7ca4`): task references now lock supported targets through task commit and apply each target read route's real permission/factory policy to actors, assignees and broadcasts. Super Admin secondary-factory grants and explicit denials match login-time behavior; legacy aliases/orphans remain compatible. The focused selection passes79 with4 expected PostgreSQL skips, all15 disposable-PostgreSQL cases pass, and Ruff/diff checks are clean. Official ledger is **97 fixed,16 partial,14 open,30 remaining**. No schema migration, merge, deployment or production contact.

- Continued 2026-09-24 (PERF36 closure at `bd36441`): task broadcasts now share the notification recipient ceiling and use one cap+1 narrow authorization projection. At1/50 recipients the endpoint uses4/4 SQLite or6/6 PostgreSQL SELECTs; at401 recipients it reads251 rows and rejects before policy iteration or writes. The combined local selection passes80 with10 expected PostgreSQL skips, the disposable PostgreSQL selection passes17, and the independent broadcast rerun passes8. Official ledger is **98 fixed,15 partial,14 open,29 remaining**. No schema migration, merge, deployment or production contact.

- Continued 2026-09-24 (AT06 closure at `135d669`): the bundled attendance connector timestamps each roster/event snapshot before device reads, and ERP advances durable per-device source checkpoints under the existing import lock using the existing settings table. Delayed older rosters cannot replace current presence; stale/unversioned event history remains append-only without metadata regression; rollback is atomic and legacy behavior remains until version adoption. Thirty-two focused attendance,44 connector and8 real-PostgreSQL race cases pass, with independent reruns of6,44 and8. Official ledger is **99 fixed,14 partial,14 open,28 remaining**. No schema migration, merge, deployment or production contact.

- Continued 2026-09-24 (PERF02 closure at `90b7f21`): PostgreSQL accessory queue pages now derive BOM demand, issuance aliases, global stock/reservations, completion, exact totals and global status order in one statement before `LIMIT`. At1/50/401 candidates the path stays one round trip with exact totals; its401-row plan runs in23.668ms and uses existing work-order/stock-batch indexes. Seven PostgreSQL and14 independently rerun compatibility/auth cases pass. Official ledger is **100 fixed,13 partial,14 open,27 remaining**. No schema migration, merge, deployment or production contact.

- Continued 2026-09-24 (SEC09 closure at `f35a62e`): forwarded client/scheme data is trusted only from exact configured CIDRs, hosted startup fails closed on ambiguous proxy ownership, and public credential/reset-proxy bodies are bounded. Self, reset, admin and configured-seed password changes now lock one user and atomically advance session cutoffs, invalidate reset links and emit metadata-only audits. The broad backend selection passes142 with7 expected skips, four targeted PostgreSQL races and20 proxy cases pass; independent review reruns the focused backend gate, all15 password/reset PostgreSQL cases and all20 proxy cases successfully. Official ledger is **101 fixed,12 partial,14 open,26 remaining**. Deployment must set exact `TRUSTED_PROXY_CIDRS` and `FORWARDED_ALLOW_IPS=`; no schema migration, merge, deployment or production contact occurred.

- Continued 2026-09-24 (WF08 partial at `c13d638`): keyed uncertain waste sales reconcile or tombstone without replaying the physical sale, and the browser coordinates one exact pending request across tabs while displaying remaining stock. The broad waste gate passes115 with9 expected PostgreSQL skips, all5 PostgreSQL races and the frontend contract pass; independent review reruns33 local cases with5 expected skips, both new PostgreSQL orderings and the frontend contract. Identical unkeyed legacy requests, historical repair and valuation/backfill policy still need owner or authoritative-data decisions, so the official ledger remains **101 fixed,12 partial,14 open,26 remaining**. No schema migration, merge, deployment or production contact occurred.

- Continued 2026-09-24 (DB08 closure at `1141d40`, OPS05 evidence at `c275be7`): ORM-created test schemas now match all30 previously unresolved migration contracts and ten equivalent index names without a migration/data rewrite. Local migration contracts pass14 with2 expected skips, disposable PostgreSQL passes16 and124 compatibility cases pass; independent review repeats the14/2 and16-case gates. Readiness now covers PostgreSQL plus the required shared store under one bounded budget, with12 independently rerun focused cases, but actual release/application/public verification is still infrastructure work. Official ledger is **102 fixed,12 partial,13 open,25 remaining**. No schema migration, merge, deployment or production contact occurred.

- Continued 2026-09-24 (PERF40/DB01 partials at `b607eda`, `3d15874`): all11 inventoried upload handlers now use one process-local limiter and off-loop file/SQL work with worker-owned transactions, row rechecks/locks and cancellation-safe ownership; independent review passes16 focused and all9 PostgreSQL attendance races. Purchase receipts now enforce the shared item-category warehouse contract before writes; all12 focused cases pass independently. PERF40 still needs deployment-selected global multi-process admission/spooling controls, and DB01 retains inventory/sewing alternate writers. Official ledger is **102 fixed,13 partial,12 open,25 remaining**. No schema migration, merge, deployment or production contact occurred.

- Continued 2026-09-24 (PERF35/OPS09 partials at `d2a65b9`, `aa8fac1`, `c99e609`): tasks, inventory colors and cutting operators have opt-in exact-total pages, TasksDrawer uses50-row load-more pages, and payroll summaries stream400-row input batches; independent PostgreSQL directory tests pass20. Frontend executable scripts use per-request CSP nonces plus HSTS, and named 1C clients have validated rotation slots/client-scoped replay helpers with17 independent PostgreSQL passes. Complete graph/queue lazy-loading contracts and wiring the protected dirty finance route to per-client identity remain. Official ledger is **102 fixed,14 partial,11 open,25 remaining**. No schema migration, merge, deployment or production contact occurred.

- Continued 2026-09-24 (DB06 partial at `a590222`): fresh migrations now recognize existing foreign keys by semantic columns/targets and omit redundant standalone unique indexes. Fresh PostgreSQL has no duplicate FK signatures or duplicate unique-constraint/index definitions; local migration contracts pass14 with2 expected skips and the complete disposable-PostgreSQL migration suite passes16. Existing deployed schemas and the173 historical missing-prefix candidates remain untouched pending measured workload/cardinality/plan evidence and explicit migration approval. Official ledger is **102 fixed,15 partial,10 open,25 remaining**. No schema migration, merge, deployment or production contact occurred.

- Continued 2026-09-24 (PERF08/PERF22 closures, FN08/OPS02 partials): `330374c` pages minimal receiving-queue rows at 50 default/100 maximum, with34 independently passing backend cases; `cadb035` defaults Usluga orders to100-row pages, with27 backend and a Bun frontend load-more contract passing. `285b69d` removes cancelled invoices from dashboard/monthly revenue while historical-cost/currency policy remains. `50a050d` bounds the configured two-slot backend database pool to48 against the documented100-connection ceiling, pending other-client inventory and load evidence. Current ledger is **104 fixed,15 partial,8 open,23 remaining**. No schema migration, merge, deployment or production contact occurred.

- Continued 2026-09-24 (DB05/OPS06 partials and FN07/DB01/DB03 follow-ups): `d23738c` previews revision0130 account-grant changes read-only with4 passes; other historical migration impacts still need review. `13f5994` pairs database and storage backup hashes with7 passes/3 Windows link skips; no actual restore, off-host durability or RTO/RPO verification exists. `f7ff339` rejects sub-cent 1C amounts with18 passes; `287fab2` enforces inventory warehouse/unit/reference consistency with72 passes; `1610d50` bounds sewing-assignment progress/resizing with40 passes; `4def8fb` validates established model-detail JSON objects with5 passes. Current ledger is **104 fixed,17 partial,6 open,23 remaining**. No migration, merge, deployment, production contact or new push occurred.

- Continued 2026-09-24 (SEC08 closure and WF08 key requirement): `df1d08f` adds owner-approved local-only revision0132 for reversible Department deactivation, named Super Admin action, atomic audit, historical-reference preservation and inactive-assignment guards. Independent gates pass84 backend cases,16 disposable-PostgreSQL migration cases, the Data Console handler contract and strict TypeScript. `01e0ee1` requires Idempotency-Key for all new waste sales;39 disposable-PostgreSQL cases and frontend recovery pass, while historical duplicates and valuation policy remain. Current ledger is **105 fixed,16 partial,6 open,22 remaining**. Revision0132 was not applied to production; no deployment, production contact or push occurred.

- Continued 2026-09-24 (DB01 deletion guard): `df14ad6` locks the sewing work order/assignment and rejects deletion when completed output, linked sewing records/reports or historical status exist; the return path also detects directly linked records with mismatched line names. Fifty-eight focused sewing cases and Ruff pass. Other alternate writers remain. Current ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (DB01 accessory-unit guard): `794f213` rejects stock-backed accessory issues with units different from the catalog item while preserving manual custom-unit issues. Twenty-nine focused/adjacent inventory cases pass, including transactional rollback after a valid first line. Other alternate writers remain; official count stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (DB03 permission JSON bounds): `7453b9b` caps new role, user-extra and factory-policy permission grant writes at 150 entries/128 characters while legacy records remain readable. Fifty-one adjacent backend cases and Ruff pass. Other editable JSON remains; ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (DB02 request status): `8711c96` makes purchase-request creation status an explicit draft/pending_approval schema vocabulary while preserving the old whitespace/null normalization. Forty-one purchase-request/compatibility tests and Ruff pass; unknown values fail before writes, unauthenticated calls retain401. DB02 stays partial and the ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (PERF35 Package Scan): `4487ed0` replaces the scan screen's complete warehouse-map fetch with a fixed-size grouped cell summary and model-scoped 50-row pages, preserving exact totals and Load more. Five new and thirteen combined local storage-map tests pass, as do strict TypeScript, Ruff and the Bun page harness. Warehouse-map/stock and other large graphs remain; PERF35 is partial and the ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (PERF35 HR Organization): `ef94d3c` bounds factory-scoped organization units and employees to 50-row default/100-row maximum pages, adds escaped search and exact SQL totals/KPIs, and makes the real screen load more units and parent/manager choices. The combined HR suite passes46; Bun screen behavior, strict TypeScript, Ruff and diff checks pass. HR attendance and other large graph scopes remain; ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (PERF35 payroll/inbox/attendance): `d7e4f15` fixes Payroll Order QR option totals beyond 500 and adds 50-row search/Load more; `59fea8b` adds exact-total Finished Goods inbox package pages while preserving reservation-backed ready-to-ship orders; `a877454` pages HR attendance employees while streaming exact factory/day summary inputs. Focused local backend suites pass5/12/18 and disposable-PostgreSQL checks pass2/5/4, respectively; frontend behavior/wiring, strict TypeScript, targeted lint and Ruff pass. Other inbox queues, payroll directories, warehouse-map/stock and broad PERF35 graphs remain. Ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (PERF35 warehouse map): `67d3ca8` replaces the main warehouse-map's full package graph with exact fixed-size cell/zone aggregates, capped rack previews and 50-row selected-cell pages. The map alone opts into a server-side mixed-model move guard; legacy clients keep their previous placement contract. Thirteen focused and four adjacent local tests, an isolated PostgreSQL concurrency case, the Bun screen contract, full frontend stabilization, strict TypeScript, targeted lint and Ruff pass. Warehouse stock, broader inbox/payroll workflows and other large graphs remain; ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (PERF35 payroll summary): `2174ade` bounds employee/currency group responses and operation aggregates to 50 groups per page, adds debounced search/Load more, and keeps exact global headline totals. Fifty-eight adjacent backend cases, strict TypeScript, targeted lint, Ruff and full frontend stabilization pass. Global totals still scan all matching records; other large workflows remain. Ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (PERF35 Finished Goods reservations): `deef9c6` adds an exact-total, 50-order ready-to-ship page on the Finished Goods screen, with full per-order reservation/package/item detail and SQL-selected latest shipment. The no-limit inbox API remains compatible. Eleven focused/adjacent backend tests, frontend runtime paging, strict TypeScript, targeted lint, Ruff and full stabilization pass. Other inbox queues, warehouse stock, payroll and broad graphs remain; ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-24 (PERF35 warehouse stock): `b38f723` replaces the full placement graph with SQL-grouped 50-row detail pages, exact filtered totals and top model cards. Legacy unplaced receipt grouping/search is preserved. Five focused tests pass on SQLite and disposable PostgreSQL; 14 combined root stock/map cases, strict TypeScript, targeted lint, Ruff and full frontend stabilization pass. Other inbox/payroll/employee graphs remain; ledger stays **105 fixed,16 partial,6 open,22 remaining**. No production contact, deployment or push occurred.

- Continued 2026-09-25 (PERF35 accessory Receive/Return): the opt-in picker now resolves historical polymorphic stock references with SQL `UNION ALL`, normalizes indirect movement units, aggregates linked manual/stock issues and returns by order/item/unit, computes distinct-order or item-group exact totals, and applies filtering/order/offset/limit before returning page rows to Python. It filters to catalog accessory/packaging rows at the canonical catalog unit because the return write rejects other categories/units; the default legacy summary still exposes those historical rows. Item select keys are composite item/unit identities while submits retain the corresponding catalog ID and unit. Fourteen focused accessory balance tests cover legacy parity, invalid historical manual/noncanonical rows, 401 unique orders/402 groups, SQL aggregation-before-limit and all six movement reference forms; Ruff passes. The frontend contract, full stabilization, strict TypeScript and targeted ESLint pass. Source tables still require a full database scan/group for exact totals and PostgreSQL plan/load behavior is unverified, so PERF35 remains partial. No migration, production contact, push, merge or deployment occurred.

- Continued 2026-09-24 (DB01 and PERF35 inbox): `5b4ac7c` enforces catalog/batch unit consistency in shared stock-consumption paths, with 73 adjacent local passes; legacy mismatched data requires correction. `bfb51a9` pages replacement-cutting work and keeps global queue suppression; `3e6c5d4` pages Awaiting Packaging one row per production batch, including grouped unbatched work. Nine combined inbox cases, strict TypeScript, targeted lint and Ruff pass. Other writer/graph scopes remain; ledger stays **105 fixed,16 partial,6 open,22 remaining**. Work is local on `codex/ismoiljon-cycle1-20260921`, descended from `feat/ismoiljon`; merge into the feature branch is deferred until completion and review. No production contact, deployment or push occurred.
...
- Second batch: incomplete attendance downloads cannot advance their checkpoint; audited accounts must be deactivated rather than deleted; pricing requires explicit permissions instead of profile names/emails; Eco-history roll queries are batched (50 dispatches: 54 → 5 SELECTs). Targeted regressions pass; permission compatibility and unmeasured paths remain explicit in the audit. [QA guide](stabilization_qa.md) and [finding ledger](audit_backlog.json) track remaining work.
- Third batch: scoped-grant escalation and linked-passport/utilization factory leaks fixed; accessory returns grouped once. Historical migration 0001 owns a frozen bootstrap; clean PostgreSQL upgrade/rerun and existing-row preservation pass. Existing installations are not changed. That checkpoint recorded30 unresolved DB08 schema contracts; the later `1141d40` closure aligns them without a migration. Browser API checks pass; cycle-3 visual UI launch was policy-blocked. CI observes API coverage, not complete correctness or capacity.
- Production was not contacted. Previously recorded active blue `20260917_100146`, rollback green `20260917_091524`, schema `0131_sewing_corrections` are unchanged by this task, not reverified live. The base application's code matches `deploy/production-base.json`; source-context records differ. Reconcile live manifests/slots before deployment. Historical risks outside the fix register remain open.
...
- Shared `.input` controls (inputs, selects, textareas and searchable dropdown wrappers) now use darker 2px borders with light/night theme colors and retained accent focus indication. The surrounding application layout is unchanged. Fabric Archive shows the existing `received_date` as a separate Arrival date column on desktop and labeled field on mobile, alongside the archived date; EN/RU/UZ labels are provided.
- Frontend-only presentation change: no business data, permissions or schema changes. Database remains `0131_sewing_corrections`; no new migration was applied. Startup seeding remained disabled.
- Standard immutable CI [35200576531](https://github.com/Shmirzaev/Milana-ERP/actions/runs/35200576531) passed its existing backend/frontend validation and published exact-commit images/source. The user explicitly requested fast deployment without extra tests or monitoring: no additional local suites, browser/performance exercises or extended observation ran. Source diff, backup, unchanged schema, candidate warm-up/runtime and four immediate health checks passed. Extended observation is intentionally omitted, not claimed complete.
- All four internal/public health/login checks returned HTTP 200. Both routers validated; two workers per backend slot, zero restarts/OOM/error markers. PostgreSQL 25/100 connections, zero invalid indexes. Disk 73% backend / 65% frontend. Historical audit/security risks remain unchanged.
- Source manifest `7a66be5dc40b82ef6365481ba46ab8f674165f688fa150103b6397a6d136d2c4`, archive SHA-256 `beabef64491c3e1640569818f428140c177c5a884223b8ae382457421e2b6d02`, 866 source files verified against the exact Git commit. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:5354c34bcd8e092237449df67008e79682cbcac7af5f687bf1705d33cacf8fa7`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:6291d8db0df5319b239b8d71e413177a00b3adee6c2db9da41e95cc6c1eaee15`.
- Backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260917_083623.dump`: mode 0600, 53785766 bytes, 1202 restore objects; SHA-256 `12ae1df49cc785bcc749f105fb78ae07822a8094cdfcd266b055d77fab872241`, restore-list SHA-256 `e983f1bfd3347a367a8c3c0857fc93d6f7f6c90eb1576feb0b7682dc6a104860`. Backup hashes/freshness verified before staging. Application rollback uses the retained green release without database downgrade.
- Worktree `C:/ERP/.codex-work/form-borders-fabric-arrival-20260917`; implementation branch `codex/form-borders-fabric-arrival-20260917` pushed/merged/deployed; record branch `codex/record-form-borders-fabric-arrival-20260917`. Evidence `outputs/deployment/evidence.json`; legacy checkout preserved; context mirrored to Obsidian.

## Sewing corrections, QR groups and Eco fabric tracking deployed (2026-09-17)

- LIVE: backend/frontend green release `20260917_081317`, exact application commit `c0c11d473e42e26037cfe686304109139ae24b93`, merged through PR #167. Both source manifests, image identities, current symlinks and slot states agree. Blue `20260917_052400` remains running for rollback. Frontend activated `2026-09-17T08:24:38.751935+00:00`.
- Saved Sewing Input/Sewn/Passed records now have guarded Edit/Delete with audited assignment/work-order total corrections and batch labels. QR Control orders collapse independently with white/yellow/green scan states. The new Milana-only Fabric to Eco Cotton section supports scan dispatch, dated counts/weights/history, scan returns, stable roll identities and automatic PDFs containing sent items plus remaining inventory. No Eco inventory or production records are created.
- Applied migrations `0130_eco_fabric_transfers` and `0131_sewing_corrections`; DB is `0131_sewing_corrections`. The new custody tables are created and sewing correction metadata added. Only existing Mubina user #8 received the new `inventory.eco_transfers` grant; before/after verification confirmed all other account grants/policies unchanged. No stock transfers, sewing edits, or other business transactions were performed during deployment. Startup seeding remained disabled.
- Required immutable CI [35198520915](https://github.com/Shmirzaev/Milana-ERP/actions/runs/35198520915) passed and published the exact commit. Prior local validation: 980 backend tests, PostgreSQL migration/concurrency checks, frontend lint/strict types/contracts/build and user-reviewed browser screenshots/PDFs. The user explicitly requested fast deployment without extra testing or monitoring: no additional local suites, signed production browser/performance exercises or extended observation ran. Immediate required backup, migration, warm-up, source/runtime and four health checks passed; extended observation is intentionally omitted, not claimed complete.
- All four internal/public health/login checks returned HTTP 200. Both routers validated; two workers per backend slot, zero restarts/OOM/error markers. PostgreSQL 25/100 connections, zero invalid indexes. Disk 73% backend / 65% frontend. Unrelated historical audit/security risks remain unchanged.
- Source manifest `7189df4af38416cd8f35089f89ddec3d60286f386039dedf75f9db269adce2ea`, archive SHA-256 `92feb9130f3b24b680a3c4855768e6c50a92f2a4427fdf80a268f2d9db4468bc`, 866 source files checked against the exact Git commit. Backend image `ghcr.io/shmirzaev/milana-erp-backend@sha256:5adee98dc2db36b00a0f2ba051770bedca3432c2076bed338fe99e7db700b6cf`; frontend image `ghcr.io/shmirzaev/milana-erp-frontend@sha256:cacc94967cb30ed9052e9b0ab921a35480c2ac36c072b831d287dfa9964cfa87`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260917_081317.dump`: mode 0600, 53776374 bytes, 1178 restore objects; SHA-256 `5dc01b859444cc0671d768f5775b19daffb4affb4856f16b061672187b110b4c`, restore-list SHA-256 `acb01e447c7d8928d9377d0717502f483122f4ba59a671873a4bf7302b70cffb`. Application rollback uses retained blue release without database downgrade; preserve custody history after any new dispatch.
- Worktree `C:/ERP/.codex-work/sewing-qr-eco-transfers-20260917`; implementation branch `codex/sewing-qr-eco-transfers-20260917` pushed/merged/deployed; record branch `codex/record-sewing-qr-eco-transfers-20260917`. Evidence `outputs/deployment/evidence.json`. Legacy checkout preserved; durable context mirrored to Obsidian.

## Sewing corrections, QR groups and Eco fabric tracking prepared (2026-09-17)

- PREPARED FOR SCREENSHOT REVIEW, NOT DEPLOYED: worktree `C:/ERP/.codex-work/sewing-qr-eco-transfers-20260917`, branch `codex/sewing-qr-eco-transfers-20260917`, based on verified origin/main `e0516965c497424df3a1c9e91d8766b4976cf844`. Both production manifests/slots and deploy/production-base.json matched active blue `20260917_052400` (application commit `42cf6a67cf3cf0e149b60ffc8ca52e943020986c`), rollback green `20260916_033607`, DB `0129_user_access_policy`. No production business data, permissions, migrations, releases or services were changed; legacy checkout work was preserved. Merge/deployment require the user's approval after screenshots.
- User clarified Edit/Delete means saved Input/Sewn records on the Sewing page, not Daily Sewing Reports. Added audited, version-checked corrections with assignment/work-order total updates, actual upstream and size validation, human batch labels and protection for Packaging/defect/replacement-linked work. Legacy assignment reconciliation is allowed only when unambiguous. Packaging handoff serializes with corrections.
- QR Control now collapses independently by order. White/yellow/green means none/part/all active QR labels scanned. Full factory/order counts remain correct across pagination, filtering and search; superseded labels do not affect status.
- New Milana-only Fabric to Eco Cotton section scans existing roll labels, queues dispatch, issues inventory on Mark sent, records dates/operator/counts/weights, and restores exact original batch/roll quantities on scan return. There are no Eco inventory/production writes. Request identities and PostgreSQL locks prevent duplicate dispatch/return effects. Reservations and availability are enforced. Sent rolls are excluded from available inventory counts/reprinted labels without renumbering. Batch mutation is blocked while rolls remain away.
- Each dispatch auto-downloads one EN/RU/UZ PDF with sent items plus remaining-inventory tables frozen at dispatch time; download remains available from history. Previously used batches require recorded roll weights instead of guessing. Migration `0130_eco_fabric_transfers` creates the custody ledger and narrowly grants the existing verified Mubina account `inventory.eco_transfers` while preserving other grants/restrictions. Migration `0131_sewing_corrections` adds assignment contributions and optimistic correction versions. The grants remain pending deployment. Do not downgrade away custody history; 0130 refuses downgrade after use.
- Validation: full backend suite **980 passed**, one existing Starlette deprecation warning; real disposable PostgreSQL migration/grant and concurrent duplicate dispatch/return/stale edit checks passed. Frontend contracts, optimized 88-page build, lint, strict types, Ruff and whitespace checks passed. Actual compiled QR/Eco pages plus the actual Sewing page component passed local browser edit/delete, scan/send/PDF/return, restricted Mubina visibility and mobile-overflow checks with disposable QA data. English/Russian PDFs were rendered and visually reviewed; all three PDF languages were generated in tests. Physical camera/scanner hardware was not exercised.
- Review details: `docs/SEWING_ECO_REVIEW_2026-09-17.md`; screenshots/PDF under this worktree's `outputs/preview/`. Production remains unchanged pending review. Historical security/audit risks are not claimed fixed.

## Batch-scoped sewing receipt deployed (2026-09-17)

- LIVE: backend/frontend blue release `20260917_052400`, exact application commit `42cf6a67cf3cf0e149b60ffc8ca52e943020986c`, merged through PR #165. Both current symlinks, source manifests, image identities and slots agree. Green `20260916_033607` remains running for rollback. Frontend activated `2026-09-17T05:29:02.200122+00:00`.
- Manual sewing receiving now selects an exact production batch, displays its batch label and receives only that batch. Sibling batches remain unchanged. An explicit batch identifier (or explicit null for legacy unbatched bundles) is required; stale clients omitting it receive 422 and must refresh. An order-scoped PostgreSQL transaction advisory lock preserves shared received totals during concurrent sibling receipts. Existing factory scope, batch QR acceptance and individual scans remain unchanged. EN/RU/UZ wording says Receive batch.
- The earlier authorized PO-0182 correction remains: Batch 1 / 0204-01 / 450 pieces returned to awaiting receipt; Extra batch 2 / 0204-02 / 510 pieces remains received. That correction and its audit/backup are documented below. This application deployment made no further business-data changes, applied no schema migration and disabled startup seeding. Database remains `0129_user_access_policy`.
- Existing validation passed: full 955-test backend suite, focused receipt/factory/correction tests, real PostgreSQL concurrent sibling receipt verification, frontend lint, strict types, contracts and optimized build. Immutable CI [35185243814](https://github.com/Shmirzaev/Milana-ERP/actions/runs/35185243814) passed the standard release validation and published exact-commit images/source. At the user's explicit request, no extra local suites, signed browser/performance exercises or extended observation were run. Immediate release gates are complete; extended observation is intentionally omitted, not claimed complete.
- All four internal/public health/login checks returned HTTP 200. Both routers valid, two workers per backend slot, zero restarts/OOM/error markers; PostgreSQL 28/100 connections and zero invalid indexes. Disk 73% backend / 65% frontend. Unrelated historical risks remain unchanged.
- Source manifest `852e871e781bad40f0d4da729e1f0f40827a11209bbac386f32101f6381fd0f5`, archive SHA-256 `d272e2936ca0f405f97f07b5fde8227811b41a711e5b04339a2756d8de163687`, 850 files verified against the exact commit. Backend image `ghcr.io/shmirzaev/milana-erp-backend@sha256:378518755d180356ad3db2591e23c7639b09373a391218a8919e7e940cd631b2`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:6aade0b80d786698025d4d23910fc774ce2f60757c09b8335c28ba66548c9921`.
- Fresh verified deployment backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260917_052400.dump`: mode 0600, 53762938 bytes, 1178 restore objects; SHA-256 `8235a4614a187b0534660a7c49540a629df7bac67f8b82abad894703119a4f11`, restore-list SHA-256 `eba29be4827d58fd2e68ab21c5b7e10437d63c2881127aa609305b6269bda207`. Backup identity/freshness checked before staging. Application rollback uses retained green release and does not reverse data correction or downgrade the database.
- Worktree `C:/ERP/.codex-work/sewing-batch-receive-20260917`; implementation branch `codex/sewing-batch-receive-20260917` pushed/merged/deployed; record branch `codex/record-sewing-batch-receive-20260917`. Evidence `outputs/deployment/evidence.json`. Legacy checkout preserved; durable context mirrored to Obsidian.

## PO-0182 Batch 1 receipt reversed; batch-only receiving prepared (2026-09-17)

- The user reported that receiving one arriving batch also marked its sibling received and explicitly requested return of Batch 1 while retaining Extra batch 2. Live evidence confirmed one manual-receive action at 2026-09-17 05:00:41 UTC received all 12 bundles for PO-0182 (production order 204, model 5459): six 75-piece bundles in batch 89 / 0204-01 and six 85-piece bundles in batch 95 / 0204-02. There were no sewing assignments, daily reports, actual sewing records or packages for this order.
- COMPLETED IN PRODUCTION: guarded script `backend/scripts/reverse_po0182_batch1_receipt.py` from commit `2a23a0ef` restored only bundle IDs 2789–2794 (Batch 1, 450 pieces) to created/awaiting receipt at Cutting, keeping their next department Besttex. Extra batch 2 bundle IDs 2819–2824 remained byte-for-byte unchanged at received_sewing, 510 pieces. Work order 796 actual_input_qty was corrected from 960 to 510; its 960-piece plans, sewing status and production order remain unchanged. The production Sewing Flows payload now exposes only batch 95 / 510 pieces as available received work for PO-0182.
- All 12 original creation scans and all 12 original receiving scans remain; six explicit receive_sewing_reversed scans were appended. System audit entries 20566–20572 retain complete before/after bundle and work-order snapshots and the user's reason. Their seven-entry chain segment validates; the unrelated historical chain risk is not claimed fixed. No stock, payroll, cutting output, package, shipment, permission, schema or unrelated business row was changed.
- Verified full backup `/opt/milana-erp/shared/backups/pre_po0182_batch1_receipt_20260917_051156.dump`: mode 0600, 53,759,817 bytes, 1,178 restore objects, SHA-256 `5b052ee30f48e0b2d50b85489ae1cf5ccf4aa331ae58da4f6489400af3eccbaa`; restore-list SHA-256 `3d7b7c7550d234def88bba440b1f8c2090583f4239e17514d195959d0d556302`. Reversal of this correction must preserve newer work and historical logs; do not restore the entire database over later transactions.
- PREPARED, NOT DEPLOYED: manual sewing options now group by order/batch/model and show a Batch column. EN/RU/UZ actions say Receive batch. The POST requires an explicit production_batch_id and filters that exact batch together with order/model/factory. Explicit null selects only legacy unbatched bundles; omitted batch is rejected with 422 so stale clients cannot receive the whole order. Batch metadata is loaded in one bounded query. Batch QR acceptance and individual bundle scans retain their existing behavior. No schema or permission change is required.
- Validation: 45 selected receiving/factory/reference/workflow tests and six correction tests passed, including all three factories, wrong/missing batch, duplicate receipt, sibling preservation, queue visibility, rollback and changed-evidence rejection. Frontend lint, strict TypeScript, all build contracts including the actual manual-receive handler, and optimized 87-page build passed. Scoped/global backend Ruff, Python compilation and whitespace checks passed. Full backend suite passed 955 tests. Final PostgreSQL-specific validation used a disposable local PostgreSQL 16 instance: two simultaneous sibling-batch receipts invoked the actual handler, the second waited on the order-scoped transaction advisory lock, each received only its own batch, and the shared received total was exactly 960. The test container was removed. This lock serializes manual receipts without reversing the existing bundle-to-work-order lock order used by individual scans. The standalone i18n checker reports the same 22 unrelated missing-key uses on the unchanged compact-access record baseline; changed EN/RU/UZ keys are present in both source and runtime dictionaries.
- All four production internal/public health/login checks returned HTTP 200 after the data correction. Both source manifests and runtime slots matched before work: active green `20260916_033607`, rollback blue `20260915_121653`, database `0129_user_access_policy`. No deployment, merge, migration or restart occurred. The batch-only code needs deployment authorization; until deployment the current manual-receive UI remains order-wide. Worktree `C:/ERP/.codex-work/sewing-batch-receive-20260917`, branch `codex/sewing-batch-receive-20260917`; legacy checkout preserved.
...
- LIVE: green backend/frontend release `20260916_033607`, application commit `53b3f383477adf1e84c94eef0a659a6e30d19f51`, merged through PR #162 (merge `cd1ca37a97005455a4f5ad4b9d9886dc26de01ac`). Backend switched at `2026-09-16T03:48:47Z`, frontend at `03:48:48Z`. Both symlinks, active slots, verified source manifests and immutable images agree. Blue `20260915_121653` remains live for rollback. This supersedes the prepared/pending entry below.
- Create and Edit User now use compact labelled checkboxes, two columns on wider screens and one on phones. Tick grants access; untick explicitly denies it, including role-inherited rights. All 78 keys, search, factory scope, EN/RU/UZ help and protected administrator controls remain. Backend authorization and schema are unchanged; no new migration or data backfill. Existing access settings are preserved until an administrator saves changes.
- Validation: all 942 backend tests, Ruff/compile, 11 observation-tool tests, frontend lint, strict TypeScript, complete workflow contracts and optimized build passed. Immutable workflow `35052417960` passed. Signed disposable browser QA verified Create/Edit inherited denial, additional grants, factory isolation and save/reload/reopen persistence. Screenshots reviewed at 667x868; 390x844 uses one column with no horizontal overflow. No browser console errors. Post-cutover signed production QA verified both forms and previewed a Planning denial, then cancelled without saving.
- Both verified backups remain: the pre-change database/source/runtime snapshot is described below; the second database backup is `/opt/milana-erp/shared/backups/milana_erp_pre_20260916_033607.dump`, 53,612,532 bytes, 1,178 restore objects, SHA-256 `e9fffecea07e5980acbde0a3b27ece4592954067a958288370302cd54faa7e3b`, restore-list SHA-256 `1990a4888e7f2a320e4a8295a9bec4ac19030606c50eb94acd3cc9a880837558`, verified mode 0600. Database stays `0129_user_access_policy`; seeding disabled. All 25 production user permission/factory/admin fingerprints matched before cutover (`f9fafc211bc790a974481f5cba03fc581c89872936bfbac311594897e44ac240`). No production users, access settings or business records were changed by this task; ordinary sign-in/activity timestamps may update.
- Candidate and immediate runtime/four internal-public health checks passed. Both backend slots have two workers, zero restarts/OOM/error markers. Immediate database headroom: 24/100 connections, zero invalid indexes. Disk: backend 73% (above the 70% alert threshold, below the 80% staging block), frontend 65%; no cleanup performed. Candidate performance passed all five endpoints with identical rows/payloads; maximum median change +6.62%, maximum p95 +7.06%. Signed public post-cutover baseline returned HTTP 200 for every request.
- OBSERVATION COMPLETE: the HIGH-risk observer ran for 1800.23 seconds from `2026-09-16T03:49:50.522494Z`, completing 61 rounds / 244 probes with zero failures or monitoring gaps. Closing active/rollback source, slot, router, image, worker, restart/OOM and strict-log checks passed. PostgreSQL 33/100 connections and zero invalid indexes. Closing old-versus-live performance passed every endpoint (maximum median change +1.72%, maximum p95 +1.55%; identical rows/payloads). The single fixed 100-sample public confirmation versus the immediate post-cutover baseline passed (maximum median -1.05%, maximum p95 -0.85%; identical rows/payloads). Blue rollback remains available. Evidence: `C:/ERP/.codex-work/compact-user-access-20260916/outputs/deployment/20260916_033607-observation.json` and `evidence.json` in that folder.
- Public-path investigation: the first closing 21-sample benchmark failed p95 for global search (+57.62%), inventory batches (+263.18%) and inventory stock (+96.79%), despite unchanged payloads, passing direct old/live comparison and near-unchanged public medians. Completion stopped and all failed evidence was retained. A predeclared rotating diagnostic took 50 samples each of public health, public login and stable internal health, capturing DNS/TCP/TLS/first-byte/total times: all HTTP 200; public-login DNS maximum 327.068ms, versus internal-health total maximum 4.536ms. This supports intermittent public-path/DNS contribution, but cannot prove the cause of every original slow API sample. Exactly one predeclared larger public confirmation (100 measured samples per endpoint plus three warmups, all 515 raw samples retained) passed all original thresholds; every median and p95 improved against the initial public baseline. No code, DNS or traffic configuration changed during the investigation, and no repeated-until-pass loop was used. Final runtime/strict logs/four health checks passed again. Evidence: `public-closing.json`, `network-diagnostic.json`, `public-confirmation-raw.json`, and separate failure/confirmation records in `evidence.json`.

- Manifest `2eace9a6a0f8fdf1cb15acb87be8d45c9be5dbb4d2976db42418effb4ff97c32`; archive SHA-256 `e726ff7f991a352056f7d262170d03d328cbaf8b76207733a91b78c242a01396`, 847 Git-verified files. Backend digest `sha256:2aa45839908ca3831921435c77753f0878a85535f760b1933922b21b65ed9a41`; frontend digest `sha256:08b2422b0b88792ee1cf58d554d000e9f474966cf7b07eb2bcb26a17b0b92a9a`.
- Application worktree/branch: `C:/ERP/.codex-work/compact-user-access-20260916`, `codex/compact-user-access-20260916`. Live record worktree/branch: `C:/ERP/.codex-work/compact-user-access-record-20260916`, `codex/record-compact-access-20260916`. Rollback restores blue `20260915_121653` via DEPLOYMENT.md with no database downgrade or business-data restore; both versions understand current access policies. Preserve later deliberate account changes.
...
- LIVE: backend/frontend blue release `20260915_121653`, reviewed application commit `3a379265f226aa7f7a45a238f78cd2a34b86336c`, merged through PR #159. Backend activated at `2026-09-15T12:38:41Z`, frontend at `12:38:42Z`. Both active slot states, symlinks, source manifests and exact images agree. Green `20260915_113521` remains running for rollback. This supersedes the staged/pending entries below.
- Users now has the complete 78-key access catalog in both Create and Edit, searchable EN/RU/UZ labels, factory-specific Use default / Allow / Deny and effective-access preview. Deny removes inherited role access for the selected person. The migration added only nullable `users.access_policy`; existing accounts keep their previous behavior until explicitly configured. Existing combined action permissions retain their granularity; shared workflow lookup reads and special administrator controls remain protected.
- Pre-activation signed local browser QA verified create/edit controls, inherited denial, added grants, isolated factory grants and effective preview; screenshot reviewed, no console errors. Supplementary 667x868 layout inspection matched the supplied screenshot width, confirmed no horizontal page overflow and usable scrolling, and reset the temporary viewport afterward. After activation, signed production browser QA verified Create/Edit and Planning's inherited production access, previewed a denial and cancelled it without saving. All 25 existing users' effective permission/factory/admin fingerprints matched the original snapshot immediately before activation; no production access settings or business records were changed. Ordinary sign-in/read activity timestamps may update.
- Local/CI validation, both verified backups, nullable PostgreSQL migration, image hashes, performance investigation and confirmation are recorded in the staged entry below. Immediate runtime and four internal/public health checks passed. Both backend slots have two workers, zero restarts/OOM/error markers; database 24/100 connections and zero invalid indexes; disk 72% backend / 64% frontend. All five signed public benchmark endpoints returned HTTP 200 after activation; this is the baseline for the closing public comparison, while old/candidate slot comparisons separately passed before activation.
- OBSERVATION COMPLETE: the HIGH-risk observer ran for 1,800.19 seconds from `2026-09-15T12:39:54.809964Z`, completing all 61 rounds / 244 health probes with zero failures or monitoring gaps. Closing checks also passed: both active/rollback source manifests and runtime slots remain correct, two workers per backend slot, zero restarts/OOM/log error markers, PostgreSQL 25/100 connections and zero invalid indexes. Old-versus-live closing performance passed every endpoint (all medians improved; maximum p95 change +0.18%; identical rows/payload sizes). Public closing versus immediate post-cutover baseline also passed (maximum median +2.44%, maximum p95 +1.70%; identical rows/payload sizes). Evidence: `C:/ERP/.codex-work/user-access-20260915/outputs/deployment/20260915_121653-observation.json` and `evidence.json` in the same folder. The retained green release remains available for reviewed rollback.
- Database is `0129_user_access_policy`, seeding disabled. Active manifest `9d176d0627dbde7dfda7d45c6ed00817b2c57bb4fd4f4040c70af5207aa7fb91`. The original full database/source/runtime/access snapshots remain available for a reviewed rollback that preserves newer business transactions; old code ignores new denial policies, so review later account changes before reverting.

## User access release staged; activation pending (2026-09-15)

- Production still serves green `20260915_113521` on both VMs. Candidate blue `20260915_121653` is staged and warmed but has not received production traffic. Reviewed application commit `3a379265f226aa7f7a45a238f78cd2a34b86336c` is pushed and merged via PR #159 (merge `05d6a683d4ea61781a954c95abe37ced673c228a`). No observation window has started. Do not claim this editor is live.
- The complete 78-key permission catalog is available in shared Create/Edit controls, with factory-specific Use default / Allow / Deny, searchable EN/RU/UZ labels and effective-access preview. Existing permission granularity is retained: no promise of independent read/write/delete controls where the application currently combines these actions. Shared workflow lookup access and special administrator operations remain separate constraints.
- Backups were verified before implementation; original application, runtime configuration, database and private user/role access snapshot are preserved as detailed below. A second verified backup immediately preceded migration: `/opt/milana-erp/shared/backups/milana_erp_pre_20260915_121653.dump`, mode 0600, 53,551,998 bytes, 1,178 restore objects, SHA-256 `b7d04cbc6594ada22978e18616f366606327a087b6488d0cfec553f554ba8349`; restore-list SHA-256 `8da7a35168c2ee871241a5b05e0a7d34352e537201ecf786bd2201978bf19e46`.
- Production database advanced to `0129_user_access_policy`. This adds a nullable JSON column only, with no permission or business-data backfill. Startup seeding stays disabled. All 25 existing users have identical effective permissions, administrator status and factory availability on old and candidate code; fingerprint `f9fafc211bc790a974481f5cba03fc581c89872936bfbac311594897e44ac240`. No production user, role, order, stock, shipment or other business record was created or edited by these checks. User sign-in/read-only browsing can update ordinary activity timestamps.
- Validation: 942 backend tests, focused access/auth/factory regressions, Ruff/compile, strict TypeScript, ESLint, frontend workflow contracts, optimized build and disposable PostgreSQL up/down/up migration checks passed. Immutable workflow 34967950447 passed and published both exact-commit images. Signed candidate API checks verified the 78-key catalog, allow/deny preview, factory isolation, invalid-policy rejection and unauthorized-manager rejection without persisting preview data.
- Candidate runtime/manifests/routers and four internal/public health probes passed; both backend slots have two workers, zero restarts/OOM and zero recent error markers. PostgreSQL was 24/100 connections with zero invalid indexes. Disk remains 72% backend / 64% frontend.
- Initial 21-sample model-group benchmark failed the p95 limit (+31.46%, 69.13 to 90.88ms); activation stopped and that evidence is preserved. Permission-resolution cost was only 4.4us old / 16.5us candidate. A predeclared larger alternating comparison (100 samples per endpoint per slot, every raw sample retained) passed all thresholds; model-group median -1.00%, p95 -1.26%. One subsequent standard confirmation also passed all endpoints (maximum median +3.30%, maximum p95 +5.00%, identical row counts/payload sizes). The initial tail delay did not reproduce; it is not silently discarded or represented as an initially clean run.
- Browser validation remains incomplete: exec automatic approval review rejected starting the localhost-only frontend preview, including after explicit user approval, with only `blocked by policy`. The user was asked to start port 13090 manually; isolated disposable backend runs on 18090. The user has signed into production Chrome for later read-only QA. No production activation is allowed until the changed-editor browser check passes; then rerun current-state/backup-freshness/candidate gates, activate backend first and frontend second, complete immediate signed QA, the full 30-minute high-risk observer and closing runtime/performance checks.
- Candidate manifest `9d176d0627dbde7dfda7d45c6ed00817b2c57bb4fd4f4040c70af5207aa7fb91`, archive SHA-256 `ded2be35d362583bd435aed2c34ce05431372c78ba9ccdce7b7686fcd7168680`, 847 verified files. Backend image digest `sha256:9133a1d151612f9638657c05e41728786e302b06d4477d4cda317034d33daa5f`; frontend image digest `sha256:3a773a1e420bfa15cebd1100ed960f5320a26113ec986097247aee668c142624`.
- Worktree `C:/ERP/.codex-work/user-access-20260915`, application branch `codex/user-access-20260915`. Documentation checkpoint uses `C:/ERP/.codex-work/user-access-record-20260915`, branch `codex/record-user-access-20260915`; keep its pending-state documentation separate from the immutable application commit. Detailed evidence and rollback procedure are under the application worktree's `outputs/deployment/`.
- Before rolling back after users have configured policies, review those account changes: old code ignores policy denials. Restore only reviewed access settings using the private snapshot; do not overwrite newer business transactions with the original full database dump. Keep the nullable column; unsafe downgrade is deliberately blocked. Restarting an old image against an unknown migration head needs a reviewed compatibility procedure; the retained running green application remains available.

## User access editor prepared; deployment pending (2026-09-15)

- Worktree `C:/ERP/.codex-work/user-access-20260915`, branch `codex/user-access-20260915`, starts from verified origin/main `2b420911377d138d7e19397460c410add1604ac5`. Both production manifests and runtime states match green `20260915_113521`; blue `20260915_102154` remains the prior rollback. No production application or access settings have changed at preparation.
- Before any implementation edits, created and verified a fresh PostgreSQL backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260915_120140.dump`: 53,514,907 bytes, 1,178 restore objects, SHA-256 `a4a4793339d115e4be93dac9d2e0d30d77b37002a6bf9fc66054fc8ef4b57ef3`. Restore-list hash `256c626b21e18fb02a02a77c5a31ff0479b2d209b1346d8e26de518dc3c93fb3`. Both VMs also retain mode-0600 source/runtime archives under `/opt/milana-erp/shared/backups/user-access-20260915_120140`; the backend has a separate private user/role access snapshot. Local checksums/state evidence: `outputs/deployment/before-change.json`.
- Create and Edit share a searchable EN/RU/UZ permission editor backed by a complete named server catalog. Each factory supports Use default / Allow / Deny plus a server-calculated effective-access preview. Existing role and extra permissions remain intact; nullable `users.access_policy` provides explicit per-factory overrides. Secondary factories never inherit a normal user's primary role. Super Admin tier can only be configured through the primary factory; administrator assignment and last-admin protections remain.
- Migration `0129_user_access_policy` only adds a nullable JSON column, with no backfill or automatic user/role changes. Downgrade refuses to discard configured policies. Existing accounts without policies retain their previous permissions and Sewing workspace behavior. Explicitly configured Sewing accounts can use granted non-Sewing pages; name/department pricing and management-task/package-approval fallbacks honor explicit denials. A catalog coverage test prevents existing authorization keys from being omitted from administration.
- Permissions retain the existing business capability granularity: e.g. sales.orders includes creating/editing/deleting orders, while payroll approval/payment and purchase request/approval/receipt remain separate. Shared lookup data may be readable through another granted workflow. This change does not invent independent read/write controls for every individual endpoint.
- Preparation validation: full backend suite 942 passed; focused access/auth/admin/factory suite 35 passed; frontend strict TypeScript, ESLint, all existing workflow contracts, and optimized build passed. Dedicated frontend policy/factory/locale tests pass. Disposable PostgreSQL 16 exercised actual upgrade/downgrade/upgrade, preserved existing values, and blocked unsafe downgrade. Final immutable CI, visual QA, candidate parity/performance checks, fresh pre-cutover backup, activation and 30-minute high-risk observation remain required.
- Rollback must preserve new factory transactions: prefer application rollback plus a reviewed, selective restoration of user/role access settings from the private snapshot. Do not restore the full pre-change database over later production work. Before reverting to code unaware of access_policy, inspect and resolve all policies and any newly created accounts; otherwise denied permissions would be ignored. Leave the additive column in place for a running retained old container; restarting older code after the new Alembic head requires a reviewed compatible startup procedure.


## Two manual test shipments deleted (2026-09-15)

- At the user's explicit follow-up request, deleted the two screenshot-identified test shipments SH-2026-000001 (Nigina Buxoro, shipped, eight packs/462 pieces) and SH-2026-000002 (Zafar Aksu, created, empty). The generated test order SO-0001 and unpaid invoice INV-2026-000001 for 3003.00 were removed together; there were no linked payments or external invoice references. The remaining eight test packages and consumed stock from PRN-2026-000003 were also removed, completing the prior test-run cleanup without returning test quantities to available inventory.
- Exact transaction removed two shipments, eight shipment links, eight shipment scans, one invoice, one sales order/item, eight packages/items/stock rows and sixteen package scans. Two cached shipment retry fingerprints were invalidated so old requests return a conflict instead of replaying deleted shipment data. Both customer profiles, all payments, original receipt/print-run/member evidence and prior audit history remain. A new audit event records the removed rows and exact reason.
- Before mutation, both active manifests/slots matched the Git production baseline, all foreign-key dependencies were inspected, the exact selection was previewed, and a fresh verified mode-0600 PostgreSQL backup was taken: `/opt/milana-erp/shared/backups/milana_erp_pre_20260915_115113.dump`, 53511350 bytes, 1178 restore objects, SHA-256 `14a5903fb970eed722d1ecd0820911597bcdb965797886a9c89127fcced66172`; restore-list SHA-256 `1d54449f56478cf79bb0e9bd35b7e442770ca996acb3d9f6d2cf21d588b4865d`. Row locks and repeatable-read transaction checks guarded the exact IDs; fingerprints verified unaffected rows in touched tables, customers, payments and immutable evidence stayed unchanged. A separate read-only postcheck confirmed both shipments, test order/invoice and all twelve run packages/stock rows are absent while both customers and twelve immutable print members remain. All four immediate health/login checks returned HTTP 200.
- Data-only cleanup: active green release remains `20260915_113521`, rollback blue `20260915_102154`, database `0128_selected_pack_labels`. No application deployment, migration, restart, new business record, broad test suite or extended monitoring. Existing unrelated risks remain unchanged.
- Worktree `C:/ERP/.codex-work/delete-test-shipments-20260915`, branch `codex/delete-test-shipments-20260915`. Reviewed one-off operation and backup/result evidence are under ignored `outputs/`. Legacy checkout preserved; this durable record is mirrored to Obsidian. This follow-up supersedes the previous choice to retain the test shipments/accounting.

## Warehouse invoice and selected manual labels deployed (2026-09-15)

- LIVE: backend/frontend green release `20260915_113521`, reviewed application commit `da9a94e92626f617be826f2eedc470035c032386`, merged through PR #156. Both source manifests, image identities, current symlinks and runtime slots agree. Blue `20260915_102154` is retained for rollback. This supersedes the preparation record below.
- Printed invoices omit Narx/Summa columns, financial totals/adjustments and the net-price/tax sentence. Saved prices, amounts and customer ledger balances are preserved. Manual aggregate packs display every configured size from the immutable receipt; no per-size quantity is invented. Invoice footer identifies the dispatching warehouse user (Omborchi); historical prints use the shipment audit actor when available without rewriting financial snapshots.
- Manual shipping has Add client (name, optional phone), saving/selecting the client in place. The scoped creation route requires storage.shipment. EN/RU/UZ controls and existing broader customer permissions remain consistent.
- Manual print runs now support selected-pack deletion, including after shipping. Unused available stock is physically removed; shipped labels are retired while shipment/package/stock/finance history remains. Pending shipment/reservation/adjustment/count links still block physical deletion. PostgreSQL migration `0128_selected_pack_labels` adds monotonic retired package IDs and extends the guarded deletion function; original receipt/member immutability remains. No label numbers are reused, retired labels cannot print and deleted receipts cannot be replayed. Rollback must retain selected-label retirement handling; migration downgrade after deletion is blocked.
- User-authorized cleanup was limited to PRN-2026-000003 (12 packs, 702 pieces). Removed four available test packs PKG-2026-000020 through PKG-2026-000023 (240 pieces) and their available stock; retired all 12 labels. Eight shipped packs PKG-2026-000012 through PKG-2026-000019 (462 pieces), their stock/shipment scans, shipment #6, linked order and client invoice were verified unchanged in the cleanup transaction. No other run, customer, order or stock was deleted. Original receipt, membership snapshots and audit history remain.
- Validation: 58 focused backend cases, scoped Ruff/compilation, strict TypeScript, ESLint and diff checks passed. Disposable PostgreSQL 16 exercised the actual migration up/down/up and selected physical deletion with immutable history. Immutable CI [34964102571](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34964102571) passed all 929 backend tests, frontend validation, optimized build and exact-commit image publication. A read-only check of the existing shipment confirmed all saved sizes, the Storage warehouse user, omitted price/tax text and persisted cleanup. Candidate warm-up and all four immediate internal/public health/login checks passed HTTP 200. No extra browser/performance exercises or extended monitoring ran per the user's explicit instruction.
- Database is `0128_selected_pack_labels`; production seeding stayed disabled. Runtime: two workers per backend slot, zero restarts/OOM/error markers, database 26/100 connections and zero invalid indexes. Disk 72% backend / 64% frontend; existing disk pressure and unrelated historical risks remain.
- Manifest `fc7c5c3d9ffdfbb825e2758960e4841c20cb7493a2d69947f238f934d9d4a51c`, archive SHA-256 `f6230514195e8fd86e0293ca6c727f81a64a80144f4ccb3c66680bca8155aec0`, 838 verified source files. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:a93dd92d667ea2014d7c0245b36bbe9a983df22f0736cb8f6dfbf0c488c47ce5`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:a13f9b8e2acb746149f6a75a3a42ea1da5267c553b6efb300df9433735522b76`.
- Verified pre-migration/cleanup backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260915_113521.dump`: mode 0600, 53508760 bytes, 1176 restore objects, SHA-256 `3a838248323d129227a95d8db24155603e7ebd90a2f8f5c5a7a4f73c4e1850fe`; restore-list SHA-256 `432207090434ee2668bdb17c8f5c3087fdc04717ac84a6c62c0d555176c5094f`. Backup freshness/hashes verified before migration, staging and cleanup.
- Worktree `C:/ERP/.codex-work/warehouse-invoice-20260915`; implementation branch `codex/warehouse-invoice-20260915` pushed/merged/deployed. Record branch `codex/record-warehouse-invoice-20260915`. Evidence: `outputs/deployment/evidence.json`; legacy checkout preserved.
...
- Cutting Workshop now displays Sewing factory beside each production number, using the existing English/Russian/Uzbek labels for Milana, Besttex and Eco Cotton. Inbox production context obtains the assigned sewing work-order department through one bulk query. Missing assignments display a dash; the column describes the order's assigned factory and does not change bundle routing, assignments, quantities, permissions, statuses or other business data.
- Preparation passed four focused backend cases covering all three factories and the existing inbox display, Ruff, scoped ESLint, strict TypeScript and diff checks. Immutable CI [34957552469](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34957552469) passed standard backend/frontend validation, optimized build and exact-commit image publication. No extra local tests, signed browser/performance exercises or extended monitoring ran during deployment, per the user's explicit instruction.
- No schema/business-data changes; database remains `0127_manual_pack_deletion`, startup migration was a no-op and seeding stayed disabled. All four immediate internal/public health/login checks returned HTTP 200. Routers/manifests valid, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 25/100 connections and zero invalid indexes. Disk 72% backend / 64% frontend. Existing disk alert and unrelated historical risks remain.
- Source manifest `729f93cf96a028e84b4920ae3e5472feedae568d7064747509c22e9203bf8098`, archive SHA-256 `ef37d80b0ee4fe934a3d0923d6f245b59ef44458b6f01ce7656c9709a7ebf779`, 836 verified source files. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:0593bfcb04768bb4bd8bd6e10089fc349f2ea05db1aa08762fcd9b65289dc53d`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:931682c61e4b75f17a455bb6de2fba643db11f97471d965f6b057170e06e7f54`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260915_102154.dump`: mode 0600, 53489842 bytes, 1176 restore objects, SHA-256 `c3bb600de7cf12597fc5717d054c6224c589bf526dfc3f88bae7156b954291b9`; restore-list SHA-256 `6b5a7190e01d2c3aaa8eaab026c5aa454770dbe8a77bd599cec5ff04c5ef1331`. Backup hashes/freshness checked before staging.
- Worktree `C:/ERP/.codex-work/cutting-sewing-factory-20260915`; implementation branch `codex/cutting-sewing-factory-20260915` pushed/merged/deployed; record branch `codex/record-cutting-sewing-factory-20260915`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved; context mirrored to Obsidian when accessible.
- Outstanding separate issue: after the validation-message release, the user supplied another screenshot still showing `422: [object Object]`. Production source/manifests were verified to contain the formatter, but the browser's loaded code and exact failed response were not captured before the user switched to this factory-column request. The original rejected field and repeat display remain unresolved; this column release makes no additional validation fix.
...
- LIVE: backend/frontend blue release `20260914_131740`, reviewed application commit `e7d8823b548279dff0e0e64cd1246392de43a7f8`, merged through PR #149. Both manifests, images, current symlinks and slots agree. Green `20260914_110313` remains running for rollback. Frontend activated `2026-09-14T13:28:33.158986+00:00`.
- Root cause: the earlier manual deletion tried to DELETE immutable `package_print_run_members` and the print run. Production PostgreSQL rejected this with `Package receipt evidence and print membership are immutable`, rolling back the request and producing HTTP 500. The earlier SQLite-only deletion regression omitted the PostgreSQL evidence triggers.
- The corrected operation retains original receipt, print run/members/snapshots and audit evidence, marks the run deleted, and removes only fully available unused manual packages and their operational stock. Shipment, reservation, count and quantity-adjustment links still block deletion; unavailable stock is also blocked. Active run lists exclude deleted receipts. GET/print/receive and cached creation retries reject deleted runs with HTTP 410. Repeated DELETE returns the original deleted count. Retired printed numbers and historical member IDs prevent code reuse; SQLite test IDs now use AUTOINCREMENT to match production sequence behavior.
- Migration `0127_manual_pack_deletion` adds nullable run `deleted_at` and turns member package IDs into historical references, replacing their live FK with PostgreSQL insert validation and a guarded package-delete trigger. All original receipt/member/run immutability triggers remain. Deleted runs cannot be changed/restored; downgrade refuses after any deletion. No data backfill or business record deletion ran during deployment. Before a future rollback after users have deleted packs, retain the deleted-run filtering/replay guards; old application code would expose retained historical runs. The old backend remains running, but restarting an older image against the newer migration head requires a reviewed compatible procedure.
- Validation: 23 focused package/manual-shipment tests, Ruff, one Alembic head and diff checks passed. A disposable PostgreSQL 16 database reproduced the original error, ran the actual migration up/down/up, verified guarded deletion, preserved immutable snapshots/receipts, duplicate-delete safety, new distinct label identities and blocked unsafe downgrade. It used synthetic data, no published ports and no production environment/data; its containers/network were removed. Immutable CI [34848362276](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34848362276) passed backend/frontend validation and exact-commit image publication. No extra broad local suites or extended monitoring, per the user's continuing fast-deployment instruction.
- No production stock, package, receipt, shipment, order, user or permission row was created/deleted/adjusted by this task; the user can retry the intended delete through the normal UI. Database advanced to `0127_manual_pack_deletion`; seeding disabled. All four immediate internal/public health/login checks passed HTTP 200; routers valid, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 25/100 connections and zero invalid indexes. Disk 72% backend / 64% frontend. Existing disk alert and unrelated historical risks remain.
- Source manifest `8de9f0d530abdf8af7f3ed3b85c2b1c1678602c53a485e9c0e3285d855a3e51f`, archive SHA-256 `bafaf35068896553946d17570d0c54eddbbcfa56cbf2999ad0fa9d1e89d39b4c`, 836 verified files. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:acaefbcc0988be972ac2bad2dbe3c7f482b33316e2c9891ea47a5affd1ce05a1`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:3d04048c30184ae0b5d21a6e8f864af072796fad1f1545f0e5b85da354a4761d`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260914_131740.dump`: 53419740 bytes, mode 0600, 1171 restore objects, SHA-256 `2bb2bceffeb1bf6f77008e69f5f568578a1eb59e5526893abcb5b5d23aa99dd1`; restore-list SHA-256 `ce430cc9cd7ad5d9c4dd0bd0aa8ba95ea33b594808aed991f96796575126e2da`. Hashes/freshness checked before migration and staging.
- Worktree `C:/ERP/.codex-work/manual-pack-delete-20260914`; implementation branch `codex/manual-pack-delete-20260914` pushed/merged/deployed; record branch `codex/record-manual-pack-delete-20260914`. Evidence: `outputs/deployment/evidence.json`, `postgres-result.json`. Legacy checkout preserved; context mirrored to Obsidian.


## Payroll sewing access deployed (2026-09-14)

- LIVE: backend/frontend green release `20260914_110313`, reviewed application commit `05bdcc87d376426e07ceda133e3872cabd5c07f9`, merged through PR #147. Both source manifests, image identities, current symlinks and slot states agree. Frontend activation completed `2026-09-14T11:14:45.606081+00:00`. Blue `20260914_073912` remains running for rollback. This supersedes the prepared section below.
- Payroll now has Sewing Flows, Milana Sewing, and Scan Bundle access through `sewing.flows`, `sewing.records`, and `sewing.bundles`. Migration `0126_payroll_sewing_access` changed exactly the existing Payroll role, preserving its previous grants and every other role. Factory assignments, secondary-factory permission tokens and users were unchanged. Daily Sewing Report retains read/export access without editing permission. No order, inventory, bundle, sewing output, payroll payment or other operational business row was created or changed by deployment.
- All 924 backend tests and frontend validation/build passed during preparation. Immutable CI [34836214633](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34836214633) passed and published exact-commit images. Candidate warm-up, immediate runtime checks and all four internal/public health/login probes passed HTTP 200. No extra local suites, browser/performance exercises or extended monitoring ran during deployment, as explicitly requested.
- PostgreSQL advanced from `0125_roll_lengths` to `0126_payroll_sewing_access`; startup seeding stayed disabled. Both backend slots have two workers, zero restarts/OOM and no matched recent error markers; routers validate, database used 27/100 connections with zero invalid indexes. Disk 72% backend / 64% frontend. Existing disk alert and unrelated historical risks remain. The old backend is retained running; restarting that old image against an unknown newer migration head requires a reviewed compatible rollback procedure. Migration downgrade intentionally preserves permission grants.
- Source manifest `1de515c8b82ede1dc7bfd67ed3406b0c0352c08cd9d180dee5937ecf74b08595`, archive SHA-256 `c0c08c36d41147592f09b49d4f2a79138ad6422cc78a0f527869d3c304c0b286`, 2461707 bytes / 834 exact-commit source files. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:2e0978f006e439c8de59b22f2d90b0f3e44b22324a4d7a73626851f3778e1880`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:56958b2b0f4a44eb5c9e111c8737c17ec390565fc1761a0d62dbd862bf284bc0`. Local Git archive CRLF conversion was disabled only for verification; release files matched exact reviewed Git bytes before upload.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260914_110313.dump`: mode 0600, 53338650 bytes, 1171 restore objects, SHA-256 `4f186330f602d855a6ae1e542164e7dc5c49c32ee2b4561a71787a7f1e99d0ff`; restore-list SHA-256 `14a5c6fd9c8258645b16170080226ca3427902c04bbc77e91a526831886d2181`. Backup hashes, size, permissions and freshness were verified before migration and staging.
- Worktree `C:/ERP/.codex-work/payroll-sewing-access-20260914`; implementation branch `codex/payroll-sewing-access-20260914` pushed, merged and deployed. Record branch `codex/record-payroll-sewing-access-20260914`. Evidence retained locally in `outputs/deployment/evidence.json`; legacy checkout preserved. Context mirrored to Obsidian.

## Payroll sewing access prepared (2026-09-14)

- Prepared, not deployed: Payroll now receives `sewing.flows`, `sewing.records`, and `sewing.bundles` in the seeded role. Existing permission-based sidebar and route guards admit Sewing Flows, Milana Sewing, and Scan Bundle with their normal operational actions. Existing Daily Sewing Report read/export access is preserved; `sewing.workspace` is not added, so the combined Sewing Floor and daily-report editing remain restricted. Existing factory/session guards remain unchanged; the grants do not assign users to other factories or alter secondary-factory permission tokens.
- Migration `0126_payroll_sewing_access` appends only missing grants to existing Payroll roles, preserving all other permissions and other roles. Startup seeding is disabled in production, so updating the seed alone would not update existing accounts. Migration downgrade intentionally retains grants because pre-existing manual grants cannot be distinguished from migration additions; access revocation requires a separate reviewed permission change.
- Before editing, both production source manifests and runtime slots matched `deploy/production-base.json`: active blue `20260914_073912`, rollback green `20260914_071001`, source manifest `eebd89a5d86d1511310598a42af180aef75ff14bb6003531f16eb586384ed33a`, verified database head `0125_roll_lengths`. Origin/main differs from the reviewed production commit only in release records. No production permissions, business data, schema, or release changed during preparation.
- Worktree `C:/ERP/.codex-work/payroll-sewing-access-20260914`, branch `codex/payroll-sewing-access-20260914`. The legacy checkout is preserved. Focused backend checks verify seeded grants, Payroll bundle receipt and duplicate rejection, cross-factory denial, and retained daily-report write denial. Migration checks cover existing grants, unrelated roles, idempotency, and downgrade preservation. Frontend checks evaluate current sidebar/route permission definitions and all 54 existing scanner factory/action cases. Validation passed: all 924 backend tests, the expanded Payroll handoff test, backend Ruff/compilation, single Alembic head, frontend lint, strict TypeScript, every frontend build contract, the optimized 87-route build, and staged whitespace checks. No signed-in production browser QA or deployment was performed.
...
- Shipment invoice printing uses the supplied Milana Premium vector logo, extracted from the user's PDF and embedded as a self-contained SVG. A4 layout retains the reference's shipment/customer/transport header and eleven item columns while using restrained rules, aligned numbers, readable size columns, localized EN/RU/UZ text, integer counts, grouped decimal amounts, clear totals and signature lines. Larger invoices repeat headings and page numbers. No external fonts/images are required. The posted footer identifies the linked ledger invoice without incorrectly claiming every manual invoice was posted on delivery.
- Frozen quantities, package grouping/shared weights, prices, adjustments and historical warnings retain their existing meaning; the renderer remains read-only. No invoice, shipment, stock, account, permission or other production business record was created or changed. No schema change; database remains `0125_roll_lengths`, startup migration was a no-op and seeding disabled.
- Validation: 33 existing invoice/shipment tests, Ruff and diff checks passed. Chrome PDF previews from the actual renderer were visually checked in EN/RU/UZ; the representative 16-row shipment fits one A4 page, and a 64-row example spans three pages with repeated headings/totals. All preview values are explicitly marked sample data. Immutable CI [34816584301](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34816584301) passed backend/frontend validation, optimized build and exact-commit image publication. No extra performance exercise or extended monitoring ran, continuing the user's explicit fast-deployment instruction.
- All four immediate internal/public health/login endpoints returned HTTP 200; routers/manifests valid, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 21/100 connections and zero invalid indexes. Disk 72% backend / 63% frontend. Existing disk alert and unrelated historical risks remain.
- Source manifest SHA-256 `1901596ac3c4d29e462ecd02b90644b960c91077c2191fc81dbf6909aa0278af`; archive 2456956 bytes / 832 verified files, SHA-256 `0443d884115a358eb66bf32270a9718a62ece140dfbdfe1953b4c4023f6ad87e`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:4043f30854a7ec0c2538c2b613f226532cb74f4a39d6b03cb344895f3218f059`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:e79459742e4ece532579c1d975f9d866dacce47f95f1aada95f8c9fba6929f92`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260914_071001.dump`: 53311476 bytes, mode 0600, 1171 restore objects, SHA-256 `5ba756cdef4789f7e2ec6424162dcc6cab5040c42534352cead1c72b7dca234d`; restore-list SHA-256 `c7eafb6eedb11beba98808818df20e385659182701fa8362d1905d1b593e17ab`. Backup freshness and hashes checked before staging.
- Worktree `C:/ERP/.codex-work/invoice-design-20260914`; implementation branch `codex/invoice-design-20260914` pushed/merged/deployed, deployment-record branch `codex/record-invoice-design-20260914`. Evidence: `outputs/deployment/evidence.json`. Sample preview: `outputs/invoice-preview/output/pdf/Milana-invoice-preview.pdf`. Legacy checkout preserved. Context mirrored to Obsidian.


## Manual pack quantities and customer shipment deployed (2026-09-14)

- LIVE: backend/frontend blue release `20260914_063958`, exact reviewed application commit `b45f8663c3460b73d6bb24c590fdcaa6ebd10761`, merged through PR #141. Both source manifests, image identities, symlinks and slot states agree. Green `20260914_041000` remains running for rollback. Frontend activated `2026-09-14T06:58:10.151812+00:00`. This supersedes the prepared section below.
- Package labels, individual reprints and print sheets show each pack's total quantity. Manual receipt now takes a quantity for every pack, supports unequal pack quantities, and removes the receipt reference/reason input. Stock enters available finished-goods inventory immediately. Mixed-size totals remain aggregate Mixed stock lines; labels use the preserved model size-range snapshot without inventing per-size quantities. Historical size-specific receipts and pending retries remain compatible.
- Print runs offers Delete mistaken packs to remove the whole unused manual receipt after confirmation. Backend rejects any reserved, used, shipped, adjusted, or shipment/scan/stocktake-linked pack and verifies full available stock. Original receipt/audit evidence remains. Deleted package/run numbers are retired so previously printed codes cannot be reassigned. This retirement safeguard depends on the new application; after actual deletions, rollback operators must retain it before permitting new manual receipts.
- Manual shipments select an existing client, scan each eligible whole pack, review missing/adjusted prices, then Ship and create invoice. Default prices come from the variants' saved sales prices. Dispatch, stock deduction, the customer accounting sales order and the unpaid invoice commit atomically from the frozen commercial total. Existing manual package provenance stays intact. Delivery is separate and cannot duplicate the invoice. Standard order-linked shipment rules remain unchanged.
- Validation: 48 focused package/shipment tests and a final 23-test affected recheck passed, plus Ruff, scoped ESLint, normal and strict TypeScript, and all frontend workflow contracts. Immutable CI [34814948083](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34814948083) passed backend/frontend checks, optimized build and exact-commit image publication. The shipment contract was narrowed to allow the client selector while retaining its removed-order-picker guard. Local Turbopack could not compile through the external node_modules junction; the clean CI and image builds passed. No extra browser/performance exercise or extended monitoring ran, per explicit request.
- No production business records, inventory quantities, orders, shipments, invoices, users or permission grants were created or changed by deployment. No schema change; database remains `0125_roll_lengths`, startup migration was a no-op and seeding disabled. All four immediate internal/public health/login endpoints returned HTTP 200; valid routers/manifests, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 22/100 connections and zero invalid indexes. Disk 72% backend / 63% frontend. Existing disk alert and unrelated historical risks remain.
- Source manifest SHA-256 `a9976c17a4b770abdf95704bee8ec3cd0c1182e1e0c3f5d181fbfc85b5e1fbca`; archive 2449529 bytes / 831 verified files, SHA-256 `fc5092e822273ecbbd221884415c180309fd0ee05335da8d4b3ed130a9c60d72`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:0f8e1ee4e047ddd6eaf4efdd8b975e84f9cbe395b2f020b4dfcc093d73561424`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:d56c74f1ff031bfd07a3c70124062a3d19a2692a313baeca1c81b3659b857391`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260914_063958.dump`: 53298028 bytes, mode 0600, 1171 restore objects, SHA-256 `58556ded1f9484e0d0e93c1ba192e8d92d3dca7a67aaad4e1667b38e6dad7783`; restore-list SHA-256 `80c06ff85d89d221109d677206f2eff3dd60608bdf66c66b6fe0b0e7a18e356a`. Backup freshness and hashes checked before staging.
- Worktree `C:/ERP/.codex-work/manual-packs-shipment-20260914`; implementation branch `codex/manual-packs-shipment-20260914` pushed/merged/deployed, deployment-record branch `codex/record-manual-packs-shipment-20260914`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved. Context mirrored to Obsidian.
...
- Eco Cotton Process Tracking is the first sidebar section, above Fabric Control and HR. Existing links, styling, translations and factory filters remain unchanged.
- Explicitly authorized account `ecoadmin@milanapremium.uz` (user 25) was created active with the requested password through the existing audited user service. No credential is stored in this document or Git. Dedicated role `Eco Cotton Admin` (role 21) grants exactly the pictured operational areas: fabric scans and cutting usage/passports/bundles/scanning; HR; sewing floor/flows/daily reports/scanning; packaging/packages/queue/receiving/reports; process tracking; Usluga planning/models/handover. Effective permissions: cutting.records, cutting.bundles, sewing.workspace, sewing.records, sewing.bundles, sewing.flows, sewing.daily_reports.view, packaging.records, packaging.packages, hr.employees, processes.view, usluga.view, usluga.manage, usluga.handover. Assigned and available factory is ECO only; no wildcard, global administration or other-factory grant.
- Access verification: real public login succeeded with the requested credentials; all 18 pictured-area read APIs returned HTTP 200. User administration, Milana and Besttex inboxes, and switching to Milana returned HTTP 403. Sidebar factory/permission filters were reviewed against the new effective permissions. Created one user and one dedicated role plus associated audit/login metadata; no orders, stock, reports, payroll, packages or other business data created or changed.
- Validation: existing Eco tracking contract, strict TypeScript, scoped ESLint and whitespace checks passed before commit. Immutable CI [34804953809](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34804953809) passed required backend/frontend checks, optimized build and exact-commit image publication. Candidate warmup and all four immediate internal/public health/login endpoints passed HTTP 200; valid routers/manifests, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 24/100 connections and zero invalid indexes. Disk 72% backend / 63% frontend. Existing disk alert and unrelated historical risks remain. Extra suites, browser/performance exercises and extended monitoring omitted at the user's explicit request. Database remains `0125_roll_lengths`; startup migration was a no-op and seeding disabled.
- Source manifest SHA-256 `7be5c75d615e9d4f05f941da1d6487a450496f410c005ff9c475ceeb6d040ff8`; archive 2443399 bytes / 829 verified files, SHA-256 `7973c0802b278f85668b3656895e879c4ef2ffec5f7a7f8057c293d5f88aae8b`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:c1b36f47676f5624790ec362c964e84ae162773f1b5d8b941956cec920b40474`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:930c8a4054f94f1c0d0c237a78faab36c239d4bb8c1bd42ad11f7ab9deab16fa`.
- Verified backup before account creation and cutover: `/opt/milana-erp/shared/backups/milana_erp_pre_20260914_041000.dump`, 53100281 bytes, mode 0600, 1171 restore objects, SHA-256 `fdedbd8a091b3016e61aed8bfa275696725c5701a4f13123b3689aff36b8b3ae`; restore-list SHA-256 `d89381479e615627552648cdc153e829a71242ff3fccef29cda4ece164d62310`.
- Worktree `C:/ERP/.codex-work/eco-tracking-top-20260914`, implementation branch `codex/eco-tracking-top-20260914` pushed/merged/deployed; record branch `codex/record-eco-admin-top-20260914`. Evidence: `outputs/deployment/evidence.json` and `account-access.json`. Legacy checkout preserved. Context mirrored to Obsidian.


## Dashboard text cleanup deployed (2026-09-14)

- LIVE: backend/frontend blue release `20260914_032838`, reviewed application commit `4fd6752aa7e28690aeba8290708b39b88663b44d`, merged through PR #137. Both source manifests, image identities, symlinks and slot states agree. Frontend activation completed `2026-09-14T03:40:06.481162+00:00`. Green `20260913_145159` remains running for rollback. This supersedes the prepared section below.
- At the user's request, removed the demo suffix and malformed separator from the main dashboard heading, and removed the DEMO prefix from active-production order references (MIL-0120, BST-0120, ECO-0120). All three languages use the standard translated heading. The existing sample-data subtitle, CSV provenance, sample metrics and disabled sample-order links remain. No backend, schema, business rows, operational order numbers, stock quantities, reports, payroll or permission changes. Database remains `0125_roll_lengths`; startup migration was a no-op and seeding disabled.
- Validation: reviewed two-line frontend change, strict TypeScript, scoped ESLint and whitespace checks passed. [Immutable CI 34802738492](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34802738492) passed backend/frontend checks, optimized build and exact-commit image publication. Candidate warmup and all four immediate internal/public health/login endpoints passed HTTP 200; valid routers/manifests, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 21/100 connections and zero invalid indexes. Disk 72% backend / 63% frontend. Existing disk alert and historical risks remain. Extra local suites, browser/performance exercises and extended monitoring omitted per the user's existing fast-deployment instruction.
- Source manifest SHA-256 `f616638eb242a671400ae3a6668122f6e87fb642c7cb51a1e8090b565bcd10d6`; archive 2442576 bytes / 829 verified files, SHA-256 `1e641a6bb8ce8edf1917de6877b6113fe89d4c3db2b89d690010bf7a83b69f08`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:45d8d88bb2920a03c38f88f7200289fbea0badd0911161c728f7e447127cf867`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:866b6095e3c45d62bf58cdf4c648a9ffd352d5b1477c1c04cad00b6e2be3fccf`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260914_032838.dump`: 53099577 bytes, mode 0600, 1171 restore objects, SHA-256 `bf85985a5eed51157d0df92f31ddca6e63e600d42ba7237f3b5810ea3b513b87`; restore-list SHA-256 `07dd92452f5d94a783b4527d0872b7b367feb377c692790a2765a2dae8e38512`. Backup freshness and hashes verified before staging.
- Worktree `C:/ERP/.codex-work/dashboard-text-20260914`, implementation branch `codex/dashboard-text-20260914` pushed/merged/deployed; record branch `codex/record-dashboard-text-20260914`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved. Context mirrored to Obsidian.
...
- LIVE: both services use green release `20260913_145159`, exact reviewed commit `c394dc8364f68353d9b66d16d63debb25fd2d5d7`, merged through PR #135 (`304b28e454740a694e465b0ef87c734e7b2a8f91`). Frontend activation completed `2026-09-13T15:01:46.660484+00:00`. Both source manifests, image identities, current symlinks and slot states agree. Blue `20260912_124815` remains running for rollback. This supersedes the prepared status below.
- User-authorized management dashboard demo display is enabled: 72 sample orders / 197,200 planned pieces / four overdue orders; Milana, Besttex and Eco Cotton line charts, department bars, order-stage donut, comparisons, orders and finance are populated. EN/RU/UZ headings explain that these are illustrative figures, not live production or finance. DEMO order numbers and CSV source labels persist outside the header; sample IDs cannot open real operational orders. Other ERP pages and existing permissions are unchanged. Live overview/finance endpoints remain available to other callers but this dashboard does not request their data.
- No database/schema/business row/order/stock/report/payroll/permission changes. Database remains `0125_roll_lengths`; normal startup migration was a no-op, seeding disabled. Returning this dashboard to live reporting requires switching `dashboardDemoEnabled` in the reviewed frontend source and deploying that change.
- [Immutable CI 34763901690](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34763901690) passed backend/frontend checks, optimized build and exact-commit image publication. Reviewed source and whitespace checks passed. No extra local tests, browser/performance exercises or extended monitoring ran, per explicit user request. Candidate warmup and immediate checks passed: all four internal/public health/login endpoints HTTP 200, valid manifests/routers, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 19/100 connections and zero invalid indexes. Disk 72% backend / 63% frontend; pre-existing disk alert and unrelated historical risks remain.
- The deployed demo JavaScript asset was fetched from the public domain at HTTP 200 and its SHA-256 matched the exact frontend container asset: `ec4845622477a3ab05338a3ab5744bf9b82c600121e8aef8ffc7dfc7b32ef00e`. Live browser interaction QA was omitted per request. No business test mutations.
- Source manifest SHA-256 `9b9bfe2e628056d5f590d7478383c01f9e0e52bc8617f9153e694047f83ee8e2`; archive 2441155 bytes / 829 verified files, SHA-256 `00af386434b5055acdf912308bbdf64ec9cb727cc9126ef8244aec45dab1a012`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:05f7bdca4e1a6a242be95fe7cd47105e4b417c95d0ada30aed9e6cb2310eb455`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:5dd165b6190b716de688c538927a18f5e238ea356d2fa85f32bdf445cc0849ab`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260913_145159.dump`: 53042752 bytes, mode 0600, 1171 restore objects, SHA-256 `c19fe19c115d536526d83da5f26bb359c3726833d37bd9391d9d653e4a18b321`; restore-list SHA-256 `eaa376d42bf6c223e5764f3904597d71f712bb5a11a648b207e661e0174d059d`. Backup freshness, hashes and permissions were checked before staging.
- Worktree `C:/ERP/.codex-work/dashboard-demo-20260913`; implementation branch `codex/dashboard-demo-20260913` pushed/merged/deployed; record branch `codex/record-dashboard-demo-20260913`. Evidence retained locally in `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- LIVE: backend/frontend release `20260912_124815` in blue, exact application commit `fae0a5463d3c46ce197601070fbc5337eeb78ebf`, merged through PR #133. Both manifests, image identities, symlinks and slot states agree. Frontend activation completed `2026-09-12T12:59:14.804622+00:00`. Green `20260912_121159` remains running for rollback. This supersedes the prepared section below.
- Each received fabric roll now has its own optional metres field alongside kg. Adding/removing rolls keeps their lengths paired; saved receipt rows identify lengths by roll number. Total-weight mode also permits optional lengths per roll. EN/RU/UZ labels are live. Historical total metres remain explicitly labelled as previous totals, without conversion into per-roll values.
- Migration `0125_roll_lengths` adds the optional per-roll JSON storage array with an empty default; no backfill or changes to existing stock quantities, orders, reports, payroll or permission grants. Existing clients can omit the array. API validates positive lengths and roll count, prevents count edits from invalidating saved lengths, and clears obsolete roll details only on an explicitly requested archived-stock restore. Prior application is compatible with the additive column; seeding disabled.
- Focused checks passed: two backend roll persistence/list/count-protection tests, exact payload/optional blank/total-mode and locale contracts, strict TypeScript, scoped ESLint/Ruff, single migration head and whitespace checks. [Immutable CI 34694673169](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34694673169) passed backend/frontend validation and published exact-commit images. Candidate warmup and all four immediate internal/public health/login endpoints passed HTTP 200; routers/manifests valid, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 20/100 connections and zero invalid indexes. Disk 72% backend / 63% frontend; existing disk alert and unrelated historical risks remain. Extra browser/performance exercises and extended monitoring omitted per user instruction.
- Source manifest SHA-256 `bab4789379ab4563836f999622f45a91f5d5b1de77d8520d762c5e853d9b5aa0`; archive 2437741 bytes / 828 verified files, SHA-256 `5d2a8954e6bcd09b499172cd8d191a4b7a6560ff9a543745dfd93d81a5da4dc7`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:5e78257cd438cab124905a6da6e49f48bdc56d4007a0f933decb07e198bfe494`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:399fea5a2971ced1019e74b8bb9a2635dd766cac94684c2d8458320b985345ff`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260912_124815.dump`: 52997304 bytes, mode 0600, 1171 restore objects, SHA-256 `aee718dae5a23b99c4218d698293aa838308e55dc284a0f71d8e1c05ea46cf9a`; restore-list SHA-256 `d4b6c4b7271500aa10c5235848bf4c495f8432ca123035345332ce85efddf976`. Application rollback does not downgrade the database.
- Worktree `C:/ERP/.codex-work/roll-length-20260912`, implementation branch `codex/roll-length-20260912` pushed/merged/deployed; record branch `codex/record-roll-length-20260912`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- Fabric Receiving defaults to exact kg entry for each roll, with Add/Remove roll, automatic roll count and one automatic total. Entered unequal weights save unchanged through the existing validated API and remain available to roll stickers. Total weight only retains its prior equal-allocation option. Accessories and purchase-order receiving retain their existing forms; optional material length is preserved. Runtime EN/RU/UZ locale files now include the preceding task's nine missing material-length/passport/sewing labels as well as the new roll-entry labels.
- No backend, schema, migration, business row, existing batch weight, stock quantity, order, report, payroll or permission changed. Database remains `0124_material_length`; normal candidate startup migration was a no-op and seeding was disabled.
- Validation passed the two existing backend roll persistence/total checks, exact unequal-weight payload and total-only rounding checks, receiving/legacy contracts, strict TypeScript, scoped ESLint and whitespace checks. [Immutable CI 34693021719](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34693021719) passed all 918 backend tests, frontend validation, production contracts/build and exact-commit image publication. Candidate warmup and four immediate internal/public health/login checks passed at HTTP 200; routers/manifests valid, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 20/100 connections and zero invalid indexes. Disk 72% backend / 63% frontend; pre-existing disk alert and historical risks remain. Extra browser/performance exercises and extended monitoring were omitted per the user's fast-deployment instruction.
- Source manifest SHA-256 `5ee040e99b0a4d9082970e3f2b2827e24969c440e867062deaa1f748d590590c`; archive 2434991 bytes / 827 verified files, SHA-256 `bf1c43d35388246bd11fc48239f0785195b80de4f52cd00a43f5bce4f0ea2195`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:fecdc60e802ad09fbe223e2d5faa95d59400dcbda8761b84d5e35fb1b00eff55`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:9e1f6838b398eb6a4c0c6600f69f5c18c2f2bc0b6289f8cff5385beda9ea9555`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260912_121159.dump`: 52995995 bytes, mode 0600, 1171 restore objects, SHA-256 `7ef6c84dd5f90ae51c3b11eeda726d2ca152511df8e565bc206239191b40d994`; restore-list SHA-256 `81027907c80e5645dbb8b3970ddcf5ae29ef2dec6780dd28706de00f6f84daf8`. Application rollback remains available in the blue slot.
- Worktree `C:/ERP/.codex-work/fabric-roll-entry-20260912`, implementation branch `codex/fabric-roll-entry-20260912` pushed/merged/deployed; record branch `codex/record-fabric-roll-entry-20260912`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- Material Receiving accepts optional total length in metres. Add batch creates a separate passport for the same order with its own number and measurements; Cutting selects and retains the intended passport for printed sheets. Daily Sewing Report offers confirmed audited factory-scoped deletion and cumulative linked-order/batch/assignment limits, with independent top/bottom limits. Manual orderless reports remain uncapped because no actual order identity exists. Report changes do not mutate production progress, stock or payroll; successful sections are cleared before retries.
- Database is now `0124_material_length`. Additive migration added nullable `stock_batches.length_m`, nullable `cutting_records.cutting_passport_id`, its index and foreign key. No backfill, receipt length, passport, report quantity, order, stock, payroll, permission or other business row changed during deployment. Migration lock timeout was 5 seconds and statement timeout 60 seconds. Both candidate/rollback images remain compatible; seeding was disabled.
- Validation passed 13 focused backend checks (including actual-cut count, two-part reporting, update overflow, deletion authorization/audit, receipt length and two-passport printed-sheet identity), backend Ruff/compilation/single migration head and frontend strict types/changed-file lint. [Immutable CI 34691944613](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34691944613) passed all 918 backend tests, frontend validation, contracts, optimized build and exact-commit image publication. Four immediate internal/public health/login checks returned HTTP 200; valid manifests/routers, two workers per backend slot, zero restarts/OOM/error markers, database 24/100 connections and zero invalid indexes. Disk 72% backend / 63% frontend. Extra browser/performance exercises and extended monitoring were omitted at the user's explicit request; these are not claimed passed. Existing disk alert and historical risks remain.
- Source manifest SHA-256 `6464866f90645082507a1658f28716d0f70f5a587ba079040892fed6c3d0a921`; archive 2430779 bytes / 827 verified files; archive SHA-256 `a688bf285788402ac693f18d62968cbcaa556124d31461f60aeec667fc5ca19b`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:faeebf59e0bb7f1c4e97ee47b456c8f13482c7ec0bf18f68f2af1271e6b672e8`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:5ad71d719534a1634f836cb104de703800675df6ddabecb44b2353cd0a6c1c27`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260912_114701.dump`: 52995007 bytes, mode 0600, 1169 restore objects, SHA-256 `0518ce4fafb5fcb18f691f29f22962e6c6d2c00e91d127e5d149bc175a86a455`; restore-list SHA-256 `e6ad3c4dd873f7432e30e009642263db452cbb360ade435993be7d0cebe876be`. Previous green application release is retained for rollback; no data values were rewritten.
- Worktree `C:/ERP/.codex-work/material-passport-sewing-20260912`, implementation branch `codex/material-passport-sewing-20260912` pushed/merged/deployed; record branch `codex/record-material-passport-sewing-20260912`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- The dashboard now reads active ProductionOrder workload instead of the empty SalesOrder ledger, with All / Milana / Besttex / Eco Cotton filters, line/bar/donut charts, date selection, source-specific CSV, searchable orders and EN/RU/UZ responsive day/night views. Activity defaults to seven-day Sewing daily reports; Stage records remains separate. Reported quantities are not combined with validated stage completion, stock or payroll. No schema, business data, permission grants, order quantities or report rows were changed by deployment; startup migration was a no-op at `0123_fabric_scan_register`, seeding disabled.
- [Immutable CI 34683873181](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34683873181) passed backend/frontend validation and published exact-commit images. Earlier local validation is recorded below. At the user's explicit request, no extra local test suite, deployed-browser/performance exercise or extended monitoring was run. Immediate candidate/live health and runtime checks passed: all four internal/public health/login endpoints HTTP 200, valid manifests/routers, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 25/100 connections and zero invalid indexes. Disk 72% backend / 62% frontend; existing disk alert and unrelated historical risks remain.
- A single authenticated, read-only GET through the public dashboard endpoint returned HTTP 200, all three factories, 56 active production orders and seven-day sewing-report totals Milana 28,966 / Besttex 1,200 / Eco Cotton zero. Zero business test mutations. Live UI browser validation was omitted per request; prior actual-page local snapshot checks passed.
- Source manifest SHA-256 `8a1cafe394aec228cdb2b89dcf36fcf7b2c9601ab66ac3d176af1045daad5d03`; archive 2425633 bytes / 826 Git-verified files, archive SHA-256 `8411a339c895962b5b51809a3b754ff8b6319738cb28a6cf221772563237a939`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:2db2ed3ae3ab384e73b8cdf4d988c20d4bfb2ba7c0ebb2fb0833003bfe582d49`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:ef61db23eb16e703d3ab59017cafecca3568bc3053a91659a2a058fb3799c68e`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260912_084025.dump`: 52981487 bytes, mode 0600, 1169 restore objects, SHA-256 `09f25ac934b71be10294cfed3bb4f792a925abacc7b38eed607b9f299d3834a4`; restore-list SHA-256 `13c4a62099b02f8f3c5533086c7ff5389d58267187324434661a098d0cb798a0`. Backup hashes, permissions and freshness were checked before staging. Previous blue release is the application rollback; there is no data/schema change to reverse.
- Worktree `C:/ERP/.codex-work/dashboard-charts-20260912`; implementation branch `codex/dashboard-charts-20260912` pushed/merged/deployed; record branch `codex/record-dashboard-charts-20260912`. Evidence retained locally under `outputs/deployment/evidence.json`. Legacy checkout preserved.

## Dashboard activity chart and sewing reports prepared for review (2026-09-12)

- READY LOCALLY, NOT DEPLOYED: the sparse chart now defaults to a seven-day Sewing reports view, with a separate Stage records selector. Real factory series have period totals, point markers, a light area fill and selectable-day values; zero-output series do not stack on the baseline. Empty periods use a compact message with Show last 30 days. The 30/90-day periods remain available. Source/factory/date changes reset chart selection; the chart-data table and CSV follow the chosen source with distinct source filenames/columns. All important text supports EN/RU/UZ and existing day/night themes.
- Investigation found the missing visible activity in the independent SewingDailyReport ledger: the live thirty-day snapshot contained 124,244 reported pieces for Milana, 10,736 for Besttex and 19,226 for Eco Cotton. Seven-day totals were 28,966 / 1,200 / zero. These are reported activity, not validated stage completion or unique finished garments. They remain separate from stage output, order progress, packaging, payroll and stock; no quantities are combined or fabricated. Stage records remain accessible and their existing KPI/comparison totals are unchanged.
- The management overview adds one SQL aggregate grouped by report_date and SewingFlow.factory_code, respecting the selected factory and inclusive business-date window, including backdated reports and historical inactive lines. The endpoint now uses eleven bounded queries. No new permission, endpoint, schema, business mutation, report edit or production-file write was introduced.
- Validation: 19 focused dashboard/sewing-report/factory-permission tests passed; scoped Ruff, full frontend lint, strict TypeScript, every inherited frontend build contract and the optimized 87-route build passed. The new regression verifies report-date grouping, factory isolation, historical lines, zero-filled dates, stage/report separation and unchanged database fingerprints. Actual-page local browser QA passed both sources, exact source-specific CSV totals/headers, three factories, period widening, desktop/mobile, EN/RU/UZ/day/night, search/series controls and empty/error/restricted states with zero unexpected console errors/page exceptions. Screenshots use an actual read-only production snapshot; no synthetic values are used in the delivered previews.
- Active release remains `20260912_065400` / blue, rollback `20260912_080500` / green. Both manifests and slot states were reverified before editing against origin/main and production-base.json. No deployment or business-data change occurred. Worktree `C:/ERP/.codex-work/dashboard-charts-20260912`, branch `codex/dashboard-charts-20260912`; focused chart previews are local-only `outputs/dashboard-preview/activity-chart.png` and `activity-chart-30days.png`.

## Three-factory management dashboard prepared for review (2026-09-12)

- READY LOCALLY, NOT DEPLOYED: the management dashboard now has All factories / Milana / Besttex / Eco Cotton selectors. Each factory updates the workload/overdue/packed metrics, stage donut, daily output line chart, department bars, active-order table and exported CSV. All factories adds a three-row comparison table. Factory names also appear on order rows. Finance is explicitly labelled all factories/all time.
- Routing follows the existing authoritative `production_order_factory_condition`: non-cancelled routed bundles take priority, with sewing department/line fallback for legacy and Eco service orders. Split orders appear once in each involved factory and once in the All total; displayed plans remain the full order quantity. Output is grouped by the actual WorkOrder department using the shared factory map, so Milana cutting for Besttex-routed orders belongs to Milana output. Unrouted orders/unassigned output remain in All with visible notices. This is read-only management reporting; existing management.view authorization and operational factory/session controls are unchanged.
- Read-only live snapshot at approximately 08:26 UTC: Milana 19 orders / 10,860 planned pieces; Besttex 31 / 29,800; Eco Cotton six / 16,460. All has 56 unique active orders / 57,120 planned pieces; no unrouted active orders. All six overdue orders belong to Eco Cotton. Thirty-day approved cutting output is 12,334 for Milana and 810 for Eco Cotton; other operation-ledger totals are zero. Sewing daily reports/payroll remain separately labelled ledgers and are not synthesized into stage output.
- Validation: 12 focused dashboard/factory-permission/Process Tracking tests passed, including split routing, stale legacy fallback, cancelled bundle exclusion, Eco service fallback, physical-department output, query bounds and read-only table fingerprints. Scoped Ruff, full frontend lint and strict TypeScript passed. Actual-page snapshot browser QA passed all three factory filters, comparison-row navigation, factory-specific CSV, desktop/mobile, EN/RU/UZ/day/night, period/series/search controls, empty/error and restricted UI states with zero unexpected console errors/page exceptions. Every inherited frontend build contract and the optimized 87-route production build passed. The new aggregate uses ten bounded SQL queries regardless of order count.
- Both live manifests/symlinks/blue slot states were reverified against origin/main before editing: active `20260912_065400`, manifest `5a0cb78718d9476609dde616aed6a11bd4b4483197a7ad3f78b19f72cf59b485`, rollback green `20260912_080500`. No deployment, migration, production-file write or business-data write occurred. Work continues in `C:/ERP/.codex-work/dashboard-charts-20260912`, branch `codex/dashboard-charts-20260912`. Private snapshots/screenshots remain local-only under `outputs/dashboard-preview/`.

## Management dashboard charts prepared for review (2026-09-12)

- READY LOCALLY, NOT DEPLOYED: management users now have a production-order dashboard with daily output lines, department bars, an active-order stage donut, current workload/overdue/packed metrics, 7/30/90-day selection, daily CSV export, searchable/type-filtered production orders, and EN/RU/UZ/day/night/mobile support. Non-management users retain the existing dashboard. Production/service order links and finance/new-order visibility respect existing permissions.
- Root cause verified read-only: production has zero SalesOrder rows but 56 active ProductionOrder rows (33 planning, 22 sewing, one cutting), so the old sales-order-only active table and management counts showed zero. The old department panel said today while reading the selected period, and output added operations across stages. The new management endpoint reads production orders directly and labels current workload separately from selected-period output. No business workflow, permission grant, schema or business data changed.
- Snapshot at 2026-09-12 08:20 UTC: 57,120 planned pieces, six overdue orders, 13,144 approved cutting pieces in the last 30 days (12,334 in seven days), and zero PrintingRecord/SewingRecord/PackagingRecord output for those periods. Stage charts deliberately use their operation ledgers; sewing daily reports, payroll and imported stock remain separate and are explicitly labelled. No fabricated output is added. Active-order list is bounded to 100 earliest-deadline rows, with full aggregate counts and a visible limit notice.
- New read-only `GET /api/dashboard/overview` requires management.view (or wildcard), validates ranges up to 366 days, groups stage records in SQL by Asia/Tashkent business day with exclusive upper bounds, rejects unapproved cutting output, and performs eight queries independent of order count. Missing dates become explicit zeros; API failures display errors instead of empty operational totals.
- Validation: 87 focused dashboard/factory-permission/production-flow tests passed; scoped Ruff, full frontend lint, strict TypeScript, every inherited frontend build contract and the optimized 87-route build passed. Actual-page browser checks using intercepted local APIs and a read-only production snapshot passed desktop/mobile, EN/RU/UZ, day/night, period selection, series controls, search, CSV, empty/error states and restricted UI permissions with zero unexpected console errors or page exceptions. Production endpoint deployment and post-deploy QA remain pending. A separate local optimized-server launch was blocked by tool policy; visual QA used the existing local development server, while the optimized build itself passed.
- Baseline verified before edits: both manifests and active blue slots match release `20260912_065400`, manifest `5a0cb78718d9476609dde616aed6a11bd4b4483197a7ad3f78b19f72cf59b485`, application commit `c45a2c516e5226979ba48d1521431fba34b3e103`; green rollback remains `20260912_080500`. Origin main application sources match that production commit. Legacy checkout preserved. No deployment, migration, production-file write, or business-data write occurred.
- Worktree: `C:/ERP/.codex-work/dashboard-charts-20260912`; branch `codex/dashboard-charts-20260912`. Screenshots and private aggregate snapshots are local-only under `outputs/dashboard-preview/`; repeatable browser tests are committed under `frontend/scripts/test-dashboard-overview.browser.mjs` and accept a local-only snapshot override.

## XJ3062 family merge, Besttex lines and sewing return deployed (2026-09-12)

- LIVE: backend/frontend release `20260912_065400` in blue, exact application commit `c45a2c516e5226979ba48d1521431fba34b3e103`, merged through PR #125 (`7586f200`). Both manifests, image identities, symlinks and slot states agree. Green `20260912_080500` remains running for rollback. Activation completed at `2026-09-12T07:04:47.782219+00:00`.
- User-authorized data reconciliation merged the duplicate Latin-X and Cyrillic-Х XJ3062 catalog cards by changing only `details_json.general.model_no` (and an existing `modelNo` alias if present) on 17 reviewed visible model rows to the established Cyrillic-Х family identity. The catalog now has one card with 155 distinct variants / 156 model rows. Existing IDs, codes, variant numbers, names, images, BOMs, quantities, order references and migration-only warehouse donor rows were preserved. No model was deleted.
- Besttex has exactly eight active lines, retaining existing IDs 96–103 and codes BST-BAND-01 through BST-BAND-08: 1 Oydinoy Mamadaliyeva; 2 Miyassar Yunusova; 3 Teshaboyeva Nargiza; 4 Shaxnoza Maxsimova; 5 Minura Toshboyeva; 6 Muqadamxon Muhammadjon qizi; 7 Kamola Xoliqova; 8 Nargiza Xoliqova. Lines 9–16 (IDs 104–111) were deactivated after confirming no active assignments or direct active work orders. Their historical reports and other references remain; existing assignment on line 3 was preserved. No Milana/Eco line or historical report was rewritten.
- Audit entry `19224` records the atomic model/line reconciliation and old values. The committed `backend/scripts/reconcile_xj3062_besttex_20260912.py` is read-only by default, validates exact reviewed identities and variant uniqueness, locks changed rows, and is idempotent after application. It changed only the 17 model metadata rows, eight line names, eight line active flags, normal affected-row timestamps and the audit entry. Full PostgreSQL backup was verified before application. Application rollback does not reverse these data changes.
- Sewing Flows now has localized Return work / Вернуть работу / Ishni qaytarish on assigned work rows for the existing Planning/Flows/Sewing-record permissions. After confirmation, a new factory-scoped endpoint cancels the unused assignment without deleting it, releases its quantity to the existing available-work queue, and updates the primary line while preserving sibling assignments. Returns reject a stale/moved line, closed work, completed quantity, actual start/end, sewing output or related daily reports. Duplicate retries are idempotent. New assignment, move, return, sewing-record and report paths use work-order locking where needed; assigning/moving also locks the destination line so retirement cannot race a new assignment. Cancelled assignments reject later explicit sewing output/report submissions.
- Standard [immutable CI 34679347038](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34679347038) passed backend/frontend validation and image publication, including four focused return regressions covering retained history/reassignment, output/report guards, cross-factory access and stale lines. Local strict TypeScript, scoped Ruff and whitespace checks passed. The superseded initial workflow was cancelled before publication. No extra local suite, browser/performance exercise or extended monitoring was run, as explicitly requested. Live verification was read-only: one 155-variant family, eight exact Besttex names from the public authenticated API, and the new return route registered. No real assignment was returned as a deployment test.
- Source manifest SHA-256 `5a0cb78718d9476609dde616aed6a11bd4b4483197a7ad3f78b19f72cf59b485`; archive 2401657 bytes / 817 Git-verified files, SHA-256 `88d6781ae73e7053eb1957f4955fc0350c025f4b9be03d4fde2a9c7098809dd2`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:802ccde6c75c2f7f03c5f39acf97bcc32ec46f29560ec77e1f28aa0b90c396ca`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:2fc5452d6d825e8c3f3e79130eb32336f98c81f08e6b3b22cb30bd8b33cee512`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260912_065400.dump`: 52977108 bytes, mode 0600, 1169 restore objects, SHA-256 `8e6ff5288c008da3eb20a5088036cfdfaa7153ab7f0a735e990b2d75c201c3eb`; restore-list SHA-256 `83bcc9f890cbbd950fdcbe204107ba24f6af164e0c8b4e6b7f8b954cee981d62`. Database remains `0123_fabric_scan_register`; no schema migration, startup migration was a no-op, seeding disabled.
- Immediate deployment checks passed: all four internal/public health/login endpoints HTTP 200, valid manifests/routers, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 25/100 connections and zero invalid indexes. Disk 72% backend / 62% frontend; existing disk alert and unrelated historical risks remain. Worktree `C:/ERP/.codex-work/besttex-return-model-merge-20260912`; implementation branch `codex/besttex-return-model-merge-20260912` pushed/merged/deployed; record branch `codex/record-besttex-return-model-merge-20260912`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved.

## Variant image recovery deployed (2026-09-12)

- LIVE: backend/frontend release `20260912_080500` in green, exact application commit `71c5cc1ce1f3a0d3813b843f782d39366d94f064`, merged through PR #123 (`57df8cf13c9c5b4de1a4c3307ef7da585720564f`). Both manifests, image identities, symlinks and slot states agree. Blue `20260911_093031` remains live for rollback. Activation completed at `2026-09-12T06:42:57.692121+00:00`.
- Checked all 18,680 model-image records and fully decoded their 8,334 distinct source files. 8,252 sources decoded normally; 65 legacy JPEGs had truncated endings and 17 older model JPEGs had broken data streams. The reported PG10515 variants V-5845/V-5848/V-5850/V-5851 failed for truncated JPEG endings, while V-5847/V-5849 decoded normally. All source files existed; no image records were deleted or reassigned.
- Existing truncated-JPEG thumbnail sources now recover in an isolated decoder subprocess, keeping the serving process and new-upload validation strict. Storage errors use `private, no-store`, and thumbnail URLs use `quality=hq3` to bypass previously week-cached failures. The committed recovery utility audits the catalog and can rebuild truncated-JPEG 160/320px thumbnails without changing originals.
- A standalone cache-repair process also recovered the 17 broken-stream legacy model sources. It regenerated 492 valid WebP thumbnails for all 82 affected sources at every currently used size (128/160/240/320/480/640). Original source SHA-256 values were identical before/after, and no business row, schema, permission or original file changed. All 82 repaired 160px thumbnails were then fetched through the live public domain with authenticated requests: HTTP 200, valid WebP, correct dimensions. The screenshot's V-5845 recovered thumbnail was visually inspected.
- Original legacy files are intentionally retained unchanged. The 17 broken-stream model files still need the isolated tolerant cache-repair procedure if their derived caches are lost; the request-time fallback covers truncated endings. Recovery inputs/report are retained locally under `outputs/deployment/` and on the backend under `/opt/milana-erp/shared/deploy-evidence/20260912_080500/thumbnail-recovery.jsonl`. Other pre-existing project risks remain unchanged.
- Source manifest SHA-256 `067e803a49ef6e3b1d67d566776d0d3f4cce061c9e4d1bdcf829440c0e2014e6`; archive 2396272 bytes / 815 Git-verified files, SHA-256 `042500afe993dd7b5d3a6fbafae0aaac4102a82d87d442bc32a0037be725820c`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:2c70652ca37b3fc73d98149906ffad835313f2f20718b22c79a11cbaafb731a0`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:608032d287603ccbfbe4436d6bd0a1cd1e74f6a13415abe8563d76fcf87932a1`. [Immutable CI 34678385875](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34678385875) passed standard backend/frontend validation and published images. Scoped Ruff and whitespace checks passed. No extra local test suite, browser/performance exercise or extended monitoring was run, per the user's explicit request.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260912_080500.dump`: 52963995 bytes, mode 0600, 1169 restore objects, SHA-256 `ab3265cf869ac159b8d429eddd7d922de90010fe748c2c12efde11f39fa595bc`; restore-list SHA-256 `f0059dbd3a3f0c77c66cb2a1ca44f2b1e910350eca4fcd0f12639f0aaddc3dbd`. Database remains `0123_fabric_scan_register`; no migration/schema change, normal startup migration was a no-op and seeding was disabled.
- Immediate deployment checks passed: four internal/public health/login endpoints HTTP 200, valid manifests/routers, two workers per backend slot, zero restarts/OOM/error markers, PostgreSQL 32/100 connections and zero invalid indexes. Disk 72% backend / 62% frontend; existing disk alert retained. Worktree `C:/ERP/.codex-work/fix-variant-images-20260912`; implementation branch `codex/fix-variant-images-20260912` pushed/merged/deployed; completion branch `codex/record-variant-images-20260912`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved.

## Continuous fabric scan register deployed (2026-09-11)

- LIVE: backend/frontend release `20260911_093031` in blue, exact application commit `e44b11239c0b75fa40b1178dfe4c678f7c8e1847`, merged through PR #121 as `ea5f88e46d521d9a31158b8234eb2a5afe286969`. Both source manifests, image identities, symlinks and slot states agree. Frontend activation completed at `2026-09-11T09:42:05.241151+00:00`. Green `20260911_055700` remains running for rollback.
- New Fabric control -> Fabric scan register (`/fabric-scans`) supports existing roll QR labels, continuous Receive/Return modes, automatic saving, held-label and database duplicate protection, daily reports and CSV export. Scans are recorded once per roll/action/Tashkent day. Migration `0123_fabric_scan_register` created only the isolated `fabric_scans` table, verified empty immediately after migration. No inventory quantities, stock movements, reservations, cutting consumption, existing business rows, users or role grants were modified by this deployment. Seeding was disabled. Workers' subsequent scans write only the reporting ledger.
- Source manifest SHA-256 `65a13916392974dbee91d789e8c6a26b32f160adeb87a487892bb87b5e0d7d8a`; archive 2394140 bytes / 814 files verified against Git, archive SHA-256 `8a71ecc950598f9a5db3ea799a1f51d64ca71cb16065a782c68b3c8790d5c3da`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:e6bcb915bdca48e2325dc5ce54bcbaa1e1f012914eaab5c0027bd0c0a5ab3591`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:fec6b95fa0445633419bfc40c3c6987fdb6faabb565839fe63a2e0f012101940`. [Immutable CI 34584595764](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34584595764) passed standard backend/frontend validation and image publication. Prior local checks and fixture browser evidence are recorded below; no extra local tests were run during deployment.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260911_093031.dump`: 52818314 bytes, mode 0600, 1160 restore objects, SHA-256 `bef9feafc1b42034393fd6679386e042a9aa87c31049f2146580eebbbfa65ca7`; restore-list 83166 bytes / SHA-256 `bd27051bfa13be5795c785756da39d1fd9075b13dfc7871e372db31e8bcd9320`. Hashes, permissions and freshness were rechecked before migration and staging. Database is now `0123_fabric_scan_register`; candidate startup migration was a no-op after the separate backed-up migration.
- Immediate candidate/live checks passed: all four internal/public health/login endpoints HTTP 200; valid manifests and routers; two workers per backend slot; zero restarts, OOM or error-log markers; PostgreSQL 26/100 connections and zero invalid indexes. Disk 72% backend / 62% frontend. Existing disk alert and historical risks remain follow-ups. At the user's explicit request, no extra tests, browser/performance exercise or extended monitoring ran. Physical-camera hardware and authenticated deployed-page UI checks remain unperformed; prior fixture browser checks passed. The old rollback container remains live; its normal restart path must be reviewed for the newer Alembic revision before restarting an old image.
- Implementation worktree `C:/ERP/.codex-work/fabric-cutting-register-20260911`, branch `codex/fabric-cutting-register-20260911`, pushed/merged/deployed. Record worktree `C:/ERP/.codex-work/record-fabric-scans-20260911`, branch `codex/record-fabric-scans-20260911`. Evidence: implementation worktree `outputs/deployment/evidence.json`. Legacy checkout preserved.

## Isolated fabric scan register prepared (2026-09-11)

- Continuous-scan follow-up (2026-09-11): workers select Receive or Return once and scan consecutive rolls without reopening the scanner or pressing Save. The camera remains active and awaits each save; a held label is submitted only once, and changing the action alone does not submit the label still in view. Hardware input remains focused/available during saves; Enter saves immediately and a valid code without Enter saves after 250 ms of settled input. Saves run sequentially with the original action; failed scans remain separately visible with a Retry action. Existing server-side per-roll/per-action/per-day deduplication and inventory isolation are unchanged. Extended browser regression passes continuous receive/return, held-label suppression, repeated rolls, slow-save queue, retry, auto-save without Enter, camera fallback, daily report and mobile/locales. Normal/strict TypeScript, scoped ESLint and optimized production build passed. No backend, schema, permission or production-data changes in this follow-up; physical-camera hardware QA remains outstanding.
- READY LOCALLY, NOT DEPLOYED: new Fabric control sidebar section and `/fabric-scans` page let workers scan existing individual fabric-roll QR URLs or enter the printed `B<batch>-R<roll>` code. Workers choose Received in Cutting or Returned from Cutting; successful scans immediately reset/focus the input. A failed save keeps the code available for retry. Camera scanning is optional on compatible browsers; unsupported/denied cameras show the scanner/manual-code fallback.
- Isolation is explicit: additive Alembic revision `0123_fabric_scan_register` creates only `fabric_scans`. It has no foreign keys/cascades into operational tables. The API writes only this reporting table and never calls stock movement, inventory quantity, reservation, cutting consumption or production mutation services. Existing fabric/batch data is read to validate roll identity and snapshot business labels. No production database or business data was changed.
- Reports count unique rolls once per direction per Tashkent calendar day, with database uniqueness protecting concurrent duplicate scans. A return can be recorded without an earlier receipt; depleted/archived fabric batches remain eligible when their roll identity is still registered. Reports intentionally do not claim fabric weight, consumption, or physical remaining stock. Records snapshot worker name, fabric, batch, color, roll, direction and timestamp. Date selection, full-day per-batch totals, paginated history and complete daily-summary CSV export are supported in EN/RU/UZ.
- Scanning uses existing Cutting records/bundles or material-storage items/receive permissions; Planning production and Management view can read reports. API, page and navigation enforce matching access. The active session determines CUT versus ECT; Besttex has no Cutting register. No existing user or role grants changed. Camera permission is limited to the register document, and the sidebar uses full document navigation for that policy.
- Validation: full backend suite 904 passed; final focused register suite 24 passed (including two additional concurrency/migration cases). Tests compare every pre-existing table before/after receipt, return and duplicate scans; only the register changes. Backend Ruff/compilation and single Alembic head passed. Frontend full lint, normal/strict TypeScript, all build contracts and optimized 87-page build passed. Fixture-only actual-page browser QA passed receipt/return, duplicate, failed-save recovery, daily filter, CSV, read-only access, camera fallback and EN/RU/UZ/mobile layouts with zero browser exceptions. Physical camera hardware and authenticated live-server UI QA have not been performed. Global i18n checking retains the same 22 unrelated missing-key reports reproduced on untouched origin/main; new register translations are complete.
- Baseline verified before edits: both production source manifests and slots match green release `20260911_055700`, SHA-256 `6ccc6ba234991402074f27cf0308e641ddbe7f9143e802ac0388df6028923c74`; rollback blue `20260911_053727`, recorded schema `0122_cutting_material_details`. Worktree `C:/ERP/.codex-work/fabric-cutting-register-20260911`, branch `codex/fabric-cutting-register-20260911`, based on verified main `c405441b`. Deployment requires authorization and the normal immutable-image/migration procedure. Screenshots and CSV evidence are under `outputs/fabric-scans-qa/`. Legacy checkout preserved.

## Daily employee salary report deployed (2026-09-11)

- LIVE: backend/frontend release `20260911_055700` in green, exact application commit `e350259205c925b43d633bff2b3852cdb778b106`, pushed and merged through PR #119. Both source manifests, image identities, current symlinks and slot states agree. Frontend activation completed at `2026-09-11T06:06:19.599204+00:00`. Blue `20260911_053727` remains running for rollback.
- Salary summary now shows one row per employee/currency with daily scanned-work earnings, zeroes for dates without work, per-day currency totals and full-period salary totals. Scan dates use Asia/Tashkent. Selected dates and all existing report filters apply before aggregation. Excel includes numeric daily columns and frozen employee columns; landscape browser print splits periods over 31 days into additional column sections. Existing scan details and bonuses/deductions scope remain. No schema, permissions, business rows or payroll amounts changed.
- Source manifest SHA-256 `6ccc6ba234991402074f27cf0308e641ddbe7f9143e802ac0388df6028923c74`; archive 2373281 bytes / 806 files verified against Git, SHA-256 `5c749bf922f71d8b1b61296ae67f759e69996edbfeacc4101650f9d6c8081381`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:125e22d38d55ec891815cf0d321ed7ae6b428ae77fa4e81e2996423781cb191f`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:1f48b95ac6639f29335a6a327766bf6d71344b1edc8e29e7d871109ff7a2e406`. [Immutable CI 34567924387](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34567924387) passed standard backend/frontend validation and image publication. Local strict TypeScript, scoped ESLint, Ruff, Python compilation and whitespace checks passed.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260911_055700.dump`: 52437040 bytes, mode 0600, 1160 restore objects, SHA-256 `8d03c589b633fb3d08d988fba7b7493a8857d3f6666035a9d04588dbca51dbe6`; restore-list 83166 bytes / SHA-256 `0747e2c8ae7047763d3b0cb6140d3409902b748b167f21381c70f4a958d5e678`. Backup hashes, permissions and freshness were rechecked before staging. Database remains `0122_cutting_material_details`; startup migration was a no-op and seed disabled.
- Candidate and immediate live checks passed: all four internal/public health/login endpoints HTTP 200; valid manifests and routers; two workers per backend slot; zero restarts, OOM or error-log markers; PostgreSQL 27/100 connections and zero invalid indexes. Disk 72% backend / 62% frontend. Existing disk alert and historical risks remain follow-ups. Continuing the user's fast-deployment preference, no extra test suite, browser/performance exercise or extended monitoring was run or claimed passed.
- Worktree `C:/ERP/.codex-work/salary-daily-report-20260911`; implementation branch `codex/salary-daily-report-20260911` pushed, merged and deployed. Deployment record branch `codex/record-salary-daily-report-20260911`. Evidence `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- Sewing Production Report now provides Scan details / Salary summary. Choose the reporting period and employee/production filters, click Build report, and see the complete scanned-work earnings grouped by employee and currency. Summary is aggregated in SQL before pagination; default Active records excludes voided work. Excel and Print follow the selected view. EN/RU/UZ text explicitly distinguishes scanned-work earnings from bonuses/deductions, which remain in Payroll Summary. No schema, permissions, business rows or payroll amounts changed.
- Source manifest SHA-256 `4c5ca14374497e17ad9becc304b2b57b31af483ba78caa8c86d70a14e3da2762`; archive 2369817 bytes / 806 files verified against Git, SHA-256 `90f0aa86a10efbdc22ea53376d3d5f34293b8e1caaf8e382ec11a9358ad6dda6`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:ed8336bef5c1e2a9c321cf852f55f129e48c5ae4bedbd5746a736b87f0e25b0b`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:314c446aa8ef09cd0dd53938beb26e2ad4c5d0cea1eeff498ef0b82d7a65b44c`. [Immutable CI 34566676630](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34566676630) passed standard backend/frontend validation and image publication. Local strict TypeScript, scoped ESLint, Ruff, Python compilation and whitespace checks passed.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260911_053727.dump`: 52432115 bytes, mode 0600, 1160 restore objects, SHA-256 `b0325dae495cef769f3dc4e3a9f69027cc022c5be594f835ce8b11392735cf03`; restore-list 83166 bytes / SHA-256 `10121600d458c3b9186c1cf92b7cdd72aa3ac25e988e4a5d3d29307642abebdf`. Backup freshness, hashes and permissions were rechecked before staging. Database remains `0122_cutting_material_details`; startup migration was a no-op, seeding disabled.
- Candidate and immediate live checks passed: all four internal/public health/login endpoints HTTP 200; valid manifests and routers; two workers per backend slot; zero restarts, OOM or error-log markers; PostgreSQL 30/100 connections and zero invalid indexes. Disk 72% backend / 62% frontend. Existing disk alert and historical risks remain follow-ups. Extra tests/review, browser/performance exercises and extended monitoring were omitted at the user's explicit request and are not claimed passed.
- Worktree `C:/ERP/.codex-work/salary-period-report-20260911`; implementation branch `codex/salary-period-report-20260911` pushed, merged and deployed. Deployment record branch `codex/record-salary-period-report-20260911`. Evidence `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- LIVE: backend/frontend release `20260910_130124` in green, exact application commit `7ea5cb6f1216d71d75391df90d6e55cbcf060928`, merged through PR #115 as `0b1fbbc11c725075179fe8fc9dba39ca08632930`. Both source manifests, image identities, current symlinks and slot states agree. Activation completed at 13:13:05 UTC / 18:13 Tashkent. Blue `20260910_122200` remains running for rollback.
- Process Tracking now shows closed Cutting with usable output and no open replacements as green Completed even below plan. PO-0150, PO-0153, PO-0154, PO-0161 and PO-0163 were already closed; their real quantities (498, 588, 366, 300, 300 out of 600), accepted shortages and Sewing acceptance were preserved. No business row, schema, permission or workflow transition changed. Open/blocked/replacement-pending Cutting and other department display rules retain their prior behavior.
- Source manifest SHA-256 `d0389087f2bd7e0153b2553e32b383f9d374ab84a5edeb8b355e4b24edd085dd`; archive 2365794 bytes / 805 files verified against Git, SHA-256 `9320b1ffa0517901c0f2546cc506e0f2eb02555fa4ebcaae24eaac409b99f6e4`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:c2cb1e8e24730f620bf27170d46f851490b9d5d57603e3b89ba34db224ab5da5`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:7f717c6e15f878caf6f2742672edbc1bb05e0f0e2dd95c31f792180f5c3ffebe`. [Immutable release CI 34480111987](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34480111987) and prior push CI passed. Prior local regression, frontend contracts, strict TypeScript, scoped lint, optimized build and read-only production payload replay are recorded below. Extra tests/reviews, live browser/performance exercises and extended observation were omitted at the explicit fast-deployment request; they are not claimed passed.
- Verified fresh backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_130124.dump`: 52259900 bytes, mode 0600, 1160 restore objects, SHA-256 `4cdf5fc90893a412f9032c71f7b018b5316a71c041575a8d0f9ee8c66fdd6e9a`; restore-list 83166 bytes / SHA-256 `e9e3ec2ee699b04f77719756a41a829391197012edfe356163d39d097a213fb7`. Backup hashes, permissions, sizes and freshness were rechecked before staging. Schema remains `0122_cutting_material_details`; standard startup migration was a no-op and seed was disabled.
- Candidate and immediate live checks passed: all four internal/public health/login endpoints HTTP 200; valid manifests/routers; two workers per backend slot; zero restarts, OOM or error-log markers; PostgreSQL 22/100 connections and zero invalid indexes. Disk 72% backend / 62% frontend; prior disk alert and historical risks remain follow-ups. Immediate postcheck completed at `2026-09-10T13:13:07.844830+00:00`. No extended observation is claimed.
- Dedicated worktree `C:/ERP/.codex-work/close-partial-cutting-20260910`; implementation branch `codex/close-partial-cutting-20260910` pushed, merged and deployed. Deployment record branch `codex/record-closed-cutting-20260910`. Evidence `outputs/deployment/evidence.json`. Legacy checkout preserved.

## Closed Cutting timeline display corrected locally (2026-09-10)

- User requested closure of all currently partial Milana Cutting orders after Process Tracking showed PO-0161 and PO-0163 as Partial. Fresh production database and read-only Process Tracking checks found every Milana cut with output below plan already completed: PO-0150 at 498/600, PO-0153 at 588/600, PO-0154 at 366/600, and PO-0161/PO-0163 each at 300/600. Accepted shortages are already recorded as 102, 12, 234, 300 and 300; none has open Cutting replacements. There were no open partial Milana Cutting orders to close, so no business row was changed.
- Root cause is the frontend timeline's requirement that output reach plan even when Cutting status is completed. The scoped fix shows completed Cutting with positive usable output and no open replacements as green/Completed while retaining actual/planned quantities. Open, blocked, cancelled and replacement-pending Cutting and all other department rules retain their prior behavior. This supersedes the receipt-timeline section's former partial-label policy only for closed Cutting.
- Validation: regression reproduced the original failure, then passed with coverage of the five live quantities, open/blocked/cancelled/replacement states and other department behavior. Replaying the five read-only production stage payloads through the corrected function returned green/Completed with unchanged quantities and Sewing acceptance. All frontend build contracts, strict TypeScript, scoped ESLint, optimized 86-route build and whitespace checks passed. The first build stopped because its API environment was unset; rerunning compilation with the documented CI API settings passed. No live UI deployment or browser QA is claimed.
- Both VM manifests and slot states match `deploy/production-base.json`: active blue `20260910_122200`, rollback green `20260910_093726`, source manifest `767e9423fa084756ca1ef527747fa5e38ba453375f89ca82d1a2ba55ca75312c`, schema baseline `0122_cutting_material_details`. No database write, backup, migration, release change or restart was needed. Production still has the old display until deployment is authorized.
- Worktree `C:/ERP/.codex-work/close-partial-cutting-20260910`, branch `codex/close-partial-cutting-20260910`, based on verified `origin/main` `8006c10f`; legacy checkout preserved. Read-only stage evidence is in ignored `outputs/live-timeline.json`. Merge and deployment remain pending authorization.

## Production order Plan size editing deployed (2026-09-10)

- LIVE: backend/frontend release `20260910_122200` in blue, exact application commit `5de311c022121182f786fd81f462b66ca681b920`, pushed and merged through PR #113 as `13b4cfbad69018ba3b02345514e72cd5a5de5adb`. Both source manifests, image identities, symlinks and slot states agree. Cutover completed at 12:13 UTC / 17:13 Tashkent. Green `20260910_093726` remains running for rollback. This supersedes the prepared section's authorization and deployment status below.
- Production Order detail now offers Plan → Edit for authorized planning users before Cutting starts. Existing size labels can be changed with Save/Cancel, shared adult/child/Free Size suggestions and EN/RU/UZ text. Colors and quantities stay intact. The narrow audited endpoint validates permissions, exact current rows and original sizes, duplicate color/size pairs, and downstream work before changing labels. No existing order sizes or other business rows were changed by deployment; schema remains `0122_cutting_material_details`, startup migration was a no-op, seeding disabled.
- Source manifest SHA-256 `767e9423fa084756ca1ef527747fa5e38ba453375f89ca82d1a2ba55ca75312c`; archive 2364044 bytes / 805 Git-verified files, SHA-256 `29cf0952af29c48ddfe3b4a2874f768d92b5df47ea78b5941caf6d750f11211b`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:7a6d2e15e70033568752134b93e2652e6f6873a6ab143e0e1ee75cdcdf9a10af`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:98b89394369d7c81d43672841125ada062feada8bb6675567897457a8dedb046`.
- Pre-deployment validation included 21 focused backend/handoff cases, Ruff, Python compilation, normal/strict TypeScript, scoped ESLint, frontend contracts, optimized build and six offline browser fixtures described below. Standard push CI passed 882 backend tests and 11 deployment-observer tests. [Immutable release CI 34474515528](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34474515528) passed backend/frontend validation and published both exact-commit images. Extra tests/reviews, live browser/performance exercises and extended observation were omitted at the user's explicit fast-deployment request, not claimed passed.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_122200.dump`: 52255191 bytes, mode 0600, 1160 restore objects, SHA-256 `2833d48497dacfdfcfaa1c420fef8c8a21208eee4a26b682afc6319b9cf2aa89`; restore-list 83166 bytes / SHA-256 `3b57af6b34bd1da0d90a7177c1405f28ebf4cc3050b6ebc072a30642e7be55ef`. Backup freshness, hashes and permissions were revalidated before staging.
- Candidate and immediate live checks passed: all four internal/public health/login endpoints HTTP 200, valid manifests and routers, two workers per backend slot, zero restarts/OOM/error-log markers, PostgreSQL 25/100 connections and zero invalid indexes. Disks 71% backend / 61% frontend; the existing disk alert and historical risks remain separate follow-ups. Immediate postcheck completed at 12:13:23 UTC; extended observation is not claimed complete.
- Dedicated worktree `C:/ERP/.codex-work/production-size-edit-20260910`; implementation branch `codex/production-size-edit-20260910` pushed/merged/deployed; completion record branch `codex/record-production-size-edit-20260910`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved.

## Production order Plan size editing prepared (2026-09-10)

- Added an Edit action to the Plan table on the standard Production Order detail page. Planning users can edit the existing size labels inline, with shared adult/child/Free Size suggestions, Save and Cancel, localized validation and immediate updated rows. Existing colors, quantities, printing flags and item identities are preserved; this is not a model-catalog size change or a quantity editor. Important text is present in both source and runtime EN/RU/UZ dictionaries.
- New narrow `PATCH /api/production-orders/{pid}/sizes` requires `planning.production` or admin permission, matching the Summary editor. It locks the order/work orders/items, rejects closed or Usluga orders, started Cutting, completed rows, cutting records, bundles and issued payroll labels. Requests must include each current item exactly once with its original size; foreign/missing/duplicate row IDs, stale sizes, blank/overlength sizes and duplicate color/size combinations are rejected before mutation. Extra fields such as quantities are forbidden. Successful changes record before/after sizes in the audit log. Existing department breakdown correction routes remain unchanged.
- Validation passed 18 focused backend cases, including authorization, failed-save atomicity, stale editors and Planning → Cutting → bundle creation with updated Free Size/child labels, plus three existing production breakdown/summary regressions. Ruff, Python compilation, normal/strict TypeScript, scoped ESLint, every existing frontend build contract and the optimized 86-route production build passed. Six offline browser fixtures exercised EN/RU/UZ at desktop and 390-pixel phone widths: Save, Cancel, duplicate validation, failed-save draft preservation and read-only visibility passed with no page errors or outer-page horizontal overflow. Desktop/mobile screenshots were visually inspected. Automatic approval review blocked launching a local app server, so integrated live-page browser QA was not performed; the isolated editor fixture was used instead.
- Worktree `C:/ERP/.codex-work/production-size-edit-20260910`, branch `codex/production-size-edit-20260910`, based on verified `origin/main` `9f43fb30`; legacy checkout preserved. Both production manifests and `deploy/production-base.json` matched active green `20260910_093726`, manifest `18e69b319fb76c1c636d6f956ab60b32a6b95a5b069b546e372205d47bc2d6b7`, rollback blue `20260910_091900`, schema `0122_cutting_material_details`. No production business row, schema, release, or service changed in this feature task. Deployment and merge are not authorized; the prepared branch is for review. Browser evidence is under this worktree's `outputs/` directory.

## Free Size option deployed (2026-09-10)

- LIVE: backend/frontend release `20260910_093726` in green, exact application commit `fea68512142929266c05d36d1611235bc67b2825`, merged through PR #111 as `d82d35ac`. Both source manifests, image identities, symlinks and slot states agree. Activation completed 09:48:36/09:48:37 UTC (14:48 Tashkent). Blue `20260910_091900` remains running for rollback.
- Added the exact label `Free Size` to the shared garment-size choices used by Models, Planning, Sales and manual model-size entry. It is a standalone size family, so model size-range entry saves exactly one `Free Size` value. Existing adult/child choices and defaults remain. Backend string fields already accept this value; no backend, schema, permission or business-data changes were needed.
- Source manifest SHA-256 `18e69b319fb76c1c636d6f956ab60b32a6b95a5b069b546e372205d47bc2d6b7`; archive 2358254 bytes / 802 Git-verified files, SHA-256 `3815f6fd68d8d723ee435d4b6f02a83028c47c80f2feed7de5c64ddff5ee2772`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:184f146a5990bfa47c47186c0a27ee89d225420d177df605983527ffbdd113a2`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:001ad8843c62a50a350d8c552d1563dd9178580c3a60a56c8aadb0cb821053db`. [Immutable CI 34461659782](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34461659782) passed standard backend/frontend validation and production image builds. The existing size contract was updated to 28 options and passed locally, including its per-option standalone range check; whitespace validation passed.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_093726.dump`: 52240911 bytes, mode 0600, 1160 restore objects, SHA-256 `d89746391979c17bb73a751ff699404f3501036c0bcccff42ced032fbc387860`; restore-list SHA-256 `215f2c2c8029eedf0ced12b5772615dbd10d1c082a56289fb6aaa91a52d7f91b`. Database remains `0122_cutting_material_details`; standard startup migration was a no-op and seed was disabled. No business rows or existing model sizes were changed.
- Candidate and immediate live checks passed: four internal/public health/login responses 200; valid manifests and routers; two workers per backend slot; zero restarts, OOM or error-log markers; PostgreSQL 24/100 connections and zero invalid indexes; disk 71% backend / 61% frontend. Extra tests/review, browser/performance exercises and extended observation were omitted at the user's explicit fast-deployment request, not claimed as passed. Backend disk alert threshold and historical risks remain unchanged.
- Dedicated worktree `C:/ERP/.codex-work/free-size-20260910`; implementation branch `codex/free-size-20260910` pushed/merged; completion record branch `codex/record-free-size-20260910`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- Sewing, Packaging, Printing and the shared department inbox show one compact list grouped by BSO/order, merging duplicate incoming/pending/in-progress/completed rows by work-order identity. Yellow means ready/received work or output still pending; green means completed in the current department; white means no ready/received/output quantity yet. Upstream completion and automatic activation alone cannot make a row completed. EN/RU/UZ labels, pictures, quantities, deadlines and existing actions remain. Specialized Cutting and physical package/shipment sections retain their established behavior.
- Source manifest SHA-256 `9d085de0c29de91e612d21f730c05c6f433c217e424a544d0baa5d3fc3c3491e`; archive 2357485 bytes / 802 Git-verified files, SHA-256 `78d30e7414f0f148171672e673a9d8697b24b561faefda816db9c972bc501d10`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:b49c21d1836e5ce9ea0a470e3a818f73e62af6ae756a7691f166e0a7735db02a`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:62170666edcc4a5be7ae2fe3a343efdbd0d76906c146a6bae9cbfe863ea742c0`. [Immutable CI 34459960486](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34459960486) passed standard backend/frontend validation and production image builds. Local strict TypeScript, scoped ESLint and whitespace checks passed.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_091900.dump`: 52235629 bytes, mode 0600, 1160 restore objects, SHA-256 `d13726c6871081d0b5b641731dbcf6c7c4182c94b059d65b4af64e00e2496486`; restore-list SHA-256 `8cf68851293c3a78dadd68c7cfc3fcd357bbf6f5bac0aaa5781f3d6d28b15e82`. Database remains `0122_cutting_material_details`; standard startup migration was a no-op and seed was disabled. No business data, schema, permissions or workflow transitions changed.
- Candidate and immediate live checks passed: four internal/public health/login responses 200; valid manifests and routers; two workers per backend slot; zero restarts, OOM or error-log markers; PostgreSQL 26/100 connections and zero invalid indexes; disk 71% backend / 61% frontend. Separate review, browser/performance exercises and extended observation were omitted at the user's explicit fast-deployment request, not claimed as passed. Backend disk alert threshold and historical risks remain unchanged.
- Dedicated worktree `C:/ERP/.codex-work/compact-departments-20260910`; implementation branch `codex/compact-departments-20260910` pushed/merged; completion record branch `codex/record-compact-departments-20260910`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- User-selected labeled timeline is deployed with garment/fabric pictures, separate Cutting, Printing, Sewing, Packaging and Storage columns, sewing line and due date. Existing filters, factory scope, navigation, batch details and actions remain; EN/RU/UZ labels and mobile scrolling are supported.
- Sewing acceptance uses persisted `received_sewing` bundle quantities, grouped once per page and attributed to the exact batch. Automatic activation, planned line assignment and input counters do not prove receipt. Available pieces fully accepted is green; pending or partial acceptance is yellow, with ready/accepted amounts and usable sewn output displayed separately. Partial output stays yellow even when a department has formally completed with shortage. Skipped Printing and future stages are gray; blocks and overdue text remain explicit.
- Candidate read-only comparison against the active application preserved every existing response field and ordering for all three factory lists: Milana 10 orders, Besttex 13, Eco Cotton 6. Receipt fields matched persisted bundles. Exactly two Milana orders had receipts: PO-0155 600 and PO-0150 498, both with zero sewn output. Eco USL-0001 separately had 910 received. This release adds read-only receipt/output fields and presentation; no workflow transition, authorization, schema or business data was modified.
- Source manifest SHA-256 `3a93c9d47439679df1f5a2998cbaf1dc166f81437c3e5f818eb442711347970e`; archive 2356108 bytes / 802 files verified byte-for-byte against Git, SHA-256 `27de1b2689d33a6dbaa7e6edb2544b6398aea2379fe525832dbd4801357e979c`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:ed22d0903c8a7538a28469318e2119ce2e030fbadd3cb4c301c6ca7b884d0e72`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:e3cd3c0cb9814e1832364b214ae318fca2c7c195b52b6daf7e7f93f763869fb3`. [Immutable CI 34456985509](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34456985509) passed 864 backend tests and frontend/deployment checks. Focused local receipt/batch/idempotency tests, timeline contracts, Ruff, strict TypeScript, scoped ESLint and production build passed.
- Fresh verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_084513.dump`: 52225766 bytes, mode 0600, 1160 restore objects, SHA-256 `23335a287d9245501e1891c688cfcf899c4e1c62e03b98a163f66a9788688a74`; restore-list SHA-256 `0060b45b029c3aae746e746d29e6ac976db4cc17fd880b441017078f647b1154`. Schema remains `0122_cutting_material_details`; candidate standard startup migration was a no-op and seed was disabled.
- Immediate candidate/live checks passed: all four internal/public health/login responses 200; valid manifests/routers; two workers per backend slot; zero restarts, OOM or log-error markers; PostgreSQL 30/100 connections and zero invalid indexes; disk 71% backend / 61% frontend. Extra browser/performance exercises and extended monitoring were omitted at the user's explicit fast-deployment request. Standard observation is not claimed complete; rollback remains available. Historical risks are unchanged.
- Dedicated worktree `C:/ERP/.codex-work/process-timeline-20260910`; implementation branch `codex/process-timeline-20260910` pushed and merged; completion record branch `codex/record-process-timeline-20260910`. Operational evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved. The prior pre-0032 cleanup and passport verification remain recorded below and were not repeated.

## BSOrders before 0032 deleted from production (2026-09-10)

- Completed the user's explicit full cleanup of BSOrders `0001` through `0031`, inclusive, using the exact committed one-off script `318d14a6` in a locked database transaction. The cutoff uses `branded_planning_orders.order_no`, not database IDs or production-order suffixes. BSOrder `0032` retains its nine production orders / 36 work orders; `0033` retains 14 production orders / 57 work orders. All retained rows in affected tables passed before/after fingerprints inside the transaction.
- Deleted 31 planning groups, 73 production orders, 293 department work orders, 49 production batches, 424 size rows, 48 material assignments, 38 passports, 50 cutting records, 24 material-usage rows, 874 bundles / 1,066 scan logs, 27 sewing assignments, 47 linked daily sewing reports, 22 sewing records, one replacement request, nine packaging records, 190 packages / 1,117 package items / 115 batch allocations / 406 scan logs, 1,117 finished-goods rows, 45 waste rows, 552 payroll QR labels, 11 already-voided payroll records, ten package change requests, 950 linked stocktake rows, 1,894 historical reference aliases, 447 notifications and ten saved idempotency responses. No payable payroll record, payroll period, sales order, shipment, customer, model or unrelated inventory was deleted.
- Preserved all physical material balances and all 1,781 stock movements. Exactly 67 historical consumption movement references were reclassified as `DeletedProductionWorkflow` with a null reference ID; their quantities and all other fields were unchanged. No material was returned to stock. No stock-batch reference needed clearing. Historical audit logs remain intact; cleanup audit entry is `18652` and contains the plan digest, counts, prior movement references and backup path. Unrelated/manual sewing history remains preserved.
- Verified full PostgreSQL backup: `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_081136.dump`, 52,385,997 bytes, mode 0600, 1,160 restore objects, SHA-256 `f2a3e195514fe2afbaa744901a6c09793c7a27ccd78fcc79959976ce1944333c`; restore-list SHA-256 `97f9c69b922fbe1d47722f0047a13f6c50b3dd266bcf9e40c5b6ff8aa27e8deb`. Rechecked both hashes immediately before apply.
- Archived and removed exactly 2,127 generated QR/barcode files (1,751,682 bytes) from active barcode storage. Recoverable copies remain outside public storage at `/opt/milana-erp/shared/backups/bs_before_0032_20260910_081136`; media-manifest SHA-256 `6ac688f843327c3fa16a386fad19e3529a4b87efe50409372343a77a252d63b5`. Each exact path and its archived checksum were validated before removal; shared model/material images were untouched.
- The user also reported the long order number on passport/report `CUT-101`. Read-only regeneration on the current production renderer returned canonical `PO-0155`, not historical `SO-2026-000155`, both before and after cleanup. Its production order belongs to retained BSOrder `0033`. The number is already fixed by the current numbering release; reprint the report. No extra number truncation or renumbering was introduced.
- Active application release remains `20260910_065757` / blue on both VMs, source manifest `207d336b31b4675b3280f727706e6ffc8cde4e9217a8246c06013c683bc9f485`; green rollback remains `20260910_061535`; schema remains `0122_cutting_material_details`. No application rebuild, release deployment, migration or restart was needed. Application rollback does not restore deleted data; recovery requires the retained database/media backup.
- Checks: both VM manifests and slot states, read-only dependency/soft-reference plan, backup restore-list and hashes, Ruff, Python compilation, exact committed script guard, transactional retained-row fingerprints, post-commit cutoff/group counts and regenerated passport number, and all four immediate internal/public health/login responses at HTTP 200. Per the user's request, no extra test suite, browser exercise, performance benchmark or extended monitoring was run.
- Worktree: `C:/ERP/.codex-work/delete-bs-before-0032-20260910`; branch: `codex/delete-bs-before-0032-20260910`; execution commit `318d14a6` pushed. Operational evidence is under `outputs/` in that worktree. The legacy `C:/ERP` checkout was preserved.

## Payroll adjustment deletion and Forecasting repair deployed (2026-09-10)

- LIVE: backend/frontend release `20260910_065757` in blue, exact application commit `3a11b7a15ac21fb8b47396c5e170c0366a5eaa30`, merged via PR #104 as `4d39bb5f`. Both manifests, image identities, symlinks and slot states agree. Activation completed around 07:09 UTC / 12:09 Tashkent. Green `20260910_061535` remains running for rollback.
- Payroll now offers Delete for ordinary adjustments in an editable period or without a period, with confirmation and immediate totals refresh. The backend requires payroll.manage, scopes by session factory, serializes deletion, blocks finalized/cancelled periods and linked reversal adjustments, and preserves a full deletion audit snapshot. Deployment deleted no adjustment: the one existing payroll adjustment remained unchanged.
- The production Forecasting `int(None)` crash is repaired. Descriptive BOM rows without an inventory identity are omitted from item demand and explicitly reported; a selected batch supplies its item identity when present. Candidate authenticated GETs returned 11 production suggestions, 11 material reorder alerts, eight weekly points totaling 1,200, zero saved recommendations and one payroll adjustment. All five affected GET endpoints returned 200. The 18 unlinked active-model BOM descriptions still need actual inventory links to contribute to material demand; no material identity or business row was invented.
- Forecasting now aligns chart buckets with Monday labels, avoids redundant stock lookups, displays load/action errors, refreshes both datasets, prevents overlapping actions, shows saved item labels and localized confidence, and opens the correct inventory group. Planning links respect production permission and carry the suggested brand. New UI messages support EN/RU/UZ.
- Immutable [CI run 34447632634](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34447632634) passed all 863 backend tests, frontend lint/strict TypeScript/contracts/optimized build and image publication. Local checks passed 27 focused backend cases, Ruff, strict TypeScript, scoped ESLint and chart contract. A first artifact download selected a Docker build record rather than the source ZIP; explicitly downloading the named source artifact succeeded before any staging.
- Source: 797 Git-verified files, archive 2346856 bytes / SHA-256 `6026c58f7c2907a6a149775b6239ea7a3e3116b23511bb61b1871186ecea1a8c`, manifest `207d336b31b4675b3280f727706e6ffc8cde4e9217a8246c06013c683bc9f485`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:c594953c7b7f97e5fa79c55fe4b6c8f0a46234dcb8fed0865c28063142b318f2`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:74f1428ea3d5a9a65dd5b14c4b2dcdff215f21cb2cf700e9e041fe2cc7b65809`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_065757.dump`: 52384089 bytes, mode 0600, 1160 restore objects, SHA-256 `4999722bbce620c62721ead2b352f73076ca79b2396366f8e43dc6f27ad58db3`; restore-list 83166 bytes / SHA-256 `8c8a5d2db4c8649979c698fa1147256d2cb06bd192c8e312c3eeb12556fadf77`. No schema change; startup Alembic was a no-op at `0122_cutting_material_details`, seeding disabled. No production business rows were created, deleted or changed.
- Immediate post-cutover checks passed: all four internal/public health/login endpoints 200; valid routers and manifests; both backend slots two workers, zero restarts/OOM/recent error markers; PostgreSQL 26/100 connections, zero invalid indexes. Disk 71% backend / 61% frontend. Historical risks and the existing backend disk alert remain separate follow-ups.
- User requested fast deployment without extra tests or monitoring. No extra browser/performance exercise or extended observation was performed or claimed passed. Retained rollback remains available. Worktree `C:/ERP/.codex-work/payroll-forecast-fix-20260910`; implementation branch `codex/payroll-forecast-fix-20260910` pushed/merged/deployed; record branch `codex/record-payroll-forecast-deploy-20260910`. Evidence `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- Cutting material rows now have Change batch / Изменить партию / Partiyani almashtirish. Saving a selected active fabric batch in the same unit updates the planned assignment, primary fabric reference and linked passport material names/batch numbers atomically. Per-material details, planned quantities and product/bundle totals are preserved. Unused old reservations are released and only the missing replacement quantity is reserved; actual stock is consumed once through the normal Cutting record submission.
- Duplicate targets, stale correction requests, insufficient stock, invalid/archived/empty batches, unit mismatch, unauthorized stage/factory, closed/rejected orders and Usluga are blocked. Already-consumed material or reservation history cannot be relabelled. Changes are audited against the order and affected passports. This deployment did not change any business row, stock quantity, reservation, passport, order assignment, permission or user; the new action runs only when a user saves a correction.
- Concurrent completed release `20260910_060201` and all Cutting completion/routing work were preserved. Initial candidate `20260910_061251` / run `34444225997` was cancelled before staging after main advanced. New source reconciled fresh main `5ced83ca` and both active manifests before a complete new immutable build. No other task was messaged, interrupted or edited.
- Source: 797 files verified byte-for-byte against Git, archive 2341231 bytes / SHA-256 `15486cc31d8af0eaa00419220279276877d1ff5d0afad102a5db4297b24dfbb2`, manifest SHA-256 `513061fb12089e1e199f50483b5672ba688cf6420719c5c546c77fff73aeaab1`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:dcbfdddf243adc0f4fdd44fa8f46289ed14ea00c1865fd1054d3b097d5803724`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:88895286811ddab24ef091bdbb6822be42d4360e1337ae731188aa0edee77f80`.
- [Immutable CI run 34444423933](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34444423933) passed all **854 backend tests**, frontend lint/strict TypeScript/contracts/optimized build and image publication. Focused local correction/reservation/passport/actual-consumption and previous addition handoff regressions passed (2), as did Ruff, TypeScript and changed-file ESLint; focused checks were repeated after integrating concurrent application changes.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_061535.dump`: 52383447 bytes, mode 0600, 1160 restore objects, SHA-256 `180420535af7c4536f3018c40917118fbc3a05735acb18ff51f8253261c16a5b`; restore-list 83166 bytes / SHA-256 `f0ddd316c9541b2a1f5ef2f39b5c7974ec13661df0808dd41ba8ae2a4b706f9a`. Both files' hashes, sizes, permissions and freshness were rechecked before staging. No migration or model/schema change; database remains `0122_cutting_material_details`, candidate startup Alembic was a no-op and seeding was disabled.
- Immediate candidate/live checks passed: four internal/public health/login endpoints HTTP 200, valid routers, both backend slots two workers, zero restarts/OOM/log-error markers, PostgreSQL 24/100 connections and zero invalid indexes. Disks 71% backend / 61% frontend; no cleanup. Backend disk alert remains a separate maintenance follow-up.
- User explicitly requested fast deployment without extra testing/review. No independent review, browser QA, performance exercise or extended observation was performed or claimed passed. Backup, immutable CI, source reconciliation, warmup and immediate health/runtime gates were retained. Rollback is retained; standard observation is not claimed complete.
- Worktree `C:/ERP/.codex-work/cutting-change-batch-20260910`; implementation branch `codex/cutting-change-batch-20260910`; record branch `codex/record-cutting-change-batch-20260910`. Implementation pushed and merged, deployed as above; evidence `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- User explicitly corrected PO-0148 and PO-0152 to Milana Cutting with Besttex Sewing. At 06:02:49 UTC, a guarded transaction changed only Cutting work orders 658/674 from ECT (department 20) to CUT (department 4), plus update timestamps and audit entries. Sewing work orders 659/675 remained BST and waiting; both productions had zero cuts/bundles before reassignment. Live CUT inbox verification includes both production orders (170/174).
- The same user-requested transaction completed the three existing short-count Milana cuts from the screenshots: PO-0150 / work 666 at 498 of 600, accepted shortage 102; PO-0153 / work 678 at 588, shortage 12; PO-0154 / work 682 at 366, shortage 234. The existing complete-with-shortage operation set completed/end time and recorded shortages in failed_qty. Original plans and all physical quantities were unchanged. Full before/after WorkOrder comparison allowed changes only to these five rows; all their other work orders were unchanged. Entire targeted CuttingRecord/Bundle payload fingerprints matched before/after. Five audit entries document the corrections. Live Branded Stock shows the three cuts completed.
- Milana's authorized final-sheet Print action now posts to `/api/cutting/records/{rid}/production-sheet`, renders the sheet and closes Cutting using recorded output even below plan. The button reads Print sheet and finish Cutting in EN/RU/UZ. Ordinary GET viewing and completed reprints do not mutate state. ECT/Usluga retains its separate batch workflow. Completion is tied to issuing the final sheet; browser APIs cannot establish whether paper physically printed. Malformed requests, cancelled/rejected work, zero usable output, unresolved replacement work and wrong-factory sessions cannot trigger completion. Passport updates, new Cutting records and final-sheet completion share production-then-work-order lock order.
- Source manifest SHA-256 `022321139360c3de1b7dc0b1a7fcc82b77f0f1c65acadc534b695b1d78577540`; deterministic archive 2334082 bytes / 795 verified files, SHA-256 `050732a3b9da8b2d2518108ebc07e0525e42635653130544696fe24e425972cb`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:4784b2f1187cc3a82f8c693c5e4979401fdd819262626dbaea328dd72d530980`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:af91b61c14a364bc6469b7cee31f8a678eb68944fe4af00ab68960597d642c61`. [Immutable CI 34443477434](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34443477434) passed 853 backend tests plus frontend/deployment checks. Focused local completion/GET/retry/malformed-request checks, Ruff, strict TypeScript and scoped ESLint passed.
- Candidate actual-data reprint and factory checks passed twice with SQL writes explicitly blocked: each issued the already-completed PO-0150 sheet twice, kept 498 passed / 102 shortage unchanged, and rejected an ECO login for Milana work. An initial candidate harness invocation exited nonzero with its inner output suppressed; its cause was not retained. Two subsequent identical checks with output enabled passed without code changes. This remains a QA evidence limitation, not a claimed diagnosed or repaired application defect.
- Backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_060201.dump`: 52384090 bytes, 0600, 1160 restore objects, SHA-256 `630e939c266a9892848037b331e7c0bb21e23e95c0c68ba884cdeb2e9622443a`; restore-list SHA-256 `16825cf8782820dea156ff3e894360a57df3926277b45fba1787f6e428b746d4`. Backup preceded the data correction and cutover. Database remains `0122_cutting_material_details`; standard startup migration was a no-op. No stock, bundle, cutting record, model or physical output was created or altered.
- Immediate candidate/live checks passed: all four internal/public health/login requests 200, valid manifests/routers, two workers in each backend slot, zero restarts/OOM/log-error markers, PostgreSQL 22/100 connections and zero invalid indexes. Disk 71% backend / 61% frontend. No cleanup, broad browser/performance exercise or extended monitoring was run, following the user's fast-deploy request. Rollback retained; standard observation is not claimed complete.
- The first candidate build `34443075443` was intentionally cancelled before production changes after passport PR #98 advanced main. Work reconciled with its verified deployed baseline before the final rebuild. Worktree `C:/ERP/.codex-work/milana-cutting-close-20260910`; implementation branch `codex/milana-cutting-close-20260910`; completion record branch `codex/record-milana-cutting-close-20260910`; evidence `outputs/deployment/evidence.json`. Legacy checkout preserved. Historical risks and backend disk alert threshold remain separate.
...
- Dedicated worktree `C:/ERP/.codex-work/milana-cutting-close-20260910`, branch `codex/milana-cutting-close-20260910`, reconciled with verified main `1e708cf0` and active passport release `20260910_054836`, manifest `659e8d58c3587f837a2cad139c7a422ca5b818076533167e91a0968eb1b20be2`. Focused completion/retry/read-only print tests, Ruff, strict TypeScript and scoped ESLint passed. Fast deployment authorization and omission of extra monitoring continue from this conversation; immutable CI, backup, candidate checks and immediate health/runtime checks remain.
...
- Cutting Passport now offers Add material / Добавить материал / Mato qo‘shish for an open warehouse-backed Cutting order. The user selects an inventory batch and positive quantity. Save appends it to the same order and reserves only the missing quantity atomically; the subsequent Cutting record must include it and consumes it once. Cancellation makes no assignment. Per-material details, stock-batch identity, passport calculations, saved evidence and printouts are preserved, while product pieces and bundles remain counted once.
- Duplicate saves cannot duplicate assignments/reservations. Insufficient stock, invalid units, archived/empty batches, unauthorized/factory-mismatched users, closed Cutting and Usluga inventory additions are rejected. Existing assigned material rows are not removed or silently replaced. The passport change is audited against the production order. This feature does not retrospectively invent missing material consumption for completed cutting records.
- Concurrent release `20260910_053604` (Branded Stock partial Cutting status/department visibility) was detected before any staging. Superseded build `34442472691` / candidate `20260910_054717` was cancelled and never deployed. The deployed source incorporates the other task's complete changes, with both new-baseline manifests verified before proceeding.
- No migration or schema change; database remains `0122_cutting_material_details` and candidate startup Alembic was a no-op. Deployment created or changed no business row, assignment, reservation, stock quantity, passport, user or permission. Only users saving passports invoke the new business action.
- Source manifest SHA-256 `659e8d58c3587f837a2cad139c7a422ca5b818076533167e91a0968eb1b20be2`; archive SHA-256 `578f3daa77571852685e3d19cc7e30410b6f5e72d2e6314fd49af00e2a48395e`; 794 files verified against Git. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:7fb04339ae6d0cc6c9686dd5655672e3d2414cd47d78caa772c41427a79f2d15`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:b0fa07d19a02f6f5492967a9e5394434b1572a7956895ac909587081d5329b1e`. Immutable [CI run 34442557961](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34442557961) passed all 852 backend tests, frontend lint/strict TypeScript/contracts/optimized build and release publication.
- Focused local checks passed the authorized Cutting add/save/reserve/consume handoff, insufficient-stock rollback, repeat-save idempotency, missing-material rejection at Cutting, preserved bundle total, closed-Cutting and Usluga exclusions. Changed-file Ruff/ESLint, strict TypeScript and optimized 86-page build passed; all runtime EN/RU/UZ dictionaries contain the new controls.
- Backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_054836.dump`: 52,383,767 bytes, mode 0600, 1160 restore objects, SHA-256 `c80bf56f7f6be396f4bc843df333cf3d0588229e43dd79cfc799d6eb01d6cc6d`; restore-list SHA-256 `f38cec13e47946faccc4624a1d06d04c7470f975fa726579656f13d70b111999`. Hashes, sizes, permissions and age were rechecked before staging.
- Immediate candidate and live checks passed: four internal/public health/login endpoints HTTP 200, both routers valid, both backend slots two workers, zero restarts/OOM/log-error markers, PostgreSQL 26/100 connections and zero invalid indexes. Backend/frontend disks 71%/61%; no cleanup. Backend 70% disk alert remains a maintenance follow-up.
- Continuing the user's earlier explicit fast-deployment/no-extra-review-or-tests preference, there was no independent review, signed browser QA, performance exercise or extended observation. These omissions are not recorded as passed checks. Backup, immutable CI, source reconciliation, warm-up and immediate health/runtime verification were retained.
- Worktree `C:/ERP/.codex-work/passport-add-material-20260910`; implementation branch `codex/passport-add-material-20260910`; record branch `codex/record-passport-add-material-20260910`. Evidence `outputs/deployment/evidence.json`. Legacy checkout preserved.
...
- Individual partial Cutting rows previously displayed Not cut because the component mapped all unfinished states to not_started. They now show Partially cut with yellow shading, actual Cutting quantity versus plan and their assigned Cutting department; completed rows remain green and untouched rows white. Branded Stock refreshes every 10 seconds while visible, using existing EN/RU/UZ labels.
- PO-0148 and PO-0152 belong to Eco Cotton Cutting (ECT); their absence from the regular CUT list is correct. Their assignment is now visible in Branded Stock. Neither order was reassigned. Read-only candidate evidence verified PO-0150/0153/0154 at 498/588/366 cut pieces respectively with partial status. All 33 open groups / 96 productions and every pre-existing response field matched the prior release exactly; only the requested quantity/department fields were added.
- Source: 794 Git-verified files, manifest SHA-256 `fae920f01b850b22ebcd791716f9dae0123ed60a1215259bfb97acaee144fcfa`, archive 2322908 bytes / SHA-256 `0a034ceecdf7dd8f6f8a609eaeb5286d1a64d74322b693ea5766e4d65cd1e0d5`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:63cb3639b0fc059fb76d09c82c002ca713c637259e1341a90891f81b8b9ba2b3`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:c32ec0251078aeae0122cba917546ce21e55c921042ae9e84db6d74297720356`. Immutable CI run [34441744880](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34441744880) passed, including 851 backend tests and frontend lint/type/build checks.
- Scoped local validation passed 2 existing Cutting-history tests with quantity and CUT/ECT coverage, Ruff, strict TypeScript, scoped ESLint, existing Branded Stock contracts and actual-component rendering for partial label/color/quantity and ECT location. Candidate data checks used read-only transactions. No business row, order assignment, quantity, role or permission was changed; no new migration was needed. Standard container startup's Alembic upgrade was a no-op at `0122_cutting_material_details`.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_053604.dump`: 52383608 bytes, mode 0600, 1160 restore objects, SHA-256 `b9fd68a530089242b9b4a9a4af1efc76f858346ea5e3f0ff3fb3b72decc3f87f`; restore-list SHA-256 `097aebdea8eb5071756680c0d67a4149e641f57c1405fdd3966ffd3842d93802`. Immediate candidate and live health/runtime checks passed: four internal/public health/login responses 200, both backend slots with two workers, zero restarts/OOM/log-error markers, PostgreSQL 26/100 connections and zero invalid indexes. Disk 71% backend / 60% frontend; no cleanup performed.
- User explicitly requested fast deployment without extra tests or monitoring. No separate browser/performance exercise or extended observation was run; this is not a completed standard observation window. Rollback remains available. The other active task was only inspected read-only, never messaged, interrupted or edited.
- Worktree `C:/ERP/.codex-work/branded-cutting-status-20260910`; implementation branch `codex/branded-cutting-status-20260910`; record branch `codex/record-branded-cutting-status-20260910`. Evidence is in ignored `outputs/deployment/evidence.json`. Legacy checkout preserved. Remaining risks are the recorded historical risks and backend disk alert threshold; no new unresolved failure was found.
...
- Cutting repeats all seven material details per selected fabric, persists them by stock batch, retains independent waste evidence, and keeps product/bundle output counted once. Passport entry loads every planned batch, preserves per-fabric calculations and operators, and supplies batch-specific Cutting autofill. Editing and printed cutting sheets retain separate evidence. Historical entries are not backfilled with invented per-material values.
- Database is now `0122_cutting_material_details`. The additive migration only added nullable JSON fields to `cutting_material_usages` and `cutting_passports`; no existing business values were updated and no business rows were created by deployment. The previous release remains schema-compatible. Migration lock acquisition was bounded to 5 seconds, statements to 60 seconds.
- Source manifest SHA-256 `8ea4bf55b6b7a725757ac7d71599c1bf8217bda474b5b47958d6fed0a357fa68`; archive SHA-256 `2048d9e0615ea932e6eb84d958ad5765de9ce96b733ff9026937974f6ec2c24b`; `794` files verified against the exact Git commit. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:35d83dd25c5492044462351bdc380de3498fb9c61dc95df97bc7917f7086335d`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:befb8172b3dd31eca803ff899dcf6debc30aa788f9497598b9f0bd1f9622be09`. Immutable build [34440758511](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34440758511) passed.
- Verified pre-migration backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260910_052142.dump`: 52,383,312 bytes, mode 0600, 1160 restore objects, SHA-256 `2faff00f174aa1bcabd6972742888354f5811f4540b8e7973cfebd100837b582`. Restore list SHA-256 `831f14c13f7deb53b635bfa027b53289dab092780fa658d58a93ffe8a51a01da`.
- Validation: 18 focused existing backend checks, an extended existing multi-fabric workflow check covering passport calculations/persistence/printing/editing and unchanged bundle totals, Ruff, changed-file ESLint, strict TypeScript, existing frontend contracts and optimized 86-page build passed. Mandatory immutable CI passed all 851 backend tests plus frontend/deployment checks. Initial local Python/dependency-path/build-environment attempts were corrected before successful runs.
- Immediate candidate and live checks passed: all four internal/public health/login probes HTTP 200, manifests and routers valid, both backend slots have two workers, zero restarts/OOM and zero log-error markers; PostgreSQL 25/100 connections, zero invalid indexes. Backend disk 71%, frontend 60%; no cleanup performed.
- At the user's explicit request to deploy without extra tests or review, there was no separate review, signed browser QA, performance exercise or extended observation. Do not interpret this as completion of the standard observation window. Retained rollback capacity and immediate health/runtime checks passed.
- Worktree `C:/ERP/.codex-work/cutting-material-details-20260910`; implementation branch `codex/cutting-material-details-20260910`; record branch `codex/record-cutting-material-details-20260910`. Evidence: `outputs/deployment/evidence.json`. Legacy checkout preserved. Backend disk above the 70% alert threshold remains a maintenance follow-up.
...
**Requested changes are LIVE and deployment closing is COMPLETE.** The full 10-minute health observation, final runtime/log/database/rollback checks and both closing performance comparisons passed. This section supersedes the preparation-only deployment status below; earlier release records remain historical.

- Active backend/frontend release `20260909_122650` is blue, exact reviewed source `f91e733284fe96d6e5134ce65d2333411b520389`. Initial feature PR [#91](https://github.com/Shmirzaev/Milana-ERP/pull/91) merged as `a8095e3c31b4a6e754756cf17419e898bbd70166`; final Uzbek QR-header correction PR [#92](https://github.com/Shmirzaev/Milana-ERP/pull/92) merged as `292573359d4f1bacb7ecff5654c62c3872a5c96a`, with the same application tree as the deployed source. Backend activation `2026-09-09T12:37:41.014098+00:00`; frontend `2026-09-09T12:37:42.240604+00:00`. Both slot states, exact image identities, manifests, routers and current symlinks passed immediate verification. `deploy/production-base.json` records this actual live identity; re-verify remotely before any future change.
- Source manifest SHA-256 `b1af565d5f3d77c9e94a4eee116c151a0debccdbeffd087755f60c7d430deb75`. Deterministic archive `2,313,696` bytes / `792` exact Git-verified source files, SHA-256 `297a571842294ece6b60727c2060108ab0508a14d8ebfa60ce4eb840760695af`. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:153a08b2df1f98add517c9f2b6d070dabac6366719ce5f66c6c2a61b90ce4e9f`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:2f361b590d749ffb98e50379c7c194093d0da991b2d397efd20f16cc3fbb9508`. Both images were built by successful [immutable CI run 34350926938](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34350926938) from the exact reviewed commit.
- Process QR has compact rate, copies and divide columns, a wrapping/growing operation-name editor, and column headers that fit in Uzbek as well as Russian/English. Long operation names remain readable. Models and manual Process QR share all 14 children's sizes from 98 through 176 in steps of six; automatic start/end ranges stay within the selected adult or child chart. Existing operation identity, size-saving API, saved model sizes, rates, quantities, splits, label issuance and payroll scanning/calculation are unchanged.
- New `/inventory/cutting-fabric-usage` shows real approved Cutting stock-consumption ledger evidence, grouped once per cutting record/material/batch/unit. It includes order/model, cutting date, material/batch, quantity, date/search filters, pagination and totals by unit. It excludes planned reservations, passport/BOM estimates, accessories and customer-owned Usluga material without warehouse consumption; archived consumed batches remain visible. Existing Storage/Cutting/Planning authorization and selected-factory restrictions apply. English/Russian/Uzbek navigation and report text are provided.
- Mubina's existing active account 8 in MIL already has the required Storage permissions and `inventory.materials_only` restriction; no grant was made. Final candidate and public signed GET-only QA both matched independent ledger aggregates: **69 rows / 16,905.3 kg**. They verified full pagination/totals, unit/date/canonical and historic-alias searches, accessory exclusion and denial of the other cutting factory. Public browser QA showed the new report and existing material-only navigation, with accessories still hidden and no browser errors. Browser identity was projected from a read-only transaction; no login, auth/me timestamp update, save or QR issuance was performed by QA.
- Schema remained `0121_stocktake_scan_snapshot`; no migration or model-schema change was applied. This deployment/QA created or changed no production model, size, order, stock, payroll, permission, audit or other business record. Seeding was explicitly disabled. Backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260909_120651.dump` was created `2026-09-09T12:09:16.089705+00:00`: `52,266,072` bytes, SHA-256 `a51d5989bfd5b476dec5e0432042517a06eaf1f3fa87414ff39360412487c531`; restore list `83,166` bytes / `1160` objects, SHA-256 `618f13b69c1f8b40c76bc63e541bc7fbb06a7ee1049f6ff9b77ae8650817b85e`. Both files' hashes, sizes, mode 0600 and age under one hour were rechecked before staging. Reuse was explicit: the preceding unpromoted candidate left active green/schema/data unchanged and no migration was needed.
- Earlier candidate `20260909_120651` / `945037b5054420963ba03d910bb10a16f31549b1` was **never activated**: actual compiled browser QA found overlapping Uzbek QR headers. Its evidence/artifact was preserved. The corrected source went through a new immutable release, fresh public baseline and candidate gates; final browser QA confirmed headers within their columns and all 47 existing operations visible. No business-data repair accompanied this correction.
- Validation: exact-source CI passed **851 backend tests**, Ruff/compilation, observer regressions, frontend lint/strict TypeScript, build contracts and optimized production build. Local focused checks covered five report regressions plus 18 reservation/factory tests, child/adult size ranges and QR ordering; fixture/browser checks covered the changed interfaces. Candidate paired performance passed all unchanged 10% median / 15% p95 / 15% payload budgets using 63 samples per endpoint per slot, three warmups and unchanged fixed queries. Public post-cutover and both closing comparisons also passed with every raw sample retained and unchanged limits; no rerun, discarded samples or budget waiver.
- Immediate postcheck `2026-09-09T12:37:46.413015+00:00` passed all four internal/public health/login endpoints, zero strict log error markers, two backend workers per slot and no restarts/OOM. PostgreSQL was `31/100` connections, zero invalid indexes, unchanged 0121. Root disks were backend `71%` / frontend `60%`; no cleanup occurred. These are immediate measurements, not final observation/closing results.
- Closing verification: the observer ran for 600.17 seconds, with 21 complete four-endpoint rounds / zero failures, finishing `2026-09-09T12:50:08.380137+00:00`. Final runtime check `2026-09-09T12:50:29.906155+00:00` found zero log error markers or restarts/OOM, two workers per backend slot, PostgreSQL 33/100 connections, zero invalid indexes and unchanged 0121. Both active blue and retained green exact identities, symlinks and routers passed. Retention was dry-run only; both releases remain protected.
- Performance maxima (median / p95 / conservative payload change): paired-candidate-01 `1.8% / 3.81% / 0.0%`; public-candidate-01 `2.41% / 5.55% / 0.0%`; paired-closing-01 `2.65% / 5.75% / 0.0%`; public-closing-01 `0.76% / 13.08% / 0.0%`. All gates passed at unchanged 10% / 15% / 15% limits. The five fixed queries retained all 63 samples per endpoint (per slot for paired); counts matched and requests succeeded. Raw evidence is in the named attempt folders.
- Retained green `20260909_102118`, source `2203cc24c5f929bab04e1e5a58b956e6f9ca2a67`, remains running as rollback with seeding disabled. It already understands schema 0121 and retains its normal exact-image startup command, so the unchanged schema remains restart-compatible with this rollback release. The older 082533/unknown-0121 warning in the historical stocktake record does not describe the current green rollback. The pre-existing Forecasting nullable-Usluga-BOM exception is **unfixed and outside scope**; any occurrence must still fail the strict log gate, never be hidden or waived.
- Implementation worktree `C:/ERP/.codex-work/qr-sizes-cutting-fabrics-20260909`, branch `codex/qr-sizes-cutting-fabrics-20260909`. Record worktree `C:/ERP/.codex-work/qr-sizes-cutting-fabrics-release-record-20260909`, branch `codex/qr-sizes-cutting-fabrics-release-record-20260909`, starts at fresh main `292573359d4f1bacb7ecff5654c62c3872a5c96a`. Evidence is retained in the implementation worktree under `outputs/deployment-next/` (identity, backup, CI, API/browser QA, performance) and `outputs/deployment/` (unpromoted attempt). Screenshots: `outputs/screenshots/candidate-final-qr.png`, `candidate-model-child-options.png`, `candidate-mubina-cutting-usage.png`, `public-mubina-cutting-usage.png`. No retention cleanup occurred. The completed durable section was mirrored to the Obsidian project-context note.

## Process QR layout, children's sizes and Cutting fabric usage prepared (2026-09-09)

- User requested compact Process QR rate/copies/divide controls, full operation names, children’s sizes 98–176 in steps of six, a Cutting fabric-usage section visible to Mubina, and a fast deployment with focused checks. Worktree `C:/ERP/.codex-work/qr-sizes-cutting-fabrics-20260909`, branch `codex/qr-sizes-cutting-fabrics-20260909`, starts from verified main `de9954f1cb679d2daee5b99a0c68a651c5ba6871`. Both active production manifests and slot states matched green `20260909_102118`, source `2203cc24c5f929bab04e1e5a58b956e6f9ca2a67`, manifest `a60ecb6c52aa85895075ab18b8e22e6b5c4c3628a62e74a3a65e5c38b3fcaee1`, schema `0121_stocktake_scan_snapshot` before editing. Preparation is not a deployment claim.
- Process QR uses compact fixed numeric columns and gives the remaining width to a wrapping, growing operation-name editor. Existing operation identities, rates, quantities, splits, issuance and payroll behavior are unchanged. Models and manual Process QR share all 14 requested children’s sizes; automatic beginning/end ranges stay within adult or child charts. Existing size-saving APIs and stored data are unchanged.
- `/inventory/cutting-fabric-usage` presents actual approved Cutting stock-consumption ledger entries, grouped once per cutting record/material/batch/unit, with date and business-name filters, pagination and totals separated by unit. Planned reservations, passport estimates, accessories and customer-owned Usluga fabric without warehouse consumption are excluded. Archived fabric batches remain visible. Existing Storage/Cutting/Planning permissions and selected-factory scope protect the page and API; the report is read-only and grants no new permission. English, Russian and Uzbek navigation and content are included.
- Read-only PostgreSQL preflight verified Mubina’s existing active account 8, MIL factory, `inventory.materials_only` restriction and existing Storage permissions already permit the report. The snapshot returned 69 consumption rows / 16,905.3 kg in 31.01 ms. No production record, permission, audit or session timestamp was changed; SQL ran under READ ONLY with autoflush disabled. No schema migration is required.
- Focused validation passed five report regressions plus 18 existing reservation/factory tests, covering exact ledger totals, mixed units, date boundaries, pagination, aliases, materials-only access, factory denial and unchanged data; Ruff/compilation passed. Shared size-range and existing model-form contracts, Process QR ordering contract, full frontend lint and strict TypeScript passed. Browser QA and immutable source CI are the remaining promotion checks. Per the user’s speed request, full application build/regressions run once in immutable CI rather than repeating the same full suite locally; no restored-database migration rehearsal is needed because schema/model files are unchanged.
- The complete change is assessed low risk for observation: two presentation/option-list adjustments plus a read-only projection of already-authorized material movements, with no schema/data migration, permission grant, stock mutation or payroll calculation/scan change. Use the standard 10-minute observation, required identity/health/rollback checks and unchanged performance budgets. The known pre-existing Forecasting NULL-BOM defect remains outside scope; retain and report any corresponding log finding explicitly rather than claiming clean logs or silently waiving it.

## Inventory scan reports and Process QR deployed; closing gate remains failed (2026-09-09)

**Requested changes are LIVE. Deployment closing is NOT complete.** The 30-minute health observer passed, but the strict closing runtime log gate failed on a verified pre-existing Forecasting defect. This is not a passing closing record and does not waive or change any gate. The earlier prepared/missing No. 1 repair section is historical and superseded: no production QR repair was performed.

- Active backend/frontend release: `20260909_102118`, green, exact source `2203cc24c5f929bab04e1e5a58b956e6f9ca2a67`. Source worktree `C:/ERP/.codex-work/stocktake-scan-reports-20260909`, branch `codex/stocktake-scan-reports-20260909`; PR [#89](https://github.com/Shmirzaev/Milana-ERP/pull/89), merge `6658d0ac7098b85cd792449b16abcfe10b0eb348`, matching source tree. Deployment documentation is recorded in clean worktree `C:/ERP/.codex-work/record-stocktake-deploy-20260909`, branch `codex/record-stocktake-deploy-20260909`.
- Current production baseline for the next task: release `20260909_102118`, source manifest `a60ecb6c52aa85895075ab18b8e22e6b5c4c3628a62e74a3a65e5c38b3fcaee1`, Git baseline `2203cc24c5f929bab04e1e5a58b956e6f9ca2a67`, schema `0121_stocktake_scan_snapshot`. `deploy/production-base.json` records this actual deployed identity while this context retains the failed closing status. Re-verify fresh remote identity before starting the next change.
- Backend image `ghcr.io/shmirzaev/milana-erp-backend@sha256:33c736d1ea730f089345a81f9c57bc8c4ee994584eccb50d38a946d32d5fdbb5`; frontend image `ghcr.io/shmirzaev/milana-erp-frontend@sha256:02093133eb749d42aba6b2c80aa6e729d773c1af01b370c80618c5c78fd27348`. Archive 2,301,591 bytes / 788 Git-verified files, SHA-256 `2dd6d8373f2e7dc352170ed92a1ce858bce50c39db37ec8ed85e0ccf7c93f446`. Backend activation `2026-09-09T10:47:38.516288+00:00`; frontend `2026-09-09T10:47:39.715127+00:00`. Latest retained backend diagnostic confirms current symlink, both source manifests and green/blue image identities; both backend slots remain running with zero restarts/OOM.
- Inventory folders now retain selected sessions, pending scans and saved latest-first history. Immediate scan feedback shows package/model/color/size/pieces. CSV/totals count unique resolved packages once; unknown/ambiguous entries stay separate. New scan evidence remains frozen after completion; historical evidence explicitly falls back to count-start estimates. No stock adjustment or permission change is introduced.
- Production migration `0120_canonical_bundle_references → 0121_stocktake_scan_snapshot` adds only nullable JSON `warehouse_stocktake_rows.scan_snapshot`; it does not backfill data. Exact-image isolated PostgreSQL 16 rehearsal preserved 92 existing tables / 288,387 rows and original schema/data/sequence hashes; all 25,425 pre-existing scan snapshots remained SQL NULL. Source backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260909_102118.dump`, 52,240,127 bytes, SHA-256 `c5aba694f36cfb3a705c79b5dd5fc75a64636cc0e48f2b5f0b95d259dff713e8`; restore list 83,166 bytes / 1160 objects, SHA-256 `2f1905dc7c16369e26a05b5873c6d8851a215e0a0c38a9442b2a2510c3775d7b`. Existing pre-migration guards checked both files' mode 0600. No production stock/BOM repair or payroll credit was performed.
- The obsolete 312-label repair plan was rejected by its membership guard. Current read-only evidence found 282 labels = 47 operations × 6 sizes, including all 6 No. 1 labels. The release preserves operation identity when raw paid-operation codes collide. Candidate/public UI and signed GET-only API QA passed with zero QA business writes. Chrome blocked the candidate browser CSV download with ERR_BLOCKED_BY_CLIENT; API CSV hashes matched, and real local-fixture downloads/completion immutability were verified separately.
- Retained full local backend run: 845 passed; exact-source GitHub CI: 846 passed. [Immutable CI run 34339764730](https://github.com/Shmirzaev/Milana-ERP/actions/runs/34339764730) passed backend/frontend/release jobs. Candidate paired and immediate public performance passed unchanged 10% median/15% p95/15% payload budgets with 315 samples per endpoint, matching counts and HTTP 200 throughout. First paired attempt failed with URLError before measured samples due bridge loopback; its preserved failure led to a host-network runner correction. Public execution method stayed unchanged.
- Health observer `2026-09-09T10:54:03.245859+00:00` → `2026-09-09T11:24:03.458001+00:00`: 1800.20 seconds, 61 rounds /244 successful health/login probes, zero observer failures. **This health result does not establish successful deployment closing.**
- Strict closing log gate FAILED: green recorded a 500 on `GET /api/forecasting/dashboard`; its traceback reaches `forecasting_dashboard → item_reorder_suggestions → _planned_bom_demand → add_bom`, `backend/app/services/forecasting.py:312`, `int(bom.item_id)` with `None`. The diagnostic has 43 matching error/traceback lines; this is not a count of 43 separate failed requests. The closing runtime report has not passed and the completion generator remains blocked.
- Independently reproduced the exact exception with a valid synthetic Usluga descriptive customer-fabric BOM (`item_id=NULL`) and active production, on BOTH old source 8763f958 and deployed 2203cc24. Forecasting source bytes are identical, SHA-256 `5a5392b84a588db8308cc0e0577fe1ed48a34ef49ed64d20fdc9127139e93011`; its catalog normalization/model schema are also unchanged. Usluga manual fabric rows intentionally have no Milana inventory item. The defect predates this release; it is not caused by the stocktake column or payroll issuance guard. An in-memory-only filter excluding NULL item IDs returned the dashboard and preserved 4 pcs of owned-accessory demand. No source fix was applied.
- No rollback was performed: the retained old code reproduces the same Forecasting crash, so rollback would not fix this defect. Old blue `20260909_082533` remains available while running, but its exact-head startup guard cannot restart against unknown 0121 without an explicitly reviewed compatible rollback procedure; do not bypass that guard. No production BOM mutation, forecasting hotfix, retention cleanup or staging-volume deletion was performed. Keep the strict log failure and pending follow-up visible.

Remaining closing checks completed at `2026-09-09T11:36:38.790748+00:00`. Both source manifests, exact image revisions, symlinks, normal green routing, retained blue/green containers, zero restarts/OOM and four health/login endpoints passed. Both backend slots have exactly two workers, seeding disabled and 8+4 pools. PostgreSQL: 33/100 connections, 0 invalid indexes; revision `0121_stocktake_scan_snapshot`. Backend/frontend root disk use: 71%/60%; no cleanup. Read-only retention checks protected both active/rollback and at least three additional releases. **The sole failed closing check remains the green Forecasting log error; overall closing is not marked passed.**

- `paired-closing-01` passed unchanged limits, retaining all 315 samples per endpoint (per slot for paired): largest median change +0.21%, largest p95 change +1.46%, maximum conservative payload change +0.00%; identical counts and all HTTP 200. No samples were dropped. Public payload changes stayed within budget; response bodies were not retained, so no specific business-field cause is asserted.
- `public-closing-01` passed unchanged limits, retaining all 315 samples per endpoint (per slot for paired): largest median change +0.52%, largest p95 change -0.15%, maximum conservative payload change +0.44%; identical counts and all HTTP 200. No samples were dropped. Public payload changes stayed within budget; response bodies were not retained, so no specific business-field cause is asserted.

Next scoped repair: make Forecasting ignore descriptive BOM rows with no inventory item when calculating inventory reorder demand, preserve linked accessory demand, add the reproduced regression and run the standard reviewed release process. This repair has not been made or deployed.

Evidence remains under `C:/ERP/.codex-work/stocktake-scan-reports-20260909/outputs/deployment`: `evidence.json`, `github-release-run.json`, `github-pr89.json`, `github-backend.log`, `backend-tests.log`, `postgres-staging-verification.json`, `20260909_102118-observation.json`, `closing-first-failure-diagnostic.json`, candidate/public browser/API reports, `qr-current-live-readonly.json`, and all performance attempt directories. Sanitized reproduction: `outputs/closing-blocked-draft/forecasting-reproduction.json` in the implementation worktree. The full backend log stays private and is not embedded here. Screenshots are under `C:/ERP/.codex-work/stocktake-scan-reports-20260909/outputs/screenshots` (candidate/public folders, completed count, Process QR No. 1; local mixed-pack/recovery/mobile/folder reports).

Remaining runtime evidence: `outputs/deployment/closing-runtime-diagnostics.json`. The strict original failure and diagnostic log hashes are retained separately.


## Inventory scan evidence and missing Process QR No. 1 prepared (2026-09-09)

- Worktree `C:/ERP/.codex-work/stocktake-scan-reports-20260909`, branch `codex/stocktake-scan-reports-20260909`, starts from verified main `44006da384fb75270bafeb60d0ce3a8fe9c52dd4`. Both production VMs and the recorded base agree on blue `20260909_082533`, source `8763f95800f1c234ea50c7838201503ea445ec6d`, manifest `8acc7034c07cb2983f7a41b26b507139b3a7de7ac4f334fba3b591c7a2087f39`, database 0120. Deployment is authorized but has not occurred at preparation time.
- Inventory count folders retain selected sessions and pending scans across reopening. Each acknowledged scan shows its package, model/color/size breakdown and pieces. Persisted latest-first history and each folder's CSV show unique scanned packs and pieces; repeated scans do not increase totals. Scan quantities are captured under package/count locks and remain frozen after completion. Migration `0121_stocktake_scan_snapshot` adds only nullable JSON, without backfilling invented historical quantities; legacy count-start quantities are explicitly marked estimates. Unknown/ambiguous labels are separate from pack totals. Undo clears only the relevant scan evidence. No stock adjustment or permission change is introduced.
- Process QR operation tokens now detect ownership collisions in previously issued labels. A read-only audit of model 7522 / PJ1108-5998, manual order MAN-7522-8840-1CPE3HT, found its six Chontak qoyish No. 1 labels missing because reused code SEW-NEW belonged to historical planka tikish. The prepared identity resolves only those six missing labels. Current operation sequence and historical extras have distinct display numbers, including separate same-name operations with different rates. Existing UIDs, payloads, quantities, rates and split identities remain unchanged. Edited labels with unresolvable renamed identities block new issuance with a localized review notice. The backend rejects reusing a UID owned by a different operation.
- Local validation: 845 backend tests passed, Ruff/compilation, final focused QR regressions, full frontend lint, strict TypeScript, runtime recovery/identity contracts and optimized build passed. The first build invocation correctly refused absent API configuration; the configured production build passed. Synthetic browser tests exercised Enter/Tab scans, mixed-model totals, duplicates, undo, folder reopening, real network interruption/retry, independent folder downloads and immutable completed reports in EN/RU/UZ and mobile. No production test scans or inventory records were created. Screenshots and evidence are under this worktree's ignored `outputs/` directory.
- Before promotion, verify a fresh PostgreSQL backup, rehearse 0120→0121 on network-isolated PostgreSQL 16 with unchanged original-table/schema/data hashes, and use exact GitHub immutable images, inactive green slots, signed read-only QA and unchanged performance gates. This is high risk because inventory scan evidence and payroll-label issuance are involved: retain blue throughout a full 30-minute observation and closing checks. The previous running blue code can read the additive schema, but its exact-head startup check will reject restarting at 0121; do not bypass that guard. The user-authorized six-label repair must recheck the exact model/order under locks, add only missing identities via the normal issuance service, preserve old label fields, and create no payroll credit. No repair has yet executed.

## Payroll, department tables and canonical references deployed (2026-09-09)

- Active backend/frontend release is `20260909_082533`, blue, from exact reviewed source `8763f95800f1c234ea50c7838201503ea445ec6d`. PRs #86/#87 merged; main merge `810da612cd3b9586a3bdbe9cbcfec6915e683690` has the same source tree. Immutable GitHub run `34329139521` passed. Both source manifests, image revision labels, slot states and symlinks agree. Archive: 2,284,587 bytes / 784 Git-verified files, SHA-256 `d1ffb2922e6b228d575c5ed3edbdcb27003ce566f7303703b9535cfcfede7286`; source manifest `8acc7034c07cb2983f7a41b26b507139b3a7de7ac4f334fba3b591c7a2087f39`. Backend image `ghcr.io/shmirzaev/milana-erp-backend@sha256:63ba08386c4b890cb0ad77452ba42f0b2111359b75b7d77bfebbbbdc37e18bcc`; frontend image `ghcr.io/shmirzaev/milana-erp-frontend@sha256:a3da54815649a65a89f4fd58739155a1628b032b10c08832a66f01f0ee65934d`.
- The eight requested workflows and follow-ups are active: actual four-digit existing/new order and bundle identities in storage, fresh QR labels and prints; durable historical aliases; BSO-grouped department tables with status colors; deduplicated paid-process search/create with OP codes and 13 sections verified in the old ERP; Process QR totals and automatic beginning/end size ranges saved to Models; numeric descending XJ/PJ search; and Control/Kontrol scan review with explicit confirmation before payroll posting. Confirmation credits the scanned Control operation only. Important UI text supports EN/RU/UZ. Exact-hash print response CSP fixes styling and Print on all four bundle HTML routes without relaxing global policy.
- Final production migration passed `0117_package_quantity_evidence → 0118_paid_process_catalog → 0119_canonical_order_references → 0120_canonical_bundle_references`. It changed 93 PO/USL, 118 PR, 47 PUR and 935 BND references; the Sales table was empty (zero SO changed). There are 2,221 durable aliases and 6,021 deduplicated catalog entries. Verification preserved immutable fields/counts across all 90 pre-existing tables and 3,746 issued payroll QR payloads. Primary/foreign IDs, barcode/scan/label identities, quantities, rates, payroll amounts and audit evidence remain intact. Post-candidate QA verification repeated the same after-snapshot SHA-256 `d107f33964b9e2a471a64047ddc1e56c07d3161ed9a66cbacc32bfd9d8995cd1`; no production test business records, scans or saves were submitted. Legitimate business activity after reopening is retained.
- Final backup after the write barrier and original ASGI shutdown: `/opt/milana-erp/shared/backups/milana_erp_pre_20260909_082533.dump`, 51,476,083 bytes, mode 0600, SHA-256 `2772cf61863895eda012cccc50dfad23639f3024528e04a59a98e7504c5ecf91`; restore list 81,900 bytes, mode 0600, 1,141 objects, SHA-256 `fc40a3601d21a8b3914f1c180981e0e29c0a568ae5c6f8e3811c214bf5fb815c`. The exact new image also passed restored, network-isolated PostgreSQL 16 migration/invariant checks. Host PostgreSQL 18 restore output omitted only `SET transaction_timeout = 0` for PostgreSQL 16 compatibility.
- Previous green release `20260909_053325` remains running **read-only**, with seed disabled and direct Uvicorn startup. Original seed-enabled container `milana-backend-green-prequiesce-20260909_082533` is retained stopped. Old source `ac572aca1c162ff6063fba487f2f323f80e165f0`, manifest `059a2b4bde4a07bf611b5017174d2fe847abe5ffa896987cff7217df1c472137`, image ID `sha256:562116cc8879fb1cc3356f526cda71b06b46b61ddf748af0a2a68b5183315301` were verified. This is not a writable rollback target: old generators/scanners are incompatible with migrated identities, and restarting its image at revision 0120 fails the existing exact-head 0117 startup check. Never bypass that guard or restart the seed-enabled original against 0120. After traffic reopened, restoring the pre-deployment database could discard new work; preserve new activity and use a compatible forward fix or reviewed reconciliation/recovery. No release/image cleanup occurred.
- Validation passed 836 backend tests, Ruff/compilation, frontend lint/strict TypeScript/contracts/optimized build, 11 observer tests, 17 barrier/quiescence tests, and focused print HTTP/browser checks. Signed read-only candidate and public API QA each passed 16 checks; browser QA covered all five department queues, sorted model groups, Process QR stages/catalog/totals, payroll scan UI, canonical bundles and actual styled labels with zero console warnings/errors. Control confirmation and model-size saves were exercised only in isolated fixtures. The QA authentication projection used a read-only transaction; production login mutation was not tested. Existing dependency-audit findings and broader historical security risks remain outside this change.
- Backend/frontend activation occurred at `08:54:45.552450` / `08:54:46.165666` UTC. The initial sequential, unpaced 63-sample inventory p95 failed (+19.71%); investigation found a real alias-search subquery and concurrent QA as a confounder, without attributing the failure conclusively. A declared quiet, alternating/rotated paired check with 315 samples per endpoint/slot passed unchanged budgets (maximum median +4.57%, p95 +2.46%, identical counts/payloads). First public reopen was `08:54:55.684950` UTC; its 63-sample public inventory p95 failed (+21.04%). Business writes were paused again `08:59:10.084324` UTC while health and five benchmark GET routes remained available. The extended public measurement used the original baseline, exact queries, zero pacing and network path, 315 rotated samples per endpoint, no retries/exclusions, and unchanged limits: maximum median +0.55%, inventory p95 −2.26%, identical row counts, inventory payload −0.25% and others unchanged. Payload change cause and initial tail cause are not established by retained evidence. Both failed runs are preserved; no migration, database restore or rollback was performed after activation.
- Public access reopened `2026-09-09T09:06:47.508039+00:00` after the public gate and runtime checks passed. The required high-risk observer ran from `2026-09-09T09:06:52.990032+00:00` through `2026-09-09T09:36:53.215706+00:00` (1800.22 seconds): 61 rounds / 244 successful health/login probes, zero failures or monitoring gaps. Closing runtime, retention and performance checks passed; **observation is complete**. At reopening, all four health/login checks passed, both slots had exactly two backend workers, zero restarts/OOM/log error markers, 28/100 PostgreSQL connections and zero invalid indexes. Backend disk usage was 70%, frontend 60%; no cleanup was performed.
- Closing paired performance used all 315 samples per endpoint/slot and passed unchanged limits: maximum median +4.05%, p95 +3.57%, conservative payload +0.36%. The wrapper initially stopped on an extra constant-payload assertion after all requests completed: inventory response sizes changed in both slots at the same sequences, with exactly matching paired bytes and unchanged row counts/HTTP 200 throughout. A separately reviewed derivation retained every sample and compared maximum candidate payload against minimum baseline payload; no threshold was waived or measurement repeated. Original failed wrapper log and derived evidence remain separate. Response bodies were not captured, so the specific changed business fields are not established. The public closing check uses the same unpaced requests and timing, maximum payload across all samples, original public baseline and unchanged budgets.
- Final public closing performance passed against the original baseline with 315 unpaced samples per endpoint and normal business access open: maximum median +0.65%, every p95 improved, maximum payload +0.42%, identical result counts and HTTP 200 throughout. Closing runtime and retention checks passed at `2026-09-09T09:48:37.570061+00:00`: both manifests/images/symlinks/blue slot states and normal router configuration match, exactly two backend workers per slot, zero restart/OOM/log error markers, PostgreSQL 27/100 connections and zero invalid indexes. Exact retained/replacement container IDs match the original quiescence receipt; the original remains stopped with PID 0 and green database sessions remain read-only. Temporary local QA servers on 3096/3099/18099 were stopped and listeners verified absent.
- Evidence and screenshots: `C:/ERP/.codex-work/payroll-departments-20260909/outputs/deployment/` and `outputs/screenshots/`. Implementation branch `codex/payroll-departments-20260909`; deployment-record branch `codex/record-payroll-deploy-20260909`. The superseded `20260909_080749` candidate was never activated and never migrated production; the final candidate added the reviewed print CSP fix before production changes. Earlier preparation notes below are historical.

## Payroll, department tables and order references prepared (2026-09-09)

- Pre-activation print QA found the existing global CSP blocked inline bundle-label styling and the Print click handler. All four bundle print routes now return a scoped policy containing hashes of their exact generated stylesheet and fixed event-listener script, with embedded Unicode fonts allowed. No global policy relaxation or inline event/style attributes are introduced. Local browser QA verified styled BND-0001 / PO-0202 and clicked Print with zero console warnings/errors; screenshot outputs/screenshots/bundle-print-styled.png. The first candidate was not activated; immutable artifacts are rebuilt for this fix before deployment.
- Bundle-number follow-up: existing and new bundles use actual stored `BND-0001` references. Migration `0120_canonical_bundle_references` follows 0119, preserves valid numbers and unique usable suffixes, and stores permanent old/canonical BND aliases. Barcode values, bundle IDs and linked receipt/movement evidence remain unchanged. Fresh QR images and print documents read the canonical number; old printed bundle references remain resolvable. Read-only production preflight found 935 bundles, all recognized, no duplicate suffixes or values above 9999, and no other persisted bundle-number column. No production mutation occurred. Follow-up validation passed 136 bundle/production/numbering/factory regressions plus 3 alias/handoff integration tests, frontend scanner runtime checks, full lint and strict TypeScript. Local migrated print screenshot: outputs/screenshots/four-digit-bundle-print.png. PostgreSQL staging verification must include 0120 and prevent old-slot business writes between migration and cutover; recheck canonical references immediately before cutover.
- Follow-up: Process QR missing-size entry now uses beginning/end selectors from the same `GARMENT_SIZE_OPTIONS` list as Models (44–68, step 2), automatically previews every included size and saves the range atomically through the existing endpoint. Reversed ranges are prevented. Chrome verified a single size and 48–58 generating/saving six sizes; component lint and full/strict TypeScript passed. Screenshot: `outputs/screenshots/automatic-size-range.png`. Production remains unchanged.
- Eight requested changes are prepared in clean worktree `C:/ERP/.codex-work/payroll-departments-20260909`, branch `codex/payroll-departments-20260909`, after verifying `origin/main`, `deploy/production-base.json` and both active production manifests/slots. Production remains green release `20260909_053325`, with blue `20260909_035800` available for rollback; production database remains at `0117_package_quantity_evidence`. No production deployment, migration, business-data write, permission grant or release cleanup occurred in this task.
- The user's clarification supersedes the initial display-only approach: four-digit references are the actual stored business numbers, including existing orders and newly printed documents/QR labels. Migration `0119_canonical_order_references` deterministically assigns canonical `SO`, `PO`, `USL`, `PR` and `PUR` references, updates their denormalized references and issued MW2/JSON reference fields, and records durable aliases for historical numbers. Existing valid canonical numbers and unique usable suffixes are preserved where possible; collisions/out-of-range historical suffixes receive available four-digit numbers. Allocation is serialized, never wraps, and blocks namespace exhaustion at 9999. Standalone Production Order `order_no` is its actual `PO`/`USL` reference; synthetic `SO` labels are no longer generated for it. Sales-linked production continues to show the linked Sales Order reference. This avoids colliding standalone-production and real-sales identities in Control review.
- Legacy printed labels continue to resolve through aliases and their existing label identities. Database IDs, package/employee/model namespaces, scan UIDs, payroll rates/quantities, raw audit evidence and idempotency request fingerprints remain unchanged. Bundle QR images now render from live canonical order data through an authenticated, non-cached endpoint; printable labels embed freshly generated QR data. Process QR printed labels include the canonical order line. Cutting passport writes use the linked order's live reference; unlinked passport, inventory and purchasing inputs resolve only known aliases and preserve unknown external/manual text. Inventory receipt replay renders canonical references without mutating its stored response/hash or creating stock again.
- Shared department incoming, pending, in-progress and completed queues now use full-width tables grouped by BSOrder, following Cutting's existing layout. Model/variant pictures and numbers, sizes, material/textile, quantities, receipt evidence, deadlines and actions remain available. White, yellow and green distinguish waiting, in-progress and completed rows; tables scroll horizontally on smaller screens. Existing Cutting and replacement-work behavior remains available.
- Paid-operation entry now searches a reusable catalogue by name or code, selects an existing process, and offers an Add paid process action with a generated stable `OP-` code. Identity is deduplicated by normalized name, section and factory. Migration `0118_paid_process_catalog` creates/seeds this catalogue from standard-model operations without changing any source model JSON, model-specific rate or issued QR snapshot. Full normalized names remain searchable as text; a SHA256 identity key bounds the PostgreSQL unique index. Invalid explicit factory ownership is not widened to all factories, and Usluga source models are excluded.
- Read-only Chrome inspection of the old ERP confirmed all 13 process sections: Sewing, Cutting, Packaging, Tikuv, Cleaning, Pressing, Control, Storage, Transfer, Snaps, Buttons, Cord and Sorting. Their source-stage identities are preserved, including separate Sewing and Tikuv sections, with English, Russian and Uzbek labels in the updated paid-operation editors.
- Process QR shows the selected paid operations' total rate per piece at the bottom of the operations table and the quantity-weighted estimated total at the bottom of the generated operation amounts.
- Two-letter model-prefix searches, including `XJ` and `PJ`, order matching model families by descending numeric model number, so `XJ5614` appears before `XJ5415`; existing query matching and family/variant scope remain intact.
- Scanning a Control/Kontrol operation opens a review of issued operations for the exact factory, model, order and size, including scanned/unscanned states, before crediting only the scanned Control operation. Numeric, compact and legacy issued-label paths use server confirmation; ordinary single/bulk payroll writes cannot bypass the guard. Confirmation preserves authoritative rates/quantities, employee/factory scope and duplicate protection, rejects stale label/employee/return snapshots, and blocks voided Control records until the QR is properly returned. Changing employee cancels pending reviews; employee selection is blocked during confirmation.
- Manual Process QR can save missing sizes directly to the selected standard-model variant. The separate setup endpoint requires `payroll.manage` or `modeling.models`, locks the model, accepts 1–40 unique trimmed sizes of up to 32 characters, audits the atomic insert and rejects any variant that already has own size rows. It does not replace existing sizes or change sibling variants. English/Russian/Uzbek entry, validation and late-response isolation are included.
- Canonical-number local validation passed 821 backend tests, frontend lint and strict TypeScript, all frontend contracts, and the optimized 85-page production build. After the final BSOrder capacity guard and expanded migration assertions, 18 focused migration/numbering tests passed. BSOrder remains numeric four digits and rejects exhaustion instead of generating 10000. Browser screenshots canonical-process-qr.png and canonical-bundle-print.png verify migrated PO-0608 payroll labels and the PO-0202 bundle print document with its loaded QR image. Focused canonical-reference checks cover fresh bundle QR/print content, old label lookup, cookie authentication, stale-client writes and immutable inventory retry snapshots. The existing test warning is the upstream Starlette/httpx deprecation. Chrome fixture QA verified catalogue creation/search/deduplication, totals of 850 per piece and 51,000 quantity-weighted, descending XJ model numbers, BSOrder department states, saved model sizes, and Control cancel/confirm/replay (zero cancel writes; one 29,750 record on confirmation). Screenshots are under `outputs/screenshots`; the review checklist is `docs/PAYROLL_DEPARTMENTS_REVIEW_2026-09-09.md`. Browser demonstrations use isolated SQLite fixtures under `outputs/local-qa`. Read-only production checks found zero orphaned references and valid BSOrder links; no production data was changed.
- Before any authorized deployment, apply the complete `0117 -> 0118 -> 0119` sequence to an isolated PostgreSQL staging copy and verify catalogue seeding, canonical uniqueness, durable aliases, denormalized and issued-payload reference rewrites, unchanged IDs/rates/quantities/scan identities/audit/fingerprints, and old/new scan compatibility. Take and verify a production PostgreSQL backup before migrations or material data changes, then follow `DEPLOYMENT.md` with the exact reviewed immutable commit/images, inactive-slot validation, all four health checks, performance gates and a ready rollback release. Neither migration has been applied to production. Broader previously documented security findings remain separate.

## Process QR variant sizes deployed (2026-09-09)

- Active backend/frontend release `20260909_053325` is green, reviewed source `ac572aca1c162ff6063fba487f2f323f80e165f0`. PR #84 merged as `ff8c6074fbb0f5edc7810e150214c04607787684` with the same tree. Blue `20260909_035800` remains running for rollback. Both manifests, slot states, image revision labels, routers and symlinks were verified. Source manifest SHA-256 `059a2b4bde4a07bf611b5017174d2fe847abe5ffa896987cff7217df1c472137`; archive 2,230,449 bytes / 753 files, SHA-256 `cfdf7781e5ae666f8f455e6f0164ac8484a2c413e63b2fffdf7d10b3ed23c4f3`. Immutable-image run `34315548471` passed. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:562116cc8879fb1cc3356f526cda71b06b46b61ddf748af0a2a68b5183315301`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:674891c782895b1ccbf5cc2dd999ddb859ac142d61e902309aa886ef0218e660`.
- Manual Process QR now displays shared model-family sizes for an empty variant when all populated family members agree; variant-specific sizes take precedence. XJ3152 variants 5412, 5413, V-5410 and V-5411 resolve the existing 48/50/52/54/56/58 sizes from 5416. Conflicting or missing families require explicit configuration. Loading, retry, stale-response handling and shared-size explanations support EN/RU/UZ. Ordinary ERP-order sizes and editable model-detail rows remain unchanged.
- No model, size, payroll, employee, stock, order, permission or other business data was created or changed by deployment/QA. Existing QR labels, issuance validation, payroll calculations and snapshot semantics are unchanged. Database head is `0117_package_quantity_evidence`; migration check was a no-op. Backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260909_053325.dump`: 51,455,674 bytes, 0600, 1141 restore objects, SHA-256 `5e9506e591ca623550a467b8570733e01eab1a8d3f657089cba5210a67bd2767`. Restore list SHA-256 `72d20fca0e80b5e5d8729246fb2eccf8723040c45837a674700f1373f63ce358`.
- Validation passed 748 backend tests, full Ruff/compilation, frontend lint/strict TypeScript, build contracts and optimized 85-page build. Focused tests exercise inherited-size label issuance, numeric scanning at the correct factory/rate, and unchanged issued snapshots after catalog changes. Actual-page browser fixtures cover own/inherited/conflicting/missing sizes, late responses, retry and EN/RU/UZ phone layouts. Signed candidate and public QA under the Payroll role verified all five XJ3152 variants, real PostgreSQL resolution, and localized editable size inputs, with zero browser errors or writes. Screenshots were inspected.
- High-risk observation started `2026-09-09T05:51:09.116624+00:00` and ran 1800.14 seconds with no failures (244 endpoint probes). Closing runtime, logs, database headroom/invalid-index and paired/public performance checks passed. Performance used 21 samples x 3 rounds per endpoint, unchanged 10% median / 15% p95 / 15% payload budgets and identical result counts. Prior release stayed live throughout.
- Two initial candidate browser attempts timed out on the generic network-idle condition. Read-only diagnosis found Next.js RSC page-prefetch abort/retry activity while Process QR and its API data were loaded. QA was changed to await DOMContentLoaded and assert the actual editable size controls and API responses. Both failed reports were retained; subsequent candidate/public checks passed without application changes.
- Worktree `C:/ERP/.codex-work/variant-sizes-20260909`; implementation branch `codex/variant-sizes-20260909`; deployment record branch `codex/record-variant-sizes-deploy-20260909`. Evidence is under `outputs/deployment` and `outputs/size-qa`. Legacy checkout preserved. No release cleanup occurred. The previously documented Forecasting exception remains separate and unchanged.

## Process QR variant size fallback prepared (2026-09-09)

- The XJ3152 family had configured sizes only on variant 5416 (model 6921); 5412 (6919), 5413 (6920), V-5410 (27), and V-5411 (28) had empty size rows. Manual Process QR previously read only the selected variant, so those variants could not display size quantity inputs.
- Read-only `/api/models/{mid}/process-qr-sizes` now resolves a variant's own sizes first, otherwise the exact catalog family when its populated members agree. Missing or conflicting families return no guessed sizes. It uses the indexed PostgreSQL family identity, excludes migration-only donors, and retains catalog scope. Model-detail/edit rows, existing labels, stock, and payroll calculations are unchanged; no data backfill or migration is required.
- Manual Process QR requests the fallback only for an empty variant, preserves the selected variant identity and paid operations, guards against stale responses, refreshes size results, and explains inherited/missing/conflicting/error states in EN/RU/UZ. Ordinary ERP-order size sourcing is unchanged.
- Validation before push: 42 focused backend tests passed, including inherited-size label issuance, numeric payroll scanning with the correct factory/rate, and preservation of issued snapshots after catalog changes. Frontend lint, strict TypeScript, existing build contracts and the optimized 85-page build passed. Isolated actual-page Chrome checks passed inherited/own/conflicting/missing sizes, late-response isolation, retry, no writes on selection and EN/RU/UZ phone layouts. Full backend and release validation are recorded in deployment evidence.
- Implementation worktree `C:/ERP/.codex-work/variant-sizes-20260909`, branch `codex/variant-sizes-20260909`, from verified `origin/main` `5def64f47184584a12453fd2b3cee36648358908`. Production remains `20260909_035800` until the deployment record below is updated; blue is active, green `20260908_121327` is rollback. Both active manifests and `deploy/production-base.json` were verified before edits. Browser fixture evidence is under `outputs/size-qa`; release evidence is under `outputs/deployment`.

## Payroll employee search and date-filter translations deployed (2026-09-09)

- Active backend/frontend release `20260909_035800` is blue, from reviewed source `ac0f0658402837ee974383651d160956b52e3b4f`. PR #82 merged as `b5298b4aa28dd40792a27b5d27b33dd097e6890f`; its application tree exactly matches the reviewed source. Green `20260908_121327` remains live for rollback. Both source manifests, slot states, image revision labels, routers and symlinks were verified. Manifest SHA-256 `b8065810c7ae66f02143a0096b932ec09422507800a408b03c4b152b336c1cb6`; source archive 2,220,488 bytes / 751 files, SHA-256 `3e4ddcf8161c1e51338cd666b167103ba7b59128ca90e7bbe33c0e8edcca6ded`. GitHub immutable-image run `34309132872` passed. Backend `ghcr.io/shmirzaev/milana-erp-backend@sha256:6430759a767b32ab18a42a5b3e0e9b651997b6f017ac9891966620b483500769`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:1bfedc67748acba4ce2b844dc4de92f0dfa3d5d47adb15fa04d12d96110d98c1`.
- Payroll Scan supports typing an employee name or number and selecting by keyboard or mouse. Results show number, department and position; selection returns focus to QR scanning. Pending save completions preserve name-entry focus. The read-only search endpoint requires existing payroll scan/manage permission, is restricted to active employees in the login factory, caps results at 20, and ignores stale UI responses. Existing employee QR/number entry and numeric-work autosave remain available. Payroll Summary uses date-specific filter labels: Uzbek `Boshlanish sanasi / Tugash sanasi`, English `Start date / End date`, Russian `Дата начала / Дата окончания`. The person-oriented shared translations remain unchanged for other contexts.
- No payroll, employee, stock, order, permission or other business data was created or changed by this deployment or QA. The schema check was a no-op at `0117_package_quantity_evidence`. Backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260909_035800.dump`: 51,437,605 bytes, 0600, 1141 restore objects, SHA-256 `9cfb3d123942b85316a704657cacf9fbb9368e0d7fa8a756eef7ad1fdeeab64c`. Restore list: 81,900 bytes, 0600, SHA-256 `06e94cf9fa094e516cbae1f2fc079f3d6bc80e8c2f719dffc430d6b049aa5469`.
- Validation passed 739 backend tests, 11 observer tests, full Ruff/compilation, frontend lint/strict TypeScript, build contracts and the optimized 85-page build. Independent review found no blockers. Isolated actual-page browser fixtures cover keyboard/mouse selection, chosen-employee numeric autosave, no payroll write on selection, number entry, late-save focus, stale requests, repeated queries, errors, permission visibility and phone layouts. Signed candidate and public browser QA passed all three date-label languages, existing employee name search/selection and scanner focus with zero console errors and zero upstream writes. Desktop/mobile screenshots were inspected. Temporary QA proxies were stopped.
- High-risk scan-workflow observation ran from `2026-09-09T04:19:34.555994+00:00` for 1800.19 seconds with zero failures; 244 endpoint probes were recorded. Closing runtime, database headroom/invalid-index checks, paired and public performance comparisons passed; the previous slot stayed live. Pre-cutover paired and immediate public comparisons used 315 samples per endpoint, with closing checks using the documented 21 samples x 3 rounds. All comparisons retained the 10% median / 15% p95 / 15% payload limits and identical business result counts.
- An existing Forecasting exception was observed on old green release `20260908_121327` at `2026-09-09T04:03:36Z`, before staging: `app/services/forecasting.py:312` calls `int(bom.item_id)` for a missing material reference. Forecasting code is unchanged in this release. The initial candidate log check included this earlier event; it was preserved and investigated, then candidate/postflight logs were checked from the candidate start and activation timestamps. This is a separate unresolved issue. A premature local public-QA attempt before its preview listener started also remains in evidence; the subsequent ready-listener run passed.
- Worktree `C:/ERP/.codex-work/payroll-employee-search-20260909`; implementation branch `codex/payroll-employee-search-20260909`; completion record branch `codex/record-payroll-search-deploy-20260909`. Evidence is in ignored `outputs/deployment`. The legacy checkout and prior authorized Sales-order deletion record were preserved. No release cleanup was performed; backend disk was 70% before staging and remained below the 80% staging stop limit.
...
- Active backend/frontend release `20260908_121327` is green, from reviewed source `974eb7a31d8be2c09266f3f7179da45843cd8aa0`. Blue `20260908_080847` remains running for rollback. Both source manifests, symlinks, slot states, image IDs and revision labels agree. Archive: 2,213,957 bytes / 748 Git-verified files, SHA-256 `016c4358d214f55f7c49edbf34860528ce5ee8cd6f5cda8da048a9f37bb7b616`; source manifest `2b8140175727b6cf437f88a08d57b413fd9ac33ad8f573a1424db26815947f38`. Backend image `ghcr.io/shmirzaev/milana-erp-backend@sha256:2304b79350ca6a4a13546e93025ed986e1fd416ea0b658ada57a6f9aa670365b`; frontend image `ghcr.io/shmirzaev/milana-erp-frontend@sha256:3cb1a811558fb7c0cb73d20e91e8b4eaedf9ba1b08c4a3ddcefc5d95f131750e`. Immutable release run `34224884539` passed. PR #78 implemented the workflows; PR #79 contains the one-class mobile correction and the already-applied schema record; PR #80 contains the scoped print-response CSP fix included in the final rebuilt source.
- Ready storage can record actual physical packs using an approved model/variant, weight per pack, size quantities, number of identical packs, and receipt reason. Dedicated immutable receipt evidence backs the package/items/stock graph; no production order or legacy provenance is fabricated. Creation is permission checked, audited and idempotent. Uncertain browser requests retain both their payload and request key per user, including reload recovery; changing values cannot silently create another receipt. Individual/selected labels and saved print runs can be reprinted without stock changes.
- Packaging creation and its exact print-run membership are committed atomically against recorded packaging output. Six labels followed by four labels for the same order remain two immutable groups. Scanning any member's package QR or unique alias receives exactly its group once, with package/stock locks and atomic validation. Individual receipt cannot strand a group. Reprinting a received group reflects reviewed current quantities while retaining original receipt/membership evidence; pending groups reject changed contents.
- Inventory count keeps the scanner ready after ordinary clicks, filters and search, supports Enter/Tab, preserves the first character after focus loss, and pauses during confirmation dialogs. Existing queued saves, duplicate handling, failed-scan recovery and non-adjusting count semantics remain.
- Ready-stock Sales now asks for physical pack count rather than piece/full/partial arithmetic. The backend reserves exactly that many intact received packs across their real size rows; heterogeneous pack sizes remain valid. The requested count is persisted and scanned quantities become authoritative at dispatch. Ordinary production sales and older piece-based orders retain their behavior. New manual packs are also covered by whole-pack reserve/release guards, including rejection of post-dispatch release that would recreate available stock.
- Shipment preparation uses compact scan/review controls and expandable detail. Warehouse can review piece quantities and the invoice amount with reasons. Reductions, zero-size removal and confirmed increases update package contents, stock and own-order reservations together. Increases additionally require receipt permission and explicit confirmation, and create immutable physical count/extra-receipt evidence. Capacity changes and shortages are recorded; original packaging allocations remain consumed, so shortages cannot authorize new packages. Stale financial review blocks dispatch. The immutable dispatch document supports read-only invoice printing in EN/RU/UZ. Delivery posts/reconciles the frozen total to one unpaid invoice, preserves matching prepaid invoices, and rejects unsafe paid mismatches or mixed/incomplete invoice evidence. Generic shipment PATCH cannot bypass dispatch or overwrite snapshots.
- Invoice printing follows the supplied Milana Tex / Ombor hisob-fakturasi workbook: A4 portrait, transport header, model/variant/description/sizes, packs/pieces/weights/prices/amounts, blue headings and yellow/green totals. Optional driver/vehicle/carrier/contact fields are editable only before dispatch and frozen with model names, measured weights and the reviewed amount. Mixed-model/price package rows share pack and weight cells without duplicate totals; unknown weights stay unknown. Financial adjustments remain explicit. Shipment creation locks and refreshes the Sales Order before checking replay/existing shipments. A shared response helper scopes the four warehouse print routes to `style-src 'unsafe-inline'`, `script-src 'unsafe-hashes'` with only the exact SHA-256 hash of `window.print()`, and `script-src-elem 'none'`; other API routes retain their existing CSP.
- Final pre-cutover backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260908_121327.dump`: 51,302,488 bytes, mode 0600, 1141 restore objects, SHA-256 `9d15734190e454de022cd630bf44815c69b9b88f84757a8899a131f9912ed5e5`. Restore list: 81,900 bytes, mode 0600, SHA-256 `80c77846249a4723ebc6e82ad812cdd822c78b45959c30ced9887810d35b02a3`. Database revision is `0117_package_quantity_evidence`; the final candidate's upgrade was a no-op because the three additive migrations had already passed during the first staging attempt. The original pre-migration backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260908_112728.dump` retains revision 0114: 51,274,812 bytes, 1090 objects, SHA-256 `23b069891ec54ae440a39af01df465e70ab5cd60f1708ac19fdde00ca83f2945`. No production business records were created by deployment QA.
- Local pre-build validation passed 737 backend tests in 209.79 seconds (one inherited Starlette/httpx deprecation warning), Ruff/compilation, 11 observer tests, frontend lint/strict TypeScript/contracts/optimized builds and isolated scanner/Sales/shipment/transport/print browser tests. Separate disposable PostgreSQL migration/concurrency/immutable-evidence checks passed using the final backend image digest. Signed read-only candidate and public browser QA passed available workflows on desktop and mobile, including an existing active stocktake's scanner focus; no production scan, receipt, shipment or form save was submitted. Local actual-middleware HTTP fixtures and browser checks passed 14/14 cases covering all four print routes, direct navigation and blob printing, EN/RU/UZ invoices, and positive/negative CSP checks. Report `C:\ERP\.codex-work\ready-storage-workflows-20260908\outputs\deployment\print-http-qa\2026-09-08T12-12-44-595Z\report.json` records source hashes verified against the reviewed source (accounting for CRLF/LF differences); the promotion record binds that source evidence to the final commit and backend image, without claiming the pre-build browser tests ran inside that image. The supplied layout, measured weights and adjusted totals were verified using isolated fixtures. No production package-label GET was used because the older QR helper can lazily persist a missing file/reference. The browser preview preserved upstream response/security headers. Its /me projection used a read-only transaction and other API reads used signed authorization; login mutation was not tested.
- High-risk observation ran from `2026-09-08T12:37:15.481349+00:00` through `2026-09-08T13:07:15.659689+00:00` (1800.17 seconds): 61 rounds / 244 successful health/login probes, zero failures or gaps. Closing source/slot/router/HAProxy checks passed, exactly two backend workers per slot, zero restarts/OOM/error markers, PostgreSQL headroom and zero invalid indexes. Closing paced paired performance (maximum median +0.12%, p95 +4.28%) and public performance (maximum median +3.47%, p95 +3.69%) passed the unchanged median/p95/payload budgets with identical result counts; each used 315 requests per endpoint (per slot for paired comparisons).
- The first candidate `20260908_112728` / `20d116910c8ebee559deb123fbe246dd4a8e15bc` passed backend/PostgreSQL/performance checks but was never activated. Actual-data mobile QA found an inventory-search form overflow; a 12rem minimum on the outer form fixed wrapping at 390px/320px. The second candidate `20260908_114527` / `31ec11e9623a0174dd6ee1ad7bd112a1203dfdb4` passed signed desktop/mobile, isolated PostgreSQL and paired performance gates but was also never activated: final print-header review found that the global API CSP blocked inline print styles and the Print button on direct HTML responses. Earlier browser fixtures omitted real middleware headers and the preview replaced upstream CSP, so those checks missed the defect. The preview now preserves upstream headers, and final HTTP fixtures cover direct and blob printing. Both candidates and failed QA evidence remain under `outputs/deployment/first-candidate-20260908_112728`, `outputs/deployment/second-candidate-20260908_114527` and timestamped reports. Earlier harness localStorage initialization and client-close corrections remain recorded separately.
- Worktree `C:/ERP/.codex-work/ready-storage-workflows-20260908`; implementation branch `codex/ready-storage-workflows-20260908`; completion-record branch `codex/record-ready-storage-deploy-20260908`. Reviewed source is merged to main. Final release evidence is under ignored `outputs/deployment`; first-candidate and failed QA evidence is retained. The legacy checkout and unrelated work were preserved. No release cleanup or permission grant was performed; broader pre-existing audit findings remain outside this change.

## Model-family approval and remaining variant approval (2026-09-08)

- User explicitly requested approval of all remaining variants and approval only for new models. Exactly 16 standard draft variants were approved through the existing authenticated API before changing its behavior: IDs 7989, 7993, 7994, 7995, 8002, 8004, 8005, 8006, 8007, 8013, 8016, 8017, 8038, 8039, 8040 and 8041. A complete pre/post model-row comparison confirmed only their status, approver, approval time and update timestamp changed; all other existing model rows, including draft base rows, were unchanged. There were zero remaining unapproved variants across standard and Usluga catalogs. No production order or stock was created.
- Verified pre-data backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260908_080034.dump`: 51,114,238 bytes, mode 0600, 1090 restore objects, SHA-256 `770be3f7dc0830d8b1b81959dff0334be3dda454240e791e21583ec87a8ac0a6`; restore-list SHA-256 `a4e7849c53b81466223f3b66d8585eaae37acaa477fc33b73fc5cf424cf9206d`. Evidence under `C:\ERP\.codex-work\model-only-approval-20260908\outputs\deployment`.
- Approval is now defined at model-family level within each catalog scope. Creating a variant inherits existing family approval, including legacy families whose approved variants coexist with a draft base. An entirely new family and its variants stay Draft until an authorized user approves the model once; that action approves its pending base/variants atomically and audits each changed row. Cloning into a new model remains Draft. Existing approval actor/time is inherited rather than attributing approval to the variant creator. PostgreSQL family locks serialize variant creation with approval; the family lookup uses the existing indexed generated key.
- The grouped Models page reports family approval and only shows its approval action to authorized users. English/Russian/Uzbek production guidance now refers to model approval. Standard approval still requires `modeling.approve`; Usluga still requires `usluga.manage`, Eco scope, and exactly one main fabric for every pending family member. No permission grant or schema migration is introduced.
- Local validation passed 615 backend tests, Ruff/compilation, 11 observer tests, frontend lint, strict TypeScript, all build contracts and the optimized 85-route build. Four new backend cases cover family approval, inherited legacy approval, production/Cutting creation, base/new-model/clone draft behavior, permission and catalog isolation, idempotency, and atomic Usluga validation. The frontend component contract covers approval visibility for both catalog permissions and approved/draft families. The existing Starlette/httpx warning remains.

- Active backend/frontend release is `20260908_080847` in blue from reviewed source `3ca835e4c31418982f185279938e1f7f60e0c49e`; green `20260908_065525` remains running for rollback. Both current symlinks, slot states, source manifests, image revision labels and image IDs were verified. Source archive contains 714 Git-verified files, 2,151,473 bytes, SHA-256 `80c79957179a8451831f038e18379be690cbd3c260c6eea239cd7491dd05f5ab`; source-manifest SHA-256 `7f155e13f214f94044f2beb439641a572cea384fac4e309e6981672ac4a5a2a8`. Backend image `ghcr.io/shmirzaev/milana-erp-backend@sha256:54243940804abe5ae49c26618020a7d6b0d4d220dabc12b59d4ce6481d77725f`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:82dacd46886a8ab2213ffa10c9b94cf70e93cc351e83141cba493ecf28c2acfd`. GitHub immutable release run `34202934623` passed.
- Verified pre-deployment backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260908_080847.dump`: 51,116,563 bytes, mode 0600, 1090 restore objects, SHA-256 `f3735bc90891ea35674d72de47d9828ec12f3a1335872ed0fca5d3056b140511`; restore-list SHA-256 `ef09b145b3fca75e1bcb7b1845ed7d403adee6c34b6874dfd58d4dffb53a7306`. Alembic was a no-op at `0114_warehouse_stocktake`. The separate pre-data backup and exact 16-row approval comparison remain recorded above; approval audit IDs are 18015 through 18030.
- High-risk observation ran from `2026-09-08T08:36:35.867447+00:00` through `2026-09-08T09:06:36.099688+00:00` (1800.23 seconds): 61 rounds / 244 successful probes, zero failures or gaps. Closing runtime checks passed: four health/login endpoints at HTTP 200, valid routers/HAProxy, two backend workers, zero restarts/OOM/error markers, database headroom and zero invalid indexes. Backend/frontend disk usage was 70%/57%; no release cleanup was performed. Closing read-only PostgreSQL QA reconfirmed inherited approval, family-lock serialization, all 16 approved targets, and no blocked variants in approved families. Public browser QA confirmed PM7015's five selectable variants and the absence of an approval button for approved families; candidate QA also covered PJ1236's three variants. Both had zero browser errors and no production form submission. Closing paced paired tests used 315 requests per endpoint per slot and passed unchanged budgets (maximum median +1.63%, p95 +3.98%). The closing 315-request public comparison also passed (maximum median +1.23%, p95 +3.69%), with identical result counts and bounded payload sizes.
- Timing caveat: initial 63-request public comparisons failed global-search p95 (+22.56%) and then inventory-stock p95 (+21.67%). Traffic was rolled back to green at 08:26 UTC, after its first activation at 08:22 UTC. Diagnostics found unchanged medians, similar old/new local search latency, clean runtime and a recovered public p95 of 157.84 ms versus original baseline 160.68 ms. A predeclared rotating 315-request comparison then passed every unchanged limit (maximum median +1.12%, p95 +5.03%, identical counts/bytes); its candidate samples also pass against the original baseline (maximum median +0.32%, p95 +4.74%). Both roles were reactivated at 08:31 UTC. After observation, a short paired closing sample failed inventory-batch p95 (+61.13%, median +2.93%); the slow burst affected both slots. Runtime inventory route/service and dependency-file hashes match rollback. An unpaced expanded local attempt hit HTTP 429 and aborted; its per-request samples were not captured, but the failure/log evidence is retained. The final expanded paired check adds 50 ms between requests outside measured latency to respect the existing shared rate limit. No application rate limits, performance budgets, source code or dependencies were changed during diagnosis. All captured samples and failed comparisons are retained; the precise cause of the brief tail-latency bursts was not established.
- Source branch `codex/model-only-approval-20260908` was pushed and merged through PR #76 as `45735cfadcdba228d2055812f357423c6ed802ed`. Implementation commit `93b9afefac4924bfcdccd9f7efa9e93be88039ce` was reconciled with the documentation-only warehouse import record from PR #75, producing reviewed release commit `3ca835e4c31418982f185279938e1f7f60e0c49e`; no other application change was introduced. Completion record branch is `codex/record-model-approval-deploy-20260908`. Worktree `C:/ERP/.codex-work/model-only-approval-20260908`; evidence under its ignored `outputs/deployment` directory. The legacy checkout and unrelated work were preserved. Existing broader audit/security findings remain outside this change.

## `...2333.zip` old-ERP sticker packs imported (2026-09-08)

- The supplied ZIP `C:\Users\User\Downloads\Telegram Desktop\...2333.zip` (8,140,365 bytes; SHA-256 `46f2c76009b538c3af8be5e3eb4026b781d3508244d0735ff011c74b186f80a0`) contained 53 unique PDFs / 100 pages. The final reviewed batch has 258 unique pack/QR records, 15,816 pieces, 45 model/variant identities, all size lists present, and 6,754.98 kg of known weight. The one weight absent from its sticker, `uzerp_ii_21127_3` / KJ13030 / V-5882 / 60 pieces, remains null as the user permitted. Three pages were genuinely blank. One exceptional page held two labels, both visually verified: `1504258` (XJ3062 / V-5890 / 6 pieces / 2.60 kg / sizes 44-54) and `1504259` (XJ3062 / V-5859 / 12 pieces / 5.33 kg / sizes 58-66).
- Every one of the 258 QR symbols decoded from rendered source PDFs; four required a 400-dpi retry. The source had zero repeated QR values. The final production preflight found zero existing receipt, package barcode, package number or QR-alias collisions. Immutable manifest SHA-256 is `d25eb867176df72e5af193cb5552c3e092ded19070cae232cd8b32a50e26501c`; import bundle SHA-256 is `6c30f4e0364becc5a6713d66d3086aa331dfb97064f5e5b01830ffa7963c03b9` (8,146,412 bytes). All 53 source PDFs and each record's exact sizes remain bound into the immutable local evidence.
- Final live catalog resolution linked 257 packs to exactly one approved catalog identity. The only absent identity, XJ3062 / V-5890, was imported under internal warehouse-only model `LEGACY-STICKER-E16D84A347A65CD6`; it remains excluded from Models/Variants as requested. There were no ambiguous identities or identifier collisions. Production apply was one transaction and created exactly 258 receipts, packages, package items, stock rows, QR aliases and scan rows. Audit record `17213`, entry hash `d5c1f2e028c6425343940b30235f1c1aaf7f80d1f6770113e7544c2b4b4cce00`.
- Immediately before/after the isolated import, Finished Goods warehouse totals moved from 4,633 to 4,891 packages and 345,259 to 361,075 pieces; available pieces moved from 345,001 to 360,817. Reserved remained 258, sold remained zero and balance failures remained zero. Later independent warehouse work can change the moving global dashboard, so closing validation uses the exact receipt-backed 258-pack scope. Repeated graph validation still found all 258 rows and 15,816 pieces, and all 258 public old-ERP QR lookups returned HTTP 200 with the correct package and quantity.
- Verified pre-change PostgreSQL backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260908_113045.dump` contains 50,725,236 bytes and 1,090 restore objects; dump SHA-256 `efeedb5aa0e55ce0f18383a0a141e3c0636ddd61d9f9ac7e57361ed11dfa4ceb`, restore-list SHA-256 `facc26c0c07b9babafe7b00aebac9b4798f7a4821890f7fef958e1a82c594875`. Schema remains `0114_warehouse_stocktake`.
- Signed-in production QA found XJ3062 / V-5890 as one 6-piece warehouse-only package. Catalog-linked KJ13031 / V-5811 stock rendered its existing full 230x320 model thumbnail and aggregated 21 packages / 630 pieces. Browser logs had zero warnings/errors. No model, order, shipment, permission or other business data was changed by this import outside the one hidden identity and the 258 complete package graphs.
- The complete high-risk import observation ran from `2026-09-08T06:38:32.970635+00:00` through `2026-09-08T07:08:33.131333+00:00`: 61 rounds / 244 successful internal/public probes, zero failures or monitoring gaps. At the closing boundary a separate authorized variant-fix task had just switched the application at `2026-09-08T07:07:27+00:00`; the strict old-release guard stopped rather than assuming continuity. After reconciliation, closing checks passed against the new active green release `20260908_065525`, source commit `60b6935a591160c1cc724ff04b9240f943129701`, manifest `4978e2c8cfed6f837a3d194502dfaa34db51acd3f9b133abd1c39d93ca2ed263`, with blue `20260908_051819` as rollback. Both current symlinks and slot states agreed, both active/rollback manifests validated, routers/HAProxy were valid, exactly two backend workers were running, all containers had zero restarts/OOM and clean logs, PostgreSQL used 33/100 connections with zero invalid indexes, and all four endpoints returned 200. The separate release subsequently completed its own 30-minute observation and closing checks before its production baseline was merged.
- This data-only operation itself did not switch an application release. The importer was updated only for the already-active `0114` schema, validated by Ruff, compilation, five focused tests and all 611 backend tests, then merged through PR `#72`: source commit `c884f457`, merge `10c37928dbee67c50cf1c5281361360fb43259e2`.
- Worktree `C:\ERP\.codex-work\pack-2333-production-20260908`; implementation branch `codex/pack-2333-production-20260908`; completion-record branch `codex/record-pack-2333-production-20260908`. Private immutable import and verification evidence remains under the worktree's ignored `outputs\pack-2333-production-20260908\import` directory. Exact task-owned server/container temporary files were removed after closing checks; the verified backup remains. The dirty legacy checkout and unrelated concurrent work were preserved.

## PJ1236 variant visibility and V-6120 approval (2026-09-08)

- The reported two-versus-three discrepancy was caused by PJ1236 variant V-6120 (`models.id = 8048`) being Draft while V-6121 and V-6123 were Approved. The branded-stock selector filtered unapproved variants out, and the backend correctly required approval for branded-stock production.
- The user explicitly requested approval of V-6120. The normal authenticated `POST /api/models/8048/approve` action succeeded with audit record `17407`. Only its status, approval actor/time and update timestamp changed; exact pre/post comparisons confirmed the PJ1236 base and sibling rows were unchanged. No production order, model, stock, package, shipment or other business record was created. Live signed-in QA immediately confirmed all three variants visible and V-6120 selectable with its own photo, brand and model details.
- The scoped UI change shows every variant in the selected eligible model family; unapproved variants display an English/Russian/Uzbek approval explanation and remain disabled for mouse and Enter selection. Family eligibility, backend approval rules and permissions remain unchanged.
- Implementation commit `60b6935a591160c1cc724ff04b9240f943129701` was pushed from `C:\ERP\.codex-work\fix-branded-variants-20260908` on `codex/fix-branded-variants-20260908`; PR #73 merged as `795b5cbe13df7059833c6d8acdb33772fa861ba2`. Local validation passed 611 backend tests, Ruff/compilation, 11 observer tests, full frontend lint, strict TypeScript, all build contracts and the optimized 85-route production build. The existing Starlette/httpx warning remains.

- Active backend/frontend release is `20260908_065525` in green, with blue `20260908_051819` retained and running for rollback. Both symlinks, slot states, 713-file manifests and image revision labels were verified. Source-manifest SHA-256 `4978e2c8cfed6f837a3d194502dfaa34db51acd3f9b133abd1c39d93ca2ed263`; archive SHA-256 `9aef0e5f3cb86e184635f568053c22c4cc9e2a5331136ac145e8d53212108f2c` (2,144,563 bytes). Backend image `ghcr.io/shmirzaev/milana-erp-backend@sha256:6f271b4f1ec14d66066187d358994cd70fa4ca9408d7539117de094db18991de`; frontend image `ghcr.io/shmirzaev/milana-erp-frontend@sha256:d45bd9c5d7aa5b617e7bfbdf9900b4f9b347a368eee5da7aecfb7242c712622a`. GitHub release run `34196846748` passed.
- Verified backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260908_065525.dump`: 50,960,696 bytes, 1090 restore objects, mode `0600`; dump SHA-256 `83d209721651a85f418c38a964d262cfba9e914b1ceb15585b1fb147d03cefc6` and restore-list SHA-256 `83c374edccd4d49a2a6e4f13f8f53f784eb21a428af2e3a12b8041a545331f7d`. Migration was a no-op at `0114_warehouse_stocktake`.
- Candidate and immediate public performance gates passed with identical response counts and payload sizes. Candidate maximum median increase was 6.88% and p95 increase 10.35%; immediate public maximum median increase was 0.06% and p95 increase 1.34%, within unchanged budgets. Signed-in candidate/public QA covered PJ1236 selection, a mixed-status PM7015 family, disabled draft mouse/Enter behavior, approved keyboard selection, model switching, and zero browser errors. English/Russian/Uzbek runtime messages passed the component contract. No production form was submitted. The temporary loopback QA proxy was stopped.
- Immediate runtime checks found both roles and rollback running with zero restarts/OOM/error markers, exactly two backend workers, valid HAProxy routers, all four health/login checks at HTTP 200, database headroom and zero invalid indexes. Backend/frontend disk usage was 71%/56%; backend is above the 70% alert level but below the 80% staging stop. No cleanup was performed. Historical security/audit findings remain outside this fix.
- High-risk observation completed from `2026-09-08T07:08:32.758134+00:00` through `2026-09-08T07:38:33.009807+00:00` (1800.25 seconds): 61 rounds / 244 successful health/login probes, zero failures or gaps. Closing runtime checks again passed with both slots retained, valid manifests/routers, two backend workers, zero restarts/OOM/error markers, database headroom and zero invalid indexes. All three PJ1236 variants remained approved. Evidence is under `C:\ERP\.codex-work\fix-branded-variants-20260908\outputs\deployment`. Completion-record branch is `codex/record-branded-variants-deploy-20260908`.
- Closing timing caveat: the first sequential comparison failed model-group p95 (+33.55%, 99.15/74.24 ms); a diagnostic-backed sequential repeat cleared that endpoint but failed Inventory batch median/p95 (+11.67%/+21.59%) and stock median/p95 (+10.25%/+17.70%). Both failures remain in evidence. Runtime comparison proved all 363 backend files identical except one test file, identical package versions/environment/resource limits, and identical first eight image layers; only source-copy/chown layers differed. A balanced comparison alternating 63 requests per endpoint per slot passed every original threshold with identical result counts and payload bytes (maximum p95 increase 0.10%, all medians improved). Model-group p95 was 69.25 ms active versus 69.63 ms rollback. No application regression was established by the inconsistent sequential timings; this measurement variability remains documented.
- Concurrent catalog activity added XJ3062-V-6124 at 07:12 UTC and PJ1149-V-6122 at 07:21 UTC, changing the benchmark's automatic newest-model query. The final public comparison restored the original PJ1058-V-6126 search, with every raw timing retained, and passed unchanged budgets: maximum median +0.43%, p95 +0.20%, identical result counts, unchanged catalog payload sizes and Inventory batch payload growth of 0.58% from concurrent data activity. No thresholds were relaxed and no failed samples were discarded.



## Warehouse inventory counts and Packaging Reports deployed (2026-09-08)

- Both user-authorized features are live in blue release `20260908_051819` on backend and frontend. Reviewed immutable source commit is `562fac2c52d3298da9bdc1ae73a6e4b422bce366`, merged through PRs `#69` and `#70` (final merge `70102192bd67df118e68e1bd01b55198ab0d3e2e`). Both current symlinks, active slot states, source manifests and image labels agree. Green `20260907_104027` remains running for rollback. The unactivated first candidate `20260908_050013` was superseded after browser QA identified a missing runtime menu translation.
- All 713 source files were verified byte-for-byte against Git. Source-manifest SHA-256 `f5204b195c245bf242834edc2c32986be53f1a858d76384c588674735d484b0b`; archive SHA-256 `ea0be91d98fdf69d6ac9758bed290a17da87ed33b652d1b143bd122e3afce5df` (2,141,214 bytes). Backend image `ghcr.io/shmirzaev/milana-erp-backend@sha256:2940d0d89d54687f489839cb0a69f004c924a98bf6e5aa4b70f3b23c7d126323`; frontend image `ghcr.io/shmirzaev/milana-erp-frontend@sha256:dcfa04b2fb6f9c25bb5c9f2bd77fdcdd4fd2c54e8c07df038ec16d5cf88baa3d`. GitHub immutable release run `34190123702` passed all 610 backend tests, Ruff, compilation, the single Alembic head check, 11 observer tests, frontend lint, strict TypeScript, translation/build contracts and the optimized 85-route build.
- Inventory count is available from the Finished Goods inbox, Warehouse Stock and sidebar at `/warehouse-stock/count`. Saved whole-warehouse sessions compare physically scanned packs with received/reserved/damaged stock captured at the start. Results show found, missing, unregistered, unexpected, ambiguous and changed packs. Duplicate QR/legacy aliases count once; unfinished scans survive a page reload and can be retried; undo is explicit and completed results are frozen. CSV exports the full comparison. Existing Storage permissions are required; counts do not adjust packages, stock, reservations, receipts or shipments.
- Packaging Reports at `/packaging/reports` uses the selected factory and inclusive Packaging creation dates in Tashkent time. Daily, detailed and completed-job views and three-sheet Excel exports include real package quantities and model photos; legacy stock imports stay out of new output. Factory-scoped authorization remains enforced. Read-only candidate/public checks matched 190 packages / 12,255 pieces, 10 daily rows, 23 detail rows and 2 completed jobs for August 1 through September 8. September output was zero. Excel contained 23 embedded photos and 251,824 bytes; downloaded workbook tables/formulas and image anchors were verified.
- Verified final pre-release backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260908_051819.dump` contains 50,622,875 bytes and 1090 restore objects; dump SHA-256 `89beb7abe2ed79d872ea8dd5267207fafafd016acbaae4b73300b73fd6ad9f8e`, restore-list SHA-256 `49f512997db2a6c3a138efb7a414d9c14c58e2d1f58a57d22ef8645d585c08fc`. The first candidate's pre-migration backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260908_050013.dump` contains 50,613,321 bytes and 1,069 restore objects, SHA-256 `8464c3831f8cb56a4246f1b0b8956138df03c841c72aa726e1e850d55b3dbb3e`. Additive migration `0114_warehouse_stocktake` added two initially empty inventory-count tables under that verified backup; the final migration check was a no-op at this head. Production QA created no business records and changed no stock or package quantities; authenticated checks only refreshed normal session timestamps. Disposable PostgreSQL tests covered concurrent creation, 20 alias scans, 12 repeated unknown scans and concurrent completion with unchanged package/stock/receipt fingerprints. The isolated test containers were removed afterward.
- Signed-in candidate and public browser/API QA passed for the actual FGS account, including the inbox entry link; scanner mutations were exercised only in the isolated database. English, Russian and Uzbek labels, desktop/mobile layouts, duplicate/unknown/undo/completion behavior and denied unrelated access passed. Packaging date selection, tabs, download, all locales, empty state and four cross-factory denials passed. Final application browser logs had zero errors. All temporary QA proxies were stopped.
- Initial timing samples taken alongside QA exceeded one candidate median and one public p95 budget. Those failures are preserved in evidence. With all QA traffic stopped and the disposable database removed, unchanged-budget repeats passed: candidate maximum median +5.42% and p95 +7.22%; public medians improved 0.55–5.18% and p95 improved 0.35–9.47%, with unchanged result counts and payload sizes. A later closing public sample had an isolated global-search p95 spike (+75.97%, median +0.43%). Read-only diagnostics did not reproduce it: local old/new p95 was 38.88/39.14 ms, and public p95 was 147.09/146.44 ms. The standard closing public benchmark was repeated with every request timing retained and passed unchanged budgets. All failed samples are retained; the intermittent spike was not proven to be an application regression. No thresholds were relaxed.
- The full high-risk observation ran from `2026-09-08T05:36:23.532353+00:00` through `2026-09-08T06:06:23.709325+00:00`: 61 rounds / 244 successful internal/public health/login probes, zero failures or monitoring gaps. Closing checks reconfirmed release/rollback identity, both source manifests, valid routers, exactly two backend workers, zero restarts/OOM/error logs, database headroom and zero invalid indexes. Evidence is in `C:\ERP\.codex-work\warehouse-stocktake\outputs\deployment`; Packaging browser/Excel evidence is in `C:\ERP\.codex-work\packaging-date-reports\outputs\deployment`.
- Implementation worktree `C:\ERP\.codex-work\warehouse-stocktake`; implementation branch `codex/warehouse-stocktake`; coordinated source branch `codex/packaging-date-reports`; completion-record branch `codex/record-warehouse-packaging-deploy`. The legacy checkout and unrelated work were preserved. Previously documented audit/security findings remain outside this feature/deployment scope.
...
## Finished-goods inventory count prepared, not deployed (2026-09-08)

- Added Inventory count / Инвентаризация / Inventarizatsiya to the warehouse sidebar, Finished Goods inbox, Finished Goods page and Warehouse Stock. `/warehouse-stock/count` creates server-persisted whole-warehouse counts, captures received/reserved/damaged packages at the start, resumes saved sessions, records QR/barcode scans, and exports the full comparison as CSV. Unscanned packages remain "not scanned yet" until the user explicitly completes and freezes the report.
- Results distinguish found, missing, unrecognized labels, known packages outside starting stock, ambiguous legacy barcodes, and package/stock/location changes since the starting snapshot. Unique legacy aliases resolve to their package; repeats through any recognized alias do not increase the count. Ambiguous codes never select an arbitrary package. Unknown labels are review evidence only: scanning and completing a count never create packages or adjust stock, reservations, receipts or shipments.
- Count mutations require existing `storage.packages` or `storage.shipment` permission (or wildcard); navigation and the existing `/warehouse-stock` route guard enforce the same scope. The backend serializes scans/undo/completion with a count-row lock, enforces unique per-count identities, audits mutations, replays creation/completion retries, and freezes completion snapshots. The client queues rapid scans, preserves unacknowledged scans in per-user/per-count session storage, offers explicit retry and undo, and renders at most 100 result rows per page (API cap 200).
- Additive migration `0114_warehouse_stocktake` creates two count/evidence tables. It has not run in production. Before editing, both VM symlinks, full source manifests and runtime slot states matched `deploy/production-base.json`: active green `20260907_104027`, rollback blue `20260907_065723`, manifest `d8fa623cc917cdb2910de4c8a5d46d2e3a9abca78b33956a6245e8366f3e83a5`. Fetched `origin/main` was `7bd6d8049a379c3b6b646411fbaf52532324b3aa`; its only differences from the deployed implementation commit were baseline/context records. No production business row, schema or runtime was changed by this task.
- Worktree `C:\ERP\.codex-work\warehouse-stocktake`, branch `codex/warehouse-stocktake`. Validation: full backend suite 598 passed, followed by all 12 final targeted tests (including the two added authorization/migration cases); backend Ruff and compilation; single Alembic head; frontend lint, strict TypeScript, all inherited build contracts and optimized 84-route build. Tests cover the complete count lifecycle, alias duplicates, unknown/ambiguous codes, missing results, movement snapshots, undo, isolation, permissions, export escaping, request bounds, frozen reports, migration roundtrip and unchanged package/stock/receipt fingerprints. Runtime concurrency on PostgreSQL and signed-in browser visual QA remain deployment checks: automatic approval review rejected starting the local frontend server with only "blocked by policy". The existing broader audit/security findings remain outside this change. Merge and deployment are not authorized by the feature request.

## Mubina accessory restriction and material restoration deployed (2026-09-07)

- Active backend/frontend release is `20260907_104027` in green, from reviewed commit `32c6df5c89607daa0068005ef25f96dfec3ae2d3`, merged through PR `#67` as `a760aff3`. Blue release `20260907_065723` remains live for rollback. Both symlinks, slot identities and all 696 source files match; manifest SHA-256 `d8fa623cc917cdb2910de4c8a5d46d2e3a9abca78b33956a6245e8366f3e83a5`, archive SHA-256 `30f7d2ef88cfbe98566ddb72c19648cddaf22cb453a4c76365029d8f4527b070` (2,107,155 bytes). Backend image digest `ghcr.io/shmirzaev/milana-erp-backend@sha256:ef2e5206aa78a7d162cc6eedaf287f101006feca2946de5d253761985325e097`; frontend `ghcr.io/shmirzaev/milana-erp-frontend@sha256:4680077ad325c1f00a35ec7e1dff772f33269f252b8d1892a477b3c6a28b2c87`.
- Exactly Mubina's existing account (user 8, `mubina@milanapremium.uz`) received `inventory.materials_only`, retaining Storage, `inventory.batches.delete`, `inventory.force_override` and all other material/shipment permissions. Audit entry `16726` records the restriction. Before/after fingerprints prove other users and shared role permissions were unchanged; a repeat dry run was a no-op. Accessory inventory, receiving, issuing, returns, reservations, pricing, master data, direct URLs and accessory purchase receipts are blocked or filtered. Rolling back to the previous application would remove enforcement of this new restriction and must be treated as an authorization rollback too.
- Archived Materials now offers Return to inventory on desktop/mobile. A receiving-authorized user supplies the quantity physically present and a reason. Restoration locks and reuses the original fabric/semi-finished batch, records an audited return, preserves receipt/usage/reservation history, rejects duplicates, and resets QC to Pending and roll details for rechecking. Inactive master materials must be reactivated or reassigned first. This deployment and QA restored no batch and changed no stock quantity, receipt, reservation, order, package, shipment or schema. The explicit business/security write was the target user's permission list and its audit entry. Authenticated `/api/auth/me` checks also refreshed Mubina's normal `last_seen_at` session timestamp.
- GitHub Actions run `34112673119` passed all 588 backend tests, Ruff, compilation, 11 observer tests, frontend lint, strict TypeScript, all build contracts and the optimized 83-route build. Local full tests passed 585 before the final three coverage additions; all final 20 targeted tests passed. Signed-in candidate and public read-only browser/API QA as Mubina verified the restore dialog, hidden accessory navigation/master-data tab, direct-route denial, and preserved material/purchasing access with zero console warnings/errors. Desktop and mobile dialog layouts were visually checked; all QA proxies were stopped.
- Verified pre-change backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260907_104027.dump` is 50,459,551 bytes with 1,069 restore objects, dump SHA-256 `cf690b70c111fec78c4d698fc83795e59c9e289c3c36b131383061ac24737dcd` and restore-list SHA-256 `df22d3325effe9880e8fac5ea94497edbbcda39d743a3486e4fc582d0d643100`. Migration was a no-op at `0113_variant_selling_price`. Candidate performance passed with unchanged counts/payloads, median changes from -9.28% to +1.79% and maximum p95 increase 1.08%. Public performance passed with unchanged counts/payloads, median improvements of 0.03–2.10% and maximum p95 increase 3.83%.
- The complete 30-minute observation ran from `2026-09-07T10:53:20.106327+00:00` through `2026-09-07T11:23:20.101040+00:00`: 61 rounds / 244 successful probes, zero failures or monitoring gaps. Closing runtime checks reconfirmed active green/rollback blue, source manifests, healthy routers, two backend workers, zero restarts/OOM/errors, database headroom and zero invalid indexes; closing performance passed. Evidence: `C:\ERP\.codex-work\mubina-access-archive\outputs\deployment`. Legacy checkout preserved. Previously documented audit/security risks remain outside this change.
...
- The original save failure was caused by hidden default operations for all three factories being submitted by a single-factory Payroll account, correctly producing HTTP 403. The new frontend avoids generating those foreign-factory defaults. Existing backend factory validation remains enforced. The 36 workbook operations previously imported for each of Milana, Besttex and Eco Cotton remain exactly present on both `XJ3183-5966` and `XJ3183-5967`: 108 rows per variant and rate total `8680` per factory. A read-only check through the newly active backend confirmed the exact manifest match with no changes needed; the import was not rerun.
- GitHub Actions run `34093143952` passed all 568 backend tests, Ruff, compilation, deployment-observer regressions, frontend lint, strict TypeScript, all frontend build contracts, and the optimized 83-route build. The importer now verifies the LF-normalized manifest hash `04fcddd4b311c3af96da306370edd78b4897213c88cf2161a855c543898b59cd`, with a cross-platform regression; original Windows manifest hash `1e97061867479739837d21d0c3ee7a6ce1835dd0155ebba631c7b56d76d91403` remains in import provenance. The added frontend regression covers the factory default-load/save paths.
- Active backend/frontend release is `20260907_065723` in blue, from immutable source commit `07963a8450902c6353df2ca2d0e8638b67b4d39c`. Release `20260905_111623` remains running in green for immediate rollback. Backend and frontend switched at `07:07:50 UTC` and `07:07:51 UTC`; both current symlinks and slot identities agree. All 693 source files were verified byte-for-byte against the reviewed commit. Source-manifest SHA-256 is `d85e5004a03d1b80104d957230edca38ebd57037965a83bb44fbf6d313decf17`; archive SHA-256 is `917c1a29923c93b8659dd914bcd7cc437b4f03b873e6ed29ff14bacff08153f7` (2,096,986 bytes). Backend image digest is `sha256:60a9a6308502f04fb89dfe1188eb3fa82ea2361a1ea69ac996ea4a0216e9125a`; frontend image digest is `sha256:50019f80aa5d2d483f5ab5b2f467858dc639ab38b003201f3ab3ca6b4852e028`. Images were pulled by digest and staged through the supported blue/green procedure.
- Verified deployment backup `/opt/milana-erp/shared/backups/milana_erp_pre_20260907_065723.dump` contains 50,384,046 bytes and 1,069 restore objects. Dump SHA-256 is `2d7b5a63ffe3329811e927535e75c060a0e1c95490dff8afdf789b5da2f5926e`; restore-list SHA-256 is `5ab7f3aeea27bec19353bb87ee28034556a94ed39d3d76f1dde4c73119dfa649`. Migration validation was a no-op at `0113_variant_selling_price`. No production business data was written during deployment or browser QA; earlier data-only changes were limited to the two model operation lists/timestamps and audit entries `16683`/`16684` documented below.
- Signed read-only browser QA using an existing Milana Payroll account passed on both the candidate and public HTTPS deployment: each XJ3183 variant displayed exactly 36 rows numbered `1` through `36`, total rates `8680`, and the expected existing controls, with zero browser console errors/warnings. Business write methods were blocked in the temporary loopback QA proxy; no save, label issue, print, payroll posting, or payment was submitted. Both temporary QA proxies were stopped after verification.
- Candidate performance passed with identical counts/payload sizes across all five measured endpoints; median changes ranged from `-7.67%` to `+2.22%`, and the largest p95 increase was `4.97%`. Public post-cutover performance also passed with identical result counts, no payload growth, median improvements of `0.43%` to `2.52%`, and maximum p95 increase `11.73%`, within budget. Initial postflight verified all four internal/public health/login endpoints at HTTP 200, valid manifests and HAProxy configurations, active routers, exactly two backend workers, zero restarts/OOM/recent error markers, 28 of 100 PostgreSQL connections, and zero invalid indexes.
- The required 30-minute observation completed from `07:11:37 UTC` to `07:41:37 UTC`, with 61 rounds / 244 successful health/login probes, zero failed probes, and no monitoring gaps. Closing checks reconfirmed matching active blue releases, the live green rollback, valid source manifests/routers, two backend workers, zero restarts/OOM/error markers since activation, 31 of 100 PostgreSQL connections, unchanged revision `0113_variant_selling_price`, and zero invalid indexes. Closing performance passed all five endpoints with median changes from `-3.51%` to `2.38%` and maximum p95 change `12.51%`; result counts were unchanged and payloads stayed within budget. Evidence directory: `C:\ERP\.codex-work\xj3183-paid-operations\outputs\deployment`. Worktree: `C:\ERP\.codex-work\xj3183-paid-operations`; implementation branch: `codex/xj3183-paid-operations`; completion-record branch: `codex/record-xj3183-payroll-deploy`. The previous rollback release remains live after observation. No unrelated legacy-checkout changes or production data were deleted or overwritten. Previously documented audit/security risks remain outside this exact change.

## XJ3183 paid operations imported; Payroll numbering/save fix prepared (2026-09-07)

- At the user's explicit request, the 36 operations from `1-3183.xlsx`, sheet `Лист1`, cells `A2:E37`, were saved to both existing standard-catalogue variants `XJ3183-5966` and `XJ3183-5967` for Milana, Besttex and Eco Cotton. Each variant now has 108 explicit factory rows, 36 per factory. Names, source order, duration, original stage and rates are preserved, including the final zero-rate `Upakovka` row. Each factory's rate total is `8680`; source stages use the established old-ERP section mapping.
- Workbook SHA-256: `a824551e8dbb33d8f15d96e17b01d40b7853422549299d7a0d3422d0265a5be0`. Reviewed manifest: `backend/data/xj3183-paid-operations-20260907.json`, SHA-256 `1e97061867479739837d21d0c3ee7a6ce1835dd0155ebba631c7b56d76d91403`. The guarded importer refuses conflicting existing operations, locks only the two target models, preserves unrelated details, validates exact factory readback before commit, and is idempotent. Audit entries `16683` and `16684` record the authorized import and source provenance.
- Verified pre-write backup: `/opt/milana-erp/shared/backups/milana_erp_pre_20260907_064828.dump`, 50,375,451 bytes, 1,069 restore objects, SHA-256 `f631073c6e861b94518fcf69f18c134ae6b0f16fa1bcbea3b5d5756cdc160236`; restore-list SHA-256 `5612616d214dd631847e161ecc21915a0c693647675ec80981c7c298e7eff592`. Post-commit readback and a repeat dry run matched exactly. All six variant/factory projections passed pure save validation while retaining the other factories' operations. All four internal/public health/login checks returned HTTP 200.
- The earlier Payroll save incident had five HTTP 403 responses for `XJ3183-5967`. Missing saved operations caused the frontend to expand default operations for all three factories, display only the current factory, and submit the hidden defaults too. The backend correctly rejected the cross-factory payload. After this import, reloading either XJ3183 variant loads explicit factory-scoped operations and avoids that trigger. Previously issued QR labels and payroll records were not changed.
- A clean branch `codex/xj3183-paid-operations` in `C:\ERP\.codex-work\xj3183-paid-operations` adds a leftmost `№` column with one-based visible operation numbers and limits legacy/default materialization to the selected session factory. Explicitly configured other-factory rows remain intact for existing administrator saves. The endpoint and authorization rules are unchanged. Implementation/import commit: `713e87ee`. The code is pushed but not merged or deployed; the numbering and general missing-operation fix still need an authorized release.
- Validation passed all 567 backend tests (including the three importer safeguards and 35 payroll/factory tests), repository-wide backend Ruff, changed-script compilation, frontend lint, strict TypeScript, all production build contracts including the new factory-load/save regression, and the optimized 83-route build. GitHub Actions is also validating the pushed branch; merge/deployment remains pending authorization. Application production remains `20260905_111623` in green, with `20260905_085355` in blue for rollback; both verified source manifests remain `3d8fe17c8c39b041aef1fec6b1b4d98e64ce56cfac9cf8cfc5ff559fb3547484`, and the database remains `0113_variant_selling_price`. Only the two models' operation lists/updated timestamps and two audit entries changed; no application release, schema, role, stock, order, issued label, payroll ledger or payment changed.

## Branded Stock Order colors and production dialog deployed (2026-09-05)

- Order-number groups now show white when no linked production has Cutting output/completion, yellow while only part is cut, and green when every linked production has completed Cutting. Empty groups stay white. Expanded production rows are green only after all their relevant Cutting work orders complete; unfinished rows stay white. Cancelled/rejected Cutting work orders are excluded. Group progress is computed across the whole group, including productions hidden by a search.
- Add production opens a separate scrollable modal. Model-number search and variant selection are separate; only approved variants belonging to the selected model are offered. The model and selected variant have full-image previews. Changing the model clears the old variant and material selection. Existing standalone approved models, forecasting preselection, creation payload, validation, authorization, material/quantity logic and downstream handoffs are preserved. New labels support English, Russian and Uzbek.
- Implementation commit `dd67bda711a4ab5cad2f8181ec41ded7222f5342` and runtime-translation fix `c8bbda964aed522501645aa069bf9ce222397959` were pushed and merged through PRs `#62` and `#63`. The final source tree matches merge `14fd094d`. GitHub Actions run `33962884744` passed validation and published immutable release `20260905_111623` from `c8bbda964aed522501645aa069bf9ce222397959`. The 689-file source-manifest SHA-256 is `3d8fe17c8c39b041aef1fec6b1b4d98e64ce56cfac9cf8cfc5ff559fb3547484`; archive SHA-256 is `0c08ff9258e3ce6beabf386b8d163c1fe74a30c3f32af8748ba7a80080ad5043`. Backend image digest is `sha256:8814f4960e22035d2b50ac020f04f17aed2d8a982bb18b2195a79eac45746f0d`; frontend digest is `sha256:03dfc3492d03d3779f6c27375da2b2d2013f03ed9a393e29e77434ff1459c02d`. Earlier candidate `20260905_110100` was withheld after browser QA found unloaded translation keys and was never activated.
- Active backend/frontend release is `20260905_111623` in green; `20260905_085355` remains running in blue for rollback. Both roles switched at approximately `11:27:26 UTC`. The verified pre-deployment backup is `/opt/milana-erp/shared/backups/milana_erp_pre_20260905_111623.dump`, 49,839,646 bytes with 1,069 restore objects; dump SHA-256 is `71fc3bfdc6793330af77d0df5fad1a7018923aa7e58eb15871a3128b396d08fe` and restore-list SHA-256 is `18fdc95868969d15bae7c9870ba9f37f3eb14189c2a486ad9d729ea739264b26`. Migrations were a no-op at `0113_variant_selling_price`. This task created or changed no production business data.
- Validation passed all 564 backend regressions, Ruff, Python compilation, frontend lint, strict TypeScript, the new runtime-locale regression, all existing frontend contracts, and the optimized 83-route build. Read-only API QA independently matched Cutting work-order evidence for all 31 groups: 14 white, 8 yellow, 9 green; all pre-existing response fields remained identical. Branded-order median was 113.74 ms versus 113.08 ms, p95 improved 18.05%, and the added status fields increased payload by 7.51%, within budget.
- Signed-in candidate Chrome QA verified group and individual colors, separate-modal behavior, model search `PJ1089`, its six approved variants, both loaded `PJ1089`/`V-6056` pictures with containment, reset on switching to `TJ2090`, phone layout, English/Russian/Uzbek Cutting labels and zero console errors. Public signed-in QA confirmed the deployed colors and modal with zero console errors. No production submit action was used; the temporary GET-only preview was closed.
- Initial timing outliers in unchanged endpoints were preserved and investigated; runtime/load diagnostics were clean and the separate warmed candidate gate passed. The public benchmark initially changed its automatically selected newest model because catalogue records were added at `11:23:04 UTC` after the baseline. Repeating with the original `XJ3187-V00000000` term restored identical search results. Closing 21- and 63-sample inventory-stock p95 results exceeded the limit despite improved medians. A separate 63-sample diagnostic returned identical 49 rows/10,808 bytes on the old and new releases, with old/new internal p95 of 22.57/21.33 ms; the public stock recheck passed at 137.22 ms. This points to intermittent public-path latency rather than an application regression. The final gate combines the four already-passing 63-sample endpoint probes and this investigated final stock probe: median changes ranged from -23.34% to -1.90% and p95 from -50.35% to -3.11%; result counts matched and payloads remained within budget. All failed samples and per-request stock timings remain preserved in deployment evidence.
- The complete diff was classified low risk: isolated display/form presentation and read-only progress serialization, with no changes to data, permissions or workflow mutations. Observation completed 21 consecutive rounds over 600.20 seconds with all four internal/public health/login probes HTTP 200 and zero failures. Closing checks verified both current symlinks, source manifests and slot identities, active routers, valid HAProxy, two backend workers, zero container restarts/OOM or recent error markers, database headroom and zero invalid indexes. Backend/frontend disk use remained 64%/56%.
- Worktree: `C:\ERP\.codex-work\branded-stock-order-window`; implementation branch `codex/branded-stock-order-window`, record branch `codex/record-branded-stock-order-deploy`. Evidence is in `outputs/deployment/20260905_111623-evidence.json` and `outputs/deployment/20260905_111623-observation.json`. Previously documented unrelated audit/security risks remain open. `deploy/production-base.json` and the Obsidian context mirror are updated to the verified active release.
...
- The correction preserved every original file byte-for-byte, created 70 lossless WebP derivatives containing all non-padding pixels, prebuilt 210 thumbnails (160/320/640), and updated only the 145 affected image file references. Source-file and relation identity hashes guard the operation, and a repeat run returned `already_applied`. No model, variant, package, QR/barcode, size, quantity, weight, stock balance, reservation, or warehouse receipt data was created, deleted, or edited.
- Model-list images now render at their meaningful source aspect ratio with `object-fit: contain`; the artificial portrait canvas is no longer imposed on corrected catalogue images. Normal detail-page images keep the existing portrait presentation.
- Implementation PR `#60` merged as `ae2de66fd8f6d17be7cd73d4659f094e93cfac54` (implementation commit `9df3a3b9866315dab6162b46ec551b968c2003de`). GitHub Actions run `33956516880` passed backend/frontend validation and published immutable release `20260905_085355`. The 686-file source-manifest SHA-256 is `357ad7c4ddcb44deff1e87a8d6cc3d8c6c437b358221ae13aacf224f7c7e1fed`; archive SHA-256 is `21ecc90791b425ec3e3b5773fb29cc4562072d16fb59a8497d5a48e6ed6bdcd0`. Backend image digest is `sha256:cdb583e7cdc5debbd5078a50e3cdb5a5071c960e38c1ffb57374ece8e07917d0`; frontend digest is `sha256:e02c87c4941ddb58499dc759a320096c059907fc2d1b6756b25d594ac1a000e3`.
- The validated pre-change backup is `/opt/milana-erp/shared/backups/milana_erp_pre_20260905_085355.dump`, 49,798,460 bytes with 1,069 restore objects; dump SHA-256 is `5d46023dfe71c0090f304e6c4f85c5b80ca9ec6864c96e1def22c0e68b8b947d` and restore-list SHA-256 is `14a371784cf1207f4648efd51dbb1f71488b6f465fbc1e2359f648166296731f`. Alembic remains `0113_variant_selling_price`.
- Local validation passed Ruff, Python compilation, all 562 backend tests, the model-picture contract, frontend lint, strict TypeScript, every inherited production contract, and the optimized 83-route build. Candidate and final warmed performance gates passed with identical rows and 0% payload change; closing median changes ranged from -4.49% to +2.91% and p95 from -10.85% to +5.68%. Isolated preliminary samples stopped on shifting outliers in unchanged endpoints; container/load diagnostics were clean and fresh warmed gates passed before completion.
- Active backend/frontend release is `20260905_085355` in blue; `20260905_073943` remains running in green as rollback. Closing checks found both source manifests valid, both routers active with valid HAProxy configuration, active and rollback containers at zero restarts/OOM, one Uvicorn master plus exactly two workers, no matched backend/frontend traceback, exception, critical, or HTTP-5xx markers, 35/100 PostgreSQL connections, zero invalid indexes, and 63%/55% backend/frontend disk use.
- Signed-in Chrome QA loaded all eight XJ318x samples from `old_erp_catalog_trim_*` sources using full-image containment, including XJ3186, XJ3185, XJ3184, and XJ3183; all images had valid natural dimensions, zero broken images, no gray lower canvas, and an empty console. The high-risk observation completed 61 consecutive 30-second rounds over 1,800.16 seconds; every internal backend, internal frontend, public health, and public login probe returned HTTP 200 with zero failures.

## Warehouse legacy pictures and full-image display deployed (2026-09-05)

- Warehouse Stock previously had no image source for 257 hidden `LEGACY-*` models covering 467 imported package rows. The original sticker-photo evidence still existed locally for every affected model. A deterministic audit found all 467 referenced source-photo names, zero missing names, zero conflicting hashes, and selected one original representative photo for each of the 257 models.
- The production attachment is deliberately warehouse-only. It created 257 `ModelImage` rows with type `warehouse_package` and copied 257 original files byte-for-byte (6,572,825 bytes total) into persistent model storage. Normal model/material pictures keep precedence, and the hidden legacy records remain excluded from Models/Variants. No model, variant, package, QR/barcode, size, quantity, weight, stock balance, reservation, or immutable legacy receipt evidence was changed.
- Models, model-detail variant thumbnails, and Warehouse Stock cards/rows now use full-image containment. The old landscape-photo rotation and `object-cover` cropping were removed. Clicking a warehouse picture still opens the original full-resolution file; thumbnails are generated only for display.
- PR `#58` merged as `cd1c0801ee0e4e6c35c557307726d4ebb1f1bee2` (implementation `c9b50a25b97146830f163102b66f75607e9dc3b5`). GitHub Actions run `33953178309` passed backend/frontend validation and published immutable release `20260905_073943`. Its 684-file source-manifest SHA-256 is `1e42f763fb12e2e8907981dc3d93b5fb5230f31c8d08da9e403a47c306bf5d09`; archive SHA-256 is `c26f792a2240b0c10a96b7716bed62bfc4deb00f3a45d4a9be32380d507f7fb3`. Backend image digest is `sha256:c785ee92ad997d115bdf96ff72522d557737abbdf4bfa97facd92335e587e40d`; frontend digest is `sha256:974b5aa3816e9576e382e1104c1ce65b47365770620bf86cd9fb27b8e30d24e5`.
- The guarded production dry run covered 257 models / 467 package rows. The verified pre-change backup is `/opt/milana-erp/shared/backups/milana_erp_pre_20260905_073943.dump`, 49,789,598 bytes, mode `0600`, with 1,069 restore objects. Dump SHA-256 is `f278a459a9a5230f216735eb8fc468c415d0e67799f95e0401b939be20cf405d`; restore-list SHA-256 is `0a7aad90bdd6be8532b8d94e93db89c11d5c1fb2be9d0c647f48b054664d785e`. Alembic remains `0113_variant_selling_price`.
- Active backend/frontend release is `20260905_073943` in green; `20260905_044515` remains running in blue as rollback. Candidate and post-cutover performance gates had zero failures and identical response payload sizes. Initial postflight found both active/rollback roles running with zero restarts/OOM, active routers and valid HAProxy configuration, exactly two backend workers, 23/100 PostgreSQL connections, zero invalid indexes, and exactly 257 warehouse-picture rows.
- Signed-in Chrome QA verified the visible Models and Warehouse Stock samples use `object-fit: contain` with no transform, restored pictures are present on the first eight legacy warehouse cards and visible table rows, every sampled image loaded, and the console had zero errors. The high-risk observation completed 61 consecutive 30-second rounds over 1,800.2 seconds; every internal backend, internal frontend, public health, and public login probe returned HTTP 200 with zero failures. Closing inspection again found release `20260905_073943` active in green with `20260905_044515` running in blue as rollback, both backend/frontend slots at zero restarts and OOM events, valid release manifests, active routers and valid HAProxy configuration, exactly two backend workers, no matched recent backend/frontend traceback, exception, critical, or HTTP-5xx markers, 31/100 PostgreSQL connections, zero invalid indexes, and exactly 257 warehouse-picture rows. Closing signed-in QA again found zero broken, cropped, or rotated images in the visible Warehouse Stock and Models samples and an empty browser error console.
...
- This was a data-only correction. Active backend/frontend release `20260905_044515` remains unchanged in blue, and `20260904_113556` remains running in green as rollback. PR `#54` merged the guarded correction tooling and production record as merge commit `5962c603bf922e3f92f4a4a563eed24324056d49` (implementation commit `68af66e9`).
...
- PR `#52` merged the fix as `4ee1a3db3cf4785094f1843483b7c97b5babc0a6` (implementation `9d37025b725a5bd1325f25db158ab87b828e0c47`). GitHub Actions run `33945408183` passed the full backend/frontend gates and published both immutable images. All 676 archive source files were verified byte-for-byte against that merged Git commit before staging. The 2,034,459-byte source archive SHA-256 is `402687f7696b419cdf9958a60f40cb0a396590e1ec8eb03fdac74e1351885ad2`; source-manifest SHA-256 is `677a4d4be3d5f83e5b9f171d83f8a1d47604679e86e426cb5db00f037e22c389`.
- Active backend/frontend release is `20260905_044515` in blue. Backend image digest is `sha256:353cba7fbbcabf42c9066c18b1b045e011ae5630a3e54c16c66b030c57ccff49`; frontend image digest is `sha256:2a1f52da752deb9bfb9a55217dd27fb51edcbb78a7b492a78543bee7f343762f`. Both images were pulled and staged by digest, with no production build. Release `20260904_113556` remains running in green as rollback. Backend cutover was at `04:54:50 UTC`, followed by frontend at `04:54:56 UTC`; both current symlinks and runtime states were verified.
- Pre-deployment backup `/opt/milana-erp/shared/backups/milana_erp_pre_bundle_scope_20260905_044515.dump` is mode `0600`, 49,789,591 bytes, with 1,069 restore objects. Dump SHA-256 is `ba03fe8a43d8ca44ff7f6821addfa304d493a0d6950c0ba6c70546d5167039ef`; the 76,226-byte restore-list SHA-256 is `d4d8475f77ee8a4708ad1ceffdea51e5de25bc7e60fb2e0eeb9ad3d903ff8582`. Candidate migrations were a no-op at `0113_variant_selling_price`.
- Candidate and public signed-in Chrome QA verified Eco Cotton bundle `BND-2026-001577` has no Receive action in the Milana workspace, while Milana bundle `BND-2026-002504` retains Receive at Milana. Both browsers had zero warning/error console entries. The candidate receiving service also returned `403` for the real Eco bundle under a Milana session inside an explicitly read-only database transaction, with no ORM changes. No receive, batch acceptance, or physical printing action was submitted against production.
- The warmed active/candidate performance gate passed with identical result counts and payload sizes. The public post-cutover comparison also passed: endpoint medians improved by `0.61%` to `2.94%`; the largest p95 increase was `5.37%`, within budget. All four internal/public health and login checks returned HTTP 200. Initial postflight verified valid source manifests and HAProxy configurations, active routers, two workers per backend slot, zero container restarts/OOM/recent error markers, 27 of 100 database connections, and zero invalid indexes. Backend/frontend disk use before staging was 63%/55%.
- The required 30-minute observation records health/login probes every 30 seconds in `C:\ERP\.codex-work\fix-sewing-bundle-factory\outputs\deployment\observation.json`; deployment, backup, performance, and runtime evidence are in the same directory. The temporary candidate browser proxy was closed after QA. Worktree: `C:\ERP\.codex-work\fix-sewing-bundle-factory`; deployment record branch: `codex/record-bundle-receiving-deploy`. No production business data or schema was changed; the only data operation was the validated backup. Other audit findings, including the previously documented credential exposure, remain open.
...
## 2026-09-25 - Audit fix pass cycle 9

- Latest local fix commits: `46f5b1b6` purchase receipt row-locking; `a538c480` Sales Order type vocabulary; `46ae5ec8` malformed legacy accessory JSON read safety; `8aebf877` fail-closed waste remaining balance; `96674427` HR Documents employee picker paging; `88cedd2c` Finance invoice paging; `640a7620` Sales Order currency provenance; `f0463331` frozen shipment invoice cent bounds; and `5f95ed1e` dashboard currency display plus two fresh-schema redundant-index removals.
