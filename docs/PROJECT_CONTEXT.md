...
- Latest reviewed batches: **41/127 findings fixed**. Admin deletion, waste read-side effects, consumed-stock release and HR input checks have targeted regression coverage. Print runs: 52→3 SELECTs for 50; attendance import: 403→4 for 401 people. WF08 backend sale capacity/retry-key support is partial: UI remaining-balance/key handling and accounting valuation remain open. Current batch covers legacy reservations, HR attendance day/hours, shipment-document scanning and purchase conversion. No deployment or production access.
