...
## Production old-ERP sticker catalog and inventory completion (2026-08-31)

- The signed-in old ERP was searched by exact model and variant for all 61 catalog identities that blocked the user's corrected sticker workbook. Sixty exact variants were found and transferred; `XJ3062 / V-5709` did not exist in the old ERP and was not created as a catalog variant. The user later confirmed its three labels were cut as mixed material and authorized inventory receipt under the existing approved blank-variant XJ3062 base model, production model ID `325` (`ХJ3062`).
- Immediately before creation, the live production catalog contained none of the 60 exact identities and none of their target model codes. Production added approved model IDs `7343` through `7402`, exactly 60 primary model-image relations, 59 exact old-ERP variant-image relations, 345 size rows, and 60 color rows. Original image bytes and dimensions were preserved; repeated image content uses one content-addressed storage file rather than duplicate files. `TJ2026 / V-5782` has no variant image in the old ERP and therefore has only its verified primary model image.
- The newly unblocked inventory import added 318 collision-free packages, 21,404 available pieces, 318 immutable legacy receipts, 318 package items, 318 finished-goods stock rows, 318 old-ERP QR aliases, and 318 scan-evidence rows across the 60 new variants. Two stickers do not print Weight (`PJ1216 / V-5828` and `XJ3172 / V-5781`); their package weight remains `NULL` exactly as the user authorized. The other 316 packs total 7,835.70 kg.
- Warehouse 8 rose from 1,896 to 2,214 packages and from 129,942 to 151,346 available pieces. Every requested target was checked against production legacy receipts, package barcodes, package numbers, and barcode aliases before apply; all collision counts were zero. All 318 QR codes resolved through the active ERP API to 318 distinct packages, and representative public-domain checks—including numeric QR `1475522`—returned HTTP 200 with the exact package.
- Model manifest SHA-256 is `bc7c5838bdaf45ed8fe145ac6cceeec9fd16368f9dd5b2d3a851f2c8c616ae51`; pack manifest SHA-256 is `81566180a0add849e37a29b4842dcad1c59dd11b524d9b6ea7b15874a0bd240b`. Catalog audit row `16053` has entry hash `2ef6625caa7776342cb7e04699f348ce9f4358629f49bd3f8caa376a7a848a56`; inventory audit row `16056` has entry hash `4f9973eeacc9ef5356288e053d1d907974372cd95fb2feee1bfe918544da45c6`.
- Verified pre-import backup: `/opt/milana-erp/shared/backups/milana_erp_pre_20260831_111954.dump`, 48,012,210 bytes and 1,067 restore objects; dump SHA-256 `5201a358b57d26b0a0578164c8a58a062a4eb2f8dfc4aa6c3d8c18ae427ee41e`, restore-list SHA-256 `b9bcedc0bfa55cc5dd813f5f5543306a22100cdceff58333b7796afe6462ce65`.
- All 80 unique image URLs passed authenticated byte-identical checks through both the active backend and public domain (160 HTTP checks). All four internal/public health and login checks returned HTTP 200. This was a guarded production data operation only: application release `20260829_112849`, source-manifest SHA-256 `9b0eee10f6ef3e2ed172f81934e182fd67e0d9dd462a764baeadde867e27380e`, rollback release `20260829_081957`, and database head `0112_price_calc_requests` remain unchanged.
- The mixed-material follow-up added exactly three packages and 180 available pieces (`uzerp_ii_20666_1`, `uzerp_ii_20713_1`, and `uzerp_ii_20713_2`) to warehouse 8 under model ID `325`. Package color is `Mixed material`; sizes, weights (26.5, 26.8, and 26.8 kg), quantities, source variant `V-5709`, exact PDF references, and hashes remain in immutable receipt evidence. No model, size, color, or image catalog row was created or changed; the production model count remained `6,711` and no `XJ3062 / V-5709` identity exists.
- Warehouse 8 now contains `2,217` packages and `151,526` available pieces. All three QRs returned HTTP 200 with model ID `325` and quantity 60 through both the active backend and public domain. Mixed-material manifest SHA-256 is `7e0fb8a3531de2c7c2f661f16fb4703b5677892befd3bf755cc4dcfa9277dab1`; audit row `16089` has entry hash `0836ff00c4d074e89f674396a02c75dbf2fbd20a8a166117f5b93a231087b1ea`.
- Verified mixed-material pre-import backup: `/opt/milana-erp/shared/backups/milana_erp_pre_20260831_114542.dump`, 48,073,343 bytes and 1,067 restore objects; dump SHA-256 `bd113ffd6560e9d996058bbfef4e0f4beef5720bd3d9439068b1b63e82e6273b`, restore-list SHA-256 `33d4ebe941a590eb469866a23f5f5dc513d62ab8a77dc006ed1275339d209ed4`. All four required health/login checks passed; the active and rollback releases, source manifest, green slot, and database head remained unchanged.
...
- The user's completed correction workbook was re-read on 2026-08-31 and checked again against the live catalog and all legacy receipt/package/barcode identifiers. Two uniquely matched, collision-free corrections were imported: `1475523` for `PJ1077 / V-5404` (6 pieces, 2.24 kg) and `uzerp_ii_20555_1` for `SJ4004 / V-5192` (18 pieces, 19.78 kg). Audit row `16004` has entry hash `1570479cbf1144deabee2d2c63a960da273e8deb63355cdbbd9b74b8cd62d6f0`.
- The correction import added exactly two packages, 24 available pieces, and 22.02 kg. Warehouse 8 rose from 1,894 to 1,896 packages and from 129,918 to 129,942 available pieces. The correction manifest SHA-256 is `c34ca2e14a7c0b403f91a90185934f94904a7e9fe5a05a6e46489214043b482f`; the user workbook SHA-256 is `c99e692bd7881aeda9239759233aecccb01e39d85879d29415dc266e607e7fc6`.
- A verified pre-correction PostgreSQL backup is `/opt/milana-erp/shared/backups/milana_erp_pre_20260831_104339.dump`, 48,006,646 bytes and 1,067 restore objects. Dump SHA-256: `9be0b8d0007162b4eb958fe1379898698f33cc1a4cd86bbde8a6785bd22f0495`; restore-list SHA-256: `cc0aa826463ce42d3673cd1c00bc9767e1ae2930933c358e0bea5d3989a5d980`.
- Sixty-one unique model/variant pairs remain absent from the live production catalog. The complete `PJ1080 / V-5787` correction remains withheld because its catalog variant is absent; `PJ1216 / V-5828` and `XJ3172 / V-5781` also remain withheld because their Weight cells are still blank. No missing model/variant or Weight value was guessed or created.
- Authenticated QR lookups for both corrected packs returned HTTP 200 and the exact package through the active backend and public domain. All four internal/public health/login checks returned HTTP 200. This was a guarded production data import only: application release `20260829_112849`, green slot, source manifest, rollback release `20260829_081957`, and database head `0112_price_calc_requests` remain unchanged.
