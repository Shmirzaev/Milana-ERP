...
- Latest reviewed batches: **53/127 findings fixed**. Legacy packages require recorded packaging output; historical correction/manual receipt/import paths retain their behavior. Management dashboard uses live data again; Home labels stage activity, not unique garments. Attendance imports serialize (five PostgreSQL cases), but source versions remain open. Purchasing references are batched; audit-head reads remain. Next batch: safe receipt reconciliation, production PATCH validation and material-planning queries. No deployment or production access.
