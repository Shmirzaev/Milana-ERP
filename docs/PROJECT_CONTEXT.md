...
- Legacy printed labels continue to resolve through aliases and their existing label identities. Database IDs, bundle/package/employee/model namespaces, scan UIDs, payroll rates/quantities, raw audit evidence and idempotency request fingerprints remain unchanged. Bundle QR images now render from live canonical order data through an authenticated, non-cached endpoint; printable labels embed freshly generated QR data. Process QR printed labels include the canonical order line. Cutting passport writes use the linked order's live reference; unlinked passport, inventory and purchasing inputs resolve only known aliases and preserve unknown external/manual text. Inventory receipt replay renders canonical references without mutating its stored response/hash or creating stock again.
