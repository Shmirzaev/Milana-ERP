...
- Latest reviewed batches: **73 fixed,37 partial,17 open;54 of127 remain**. Partial findings are not counted as resolved. Reserved stock can no longer race into damaged state. Usluga cutting reads are chunked; first-attempt429 receipt retries can be corrected. Validation-only run `35675969462` passed backend, frontend lint/strict types/optimized build/contracts and all PostgreSQL selections on application revision `bae444d`; release skipped. No deployment/production access; develop conflicts, historical audit damage, runtime lifecycle and load validation remain open.
...
- Continued 2026-09-22 (continuation batch): reviewed commits add bounded or paged list reads, batched lookups, grouped aggregates, deferred hidden directories, request-scoped validation and focused security/workflow corrections across catalog, inventory, finance, HR, production and admin paths. Broad backend selections pass 173 tests with13 skips,145 with8, and the latest dirty-module review156 with11; frontend stabilization, strict TypeScript and targeted ESLint pass. Later follow-ups preserve normalized accessory search, batch cutting-passport Item validation and package-order synchronization, preload pricing mutation assets, use Decimal profit intermediates, group 1C status refreshes, page HR Documents and Calendar end-to-end, validate forecast references, and gate backend traffic on /ready while retaining /health liveness warm-up. Packaging BOM reads are batched; payroll matching is indexed; receipt access is not duplicated; inbox image blobs are deferred; forecast BOM batches are eager-loaded; cost breakdown uses exact intermediates; HR employee status codes match the UI; and legacy Usluga responses no longer truncate above500 while the UI remains paged. Reviewed commits through `d71c426` are pushed to `feat/ismoiljon`; `bf36441` through `652f754` are queued. Counts are73 fixed,37 partial,17 open,54 remaining. No migration, merge, deployment or production contact.
