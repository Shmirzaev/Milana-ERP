...
- Latest reviewed batches: **45/127 findings fixed**. HR attendance now uses Tashkent dates and validated hours; shipment documents use indexed lookups (46 related tests; 4 independently repeated). Admin, stock reserve/release and purchase conversion races have targeted coverage. Print runs: 52→3 SELECTs for 50; attendance import: 403→4 for 401 people. WF08 remains partial (UI retry/balance handling); ST09 blocks new reservations of damaged goods but reverse damage transition policy remains open. Next batch: attendance history/results, package model validation and sewing capacity queries. No deployment or production access.
