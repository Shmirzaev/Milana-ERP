...
- Additive migrations `0115_package_print_runs`, `0116_pack_sales_dispatch`, and `0117_package_quantity_evidence` are prepared; none ran in production. Existing rows are retained, new optional fields are not backfilled with guessed history, and downgrade guards preserve receipt/count evidence. Production release, database revision `0114_warehouse_stocktake`, business data, roles and permissions remain unchanged.
