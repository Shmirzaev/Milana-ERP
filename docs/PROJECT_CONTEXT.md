...
- Continued 2026-09-25 (PERF35 accessory Receive/Return): this real screen now searches returnable production-order choices as distinct 50-order pages with exact order totals, then searches the selected order's catalog-item issue groups in separate 50-row pages with exact group totals. The endpoint's default legacy array and itemless history remain compatible; only the opt-in returnable picker filters out itemless/nonpositive rows. Eleven focused accessory balance tests pass, including a case with 401 unique orders and 402 returnable item groups, as do the frontend contract, full stabilization, strict TypeScript, targeted ESLint and Ruff. Exact totals still scan and group complete issue/return history in Python, so this bounds transferred/rendered data, not source-query/aggregation cost. PERF35 remains partial; no migration, production contact, push, merge or deployment occurred.
