...
- Latest reviewed batches: **69/127 findings fixed;36 open;22 partial**. PostgreSQL audit appends serialize at outer commit (`735961d`);12 focused cases independently pass within30 PG regressions. Audit IDs/hashes are transient until commit; READ COMMITTED is required, SQLite unchanged, historical damage unrepaired. Password races, accessory/label batching, partial stocktake paging, numeric bounds and revenue-date comparisons are reviewed. CI passed at `92baf3b`; later changes need a combined run. Review audit transaction contract, scan-only payroll roles and manual-status clients before rollout. No deployment/production access; main conflicts remain unresolved.
