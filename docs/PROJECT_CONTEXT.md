...
- The user later requested that branded-stock Sales Order `SO-2026-000002` be returned so it could be recreated. A locked atomic transaction reverified order ID `3` was `ready`, had exactly 27 stock reservations totaling 258 pieces, and had zero linked production orders, order packages, shipments, invoices, or payments. It released those 258 pieces back to Finished Goods across source packages `102`, `103`, `104`, `105`, and `108`, deleted only the 27 reservation rows, set the Sales order to `cancelled`, preserved the order/items for history, and wrote audit row `16214`. Touched-stock available quantity rose from 4 to 262 and reserved quantity fell from 266 to 8; the remaining 8 pieces belong to other reservations. Signed-in Order History showed the cancelled status, and all four internal/public health and login checks returned HTTP 200.
- The Sales-order return has its own verified pre-change backup: `/opt/milana-erp/shared/backups/before_return_SO-2026-000002_20260902T103040Z.dump`, mode `0600`, 48,481,487 bytes and 1,067 restore objects; dump SHA-256 `53ab58ce455731dce2e265b769819bff803ac6c401ff01574b8258bd2c38abba`, restore-list size 76,041 bytes, and restore-list SHA-256 `3c5abb01fc8299d4ed05220a83a63df602e5b39c6960614d987581788e112212`.
- The user clarified that `SO-2026-000002` should be permanently deleted rather than retained as cancelled. A fresh locked check found the order still cancelled with zero reservations, production orders, shipments, order-owned packages, invoices, or payments. The supported guarded Sales-order deletion removed exactly order ID `3` and its two item rows and wrote audit row `16216`; it did not alter package or finished-goods stock. Signed-in production verification showed only `SO-2026-000001` in Sales Orders, and all four internal/public health and login checks returned HTTP 200.
- The permanent deletion has a separate verified pre-change backup: `/opt/milana-erp/shared/backups/before_delete_SO-2026-000002_20260902T103851Z.dump`, mode `0600`, 48,481,627 bytes and 1,067 restore objects; dump SHA-256 `891baf263bb7bbcf3dfada609190f0d9182208c4b63d6ad3fe2f3639938e92e3`, restore-list size 76,041 bytes, and restore-list SHA-256 `9687a2e685c9350d7b06d41a29e410c6d16c3329d7ba03b0dc54961b3f654d72`.
- The user then requested permanent deletion of the remaining branded-stock Sales Order `SO-2026-000001`. A locked atomic transaction reverified order ID `2` was `ready`, had exactly 14 reservations totaling 138 pieces, and had zero production orders, shipments, order-owned packages, or payments. It released the 138 pieces to Finished Goods from source packages `105`, `106`, and `107`, removed the 14 reservation rows, deleted unpaid internal invoice `INV-2026-000001` (invoice ID `1`, $938.40, no external link), and deleted the Sales order plus its two item rows. The source packages remain intact; touched-stock available quantity rose from 2 to 140 and reserved quantity fell from 138 to 0. Audits `16219`, `16220`, and `16221` record the stock release, invoice deletion, and order deletion respectively. Signed-in production showed zero Sales orders, and all four internal/public health and login checks returned HTTP 200.
- This deletion has a verified pre-change backup: `/opt/milana-erp/shared/backups/before_delete_SO-2026-000001_20260902T104436Z.dump`, mode `0600`, 48,504,187 bytes and 1,067 restore objects; dump SHA-256 `a7fe5cccf06e53ec85b150766adbd7f8e6e3a703b0836e962d6c8fa8651eeb92`, restore-list size 76,041 bytes, and restore-list SHA-256 `a4c1d3703d178c34dab8559d9607d13a13e200de60a7109d1c029a8b6884581b`.
...
- Deployment added no migration and changed no production business row. Later separately authorized business mutations were limited to deleting the one empty shipment; returning and deleting `SO-2026-000002`; and returning and deleting `SO-2026-000001` together with its unpaid zero-payment invoice. No source package, scan, finished-goods total quantity, stock movement, user, role, permission, or other row changed; the corresponding audit rows and backups are recorded above.
