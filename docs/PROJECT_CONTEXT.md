...
- Latest reviewed batches: **60/127 findings fixed**. Usluga handover locks packages→order and rejects changed membership; three PostgreSQL races independently pass. Pricing assets and sales customers are batched; remaining list/polling paths stay partial. Payroll finalization, daily reports, Usluga lists and shipment queries are in progress. Payroll payable authorization requires owner review for custom scan-only issuer roles before rollout. Audit/QA record tests and limits. No deployment or production access; main conflicts remain unresolved.
