...
- Continued 2026-09-25 (PERF35 accessory Receive/Return): the opt-in picker now resolves the historical polymorphic stock-reference paths with a SQL `UNION ALL`, aggregates linked manual/stock issues and returns by order/item/unit, computes distinct-order or item-group exact totals, and applies filtering/order/offset/limit before returning page rows to Python. The default legacy array and itemless history remain compatible; only the opt-in returnable picker filters untracked/nonpositive rows. Twelve focused accessory balance tests cover legacy parity, 401 unique orders/402 item groups, SQL aggregation-before-limit and all six movement reference forms; Ruff passes. The frontend contract, full stabilization, strict TypeScript and targeted ESLint pass. Source tables still require a full database scan/group for exact totals and PostgreSQL plan/load behavior was not verified, so PERF35 remains partial. No migration, production contact, push, merge or deployment occurred.
