...
- Latest reviewed batches: **73 fixed,36 partial,18 open;54 of127 remain**. Partial findings are not counted as resolved. Reserved stock can no longer race into damaged state. Usluga cutting reads are chunked; first-attempt429 receipt retries can be corrected. Validation-only run `35675969462` passed backend, frontend lint/strict types/optimized build/contracts and all PostgreSQL selections on application revision `bae444d`; release skipped. No deployment/production access; develop conflicts, historical audit damage, runtime lifecycle and load validation remain open.
...
- Continued 2026-09-22 (continuation batch): reviewed commits add bounded or paged list reads, batched lookups, grouped aggregates, deferred hidden directories, request-scoped validation and focused security/workflow corrections across catalog, inventory, finance, HR, production and admin paths. Broad backend selections pass 173 tests with 13 skips and 145 tests with 8 skips; frontend stabilization, strict TypeScript and targeted ESLint pass. Later follow-ups preserve normalized accessory search, batch cutting-passport Item validation and package-order synchronization, preload pricing mutation assets, use Decimal profit intermediates, group 1C status refreshes, page HR Documents and Calendar end-to-end, validate forecast references, and gate backend traffic on /ready while retaining /health liveness warm-up. Packaging BOM reads are batched; payroll automatic period matching is indexed; branded stock value uses Decimal intermediates; and purchasing avoids duplicate post-write access checks. The latest combined backend selection passes 27 tests with 8 PostgreSQL-only skips plus Ruff and compilation. Reviewed commits through `e5760f0` are pushed to `feat/ismoiljon`; `c625aad`, `77205c9` and `d200161` are queued. Counts remain 73 fixed, 36 partial, 18 open, 54 remaining. No migration, merge, deployment or production contact.
