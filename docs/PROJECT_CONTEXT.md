...
- Continued 2026-09-25 (DB01 production-order quantity input): `c279b594` makes create, item-breakdown and update planned quantities strict nonnegative signed-SQL-INT values; invalid booleans, fractions, negatives and overflow reject before writes, with unauthenticated 401 precedence retained. The integrated focused and adjacent material-bound selection passes 17 tests. Other alternate writers and historical inconsistencies remain, so DB01 stays partial. Local only; no production contact, push, deployment or merge.
