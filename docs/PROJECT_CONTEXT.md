...
- New order references use four-digit numbers such as `PO-0001` and `SO-0606`. Generation continues above the highest historical suffix, serializes allocation, and blocks at 9999 rather than wrapping. Existing visible order references use compact labels and searches accept those labels; persisted historical references, QR payloads, label identities and edit-form source values remain intact. Existing numbers above 9999 are not truncated.
...
- Final local validation passed 802 backend tests, frontend lint and strict TypeScript, all frontend contracts, and the optimized 85-page production build. The only reported test warning is the upstream Starlette/httpx deprecation. Chrome fixture QA verified catalogue creation/search/deduplication, totals of 850 per piece and 51,000 quantity-weighted, descending XJ model numbers, BSOrder department states, three sizes persisted to the selected model, and Control cancel/confirm/replay (zero cancel writes; one 29,750 record on confirmation). Screenshots are under `outputs/screenshots`; the review checklist is `docs/PAYROLL_DEPARTMENTS_REVIEW_2026-09-09.md`. Browser demonstrations used isolated SQLite fixtures under `outputs/local-qa`, not production data. Migration 0118 still requires PostgreSQL staging application and the normal deployment gates before any authorized production release. Broader previously documented security findings remain separate.
