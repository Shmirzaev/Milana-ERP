...
- Continued 2026-09-23 (closure pass through `b4db4b4`): whole-finding review closes PERF13, PERF28, PERF26 and PERF30. Bundle receipt/acceptance query counts are constant at20/29 SELECTs for1/50/401 bundles with26 focused passes; purchasing request/order/approval/receipt references are chunk-bounded and audit-head reads remain one with140 passes/5 PostgreSQL-only skips. PERF26 reservation candidates use3/3/5 total and2/2/4 stock SELECTs at1/50/401 variants; its legacy metadata repair uses1/1/2 sales-item and1/1/2 collection reads for1/50/401 distinct references, with15 focused and99 adjacent passes. PERF30 clone allocation uses1/1/2 exact code-only reads for1/50/401 occupied codes, with66 compatibility and6 real-PostgreSQL plan/concurrency passes; rename and approval plans use their existing indexes without sequential scans. PERF02 remains blocked on parity-safe database derivation before paging; PERF08 retains complete per-package child arrays; PERF22 retains the legacy unpaged compatibility response; PERF33 retains a live-read atomicity gap under default READ COMMITTED. The committed range through `f25b064` is pushed to `feat/ismoiljon`; later closure commits remain local pending separate push approval. Official ledger is **83 fixed,29 partial,15 open,44 remaining**. No migration, merge, deployment or production contact.
