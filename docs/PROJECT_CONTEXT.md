...
- Latest reviewed batches: **50/127 findings fixed**. Attendance retains historical profiles, uses Tashkent dates and excludes explicit failed events (legacy NULL retained). Packages enforce order models; packaging counters survive concurrent receipts (real PostgreSQL regression). Shipment documents use indexed lookups; sewing capacity queries drop60→2 for12 assignments. WF08 remains partial (UI retry/balance handling); ST09 reverse damage transition policy remains open. Next batch: production evidence, attendance import races and purchasing query growth. No deployment or production access.
