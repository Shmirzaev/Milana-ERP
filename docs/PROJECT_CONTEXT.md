...
- Latest reviewed batches: **72 fixed,35 partial,20 open;55 of127 remain**. Partial findings are not counted as resolved. Reserved stock can no longer race into damaged state. Usluga cutting reads are chunked; first-attempt429 receipt retries can be corrected. Validation-only run `35589397268` passed backend, frontend and PostgreSQL at the earlier branch revision; the three 2026-09-21 code commits below have focused local evidence and still require final combined CI. No deployment/production access; develop conflicts, historical audit damage, runtime lifecycle and load validation remain open.
