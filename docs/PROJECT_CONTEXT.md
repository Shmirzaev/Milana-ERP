...
- Continued 2026-09-22 (third batch): `2e4dbb1` completes WF04 with Sewing bounds/conservation and no-side-effect API regressions. `4a4d0e6` changes Usluga family approval from per-model BOM counts to one grouped query; clone work remains. `be696e1` defers model BOM metadata until the visible Materials tab while preserving read-only metadata. `8240fc9` caps payroll bulk input at500. The combined backend slice passed31 tests; Ruff, compilation and both model-detail fetch contracts pass. Independent review caught and corrected an initial edit-only BOM gate before commit. No migration, merge, deployment or production contact; CI for this batch is pending.
