...
- Latest reviewed batches: **43/127 findings fixed**. Admin deletion, waste read-side effects, stock reserve/release races, purchase conversion/rejection and HR input checks have targeted regression coverage. Print runs: 52→3 SELECTs for 50; attendance import: 403→4 for 401 people. WF08 backend sale capacity/retry-key support is partial: UI remaining-balance/key handling and accounting valuation remain open. Current batch covers damaged-stock eligibility, HR attendance day/hours and shipment-document scanning. No deployment or production access.
