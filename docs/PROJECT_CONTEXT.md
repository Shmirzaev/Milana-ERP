...
- Latest reviewed batches: **59/127 findings fixed**. Daily sewing reports now lock work order before parent; PostgreSQL wait/NOWAIT regressions independently pass. Both flow-utilization endpoints avoid per-row queries. Partial task-broadcast batching preserves recipient/audit/rollback behavior on PostgreSQL. Payroll payable authorization requires owner review for custom scan-only issuer roles before rollout. Next: payroll-finalization races, Usluga handover and pricing/sales queries. Audit/QA record tests and limits. No deployment or production access; main conflicts remain unresolved.
