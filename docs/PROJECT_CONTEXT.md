...
- Latest reviewed batches: **55/127 findings fixed**. Production PATCH now uses a typed allowlist while retaining Usluga rejection; planning batches BOM/stock/enrichment. Partial fixes add safe manual-receipt reconciliation, fractional token revocation/factory profile responses and bounded off-loop image processing. Attendance source versions, other receipt recovery, reset proxy trust and upload lifecycle remain open. Next batch: work-order checks, cutting-passport queries and payable-value authorization. No deployment or production access.
