...
## September employee roster reconciled (2026-09-14)

- Production HR data updated at the user's request using 26 supplied scanned pages (421 entries), the current employee register, and 2,187 old ERP list records. Selected identities, factory branches, employee numbers and available sections were checked in the old ERP. The supplied lists take precedence for current departmental assignments; unresolved identities/numbers were not guessed.
- Created 160 MIL employee profiles (147 with verified old ERP numbers, 13 without a verified number), updated 266 existing profiles, and created department `XJL` (`Xo'jalik / General Services`). Current employee count is 472. Existing names, employee numbers and employment statuses were preserved. The 46 current employees absent from the supplied sheets were preserved without changes; no deletion/deactivation, attendance, payroll amounts, users, permissions, orders, stock or other operational data were changed.
- Employee department IDs, manager links and the existing HR profile `section` field now hold verified assignments. There are 312 manager links and 393 populated sections. Support employees retain the Xo'jalik roster group with the more specific old ERP operational department where known. Named masters required for reporting links were included. One master has both verified sections recorded as `Patok-3; Patok-7`.
- Remaining gaps among the 426 applied profiles: 13 employee numbers, 114 manager/master assignments (including managers above masters), and 33 subdepartments. One additional abbreviated-name entry was held because it may duplicate another listed employee. One employee occurs on both Patok-1 and Patok-2 sheets, so their section/master remain unresolved. Old ERP duplicate numbers and patronymic conflicts are documented in the report. Pages 15-16 have cropped headings; sections come from old ERP, including one Patok-6 assignment among Patok-10 rows. Page 21 is printed September 2025 but was treated as current under the user's supplied-list instruction.
- Data-only operation: no software deployment or schema migration. Both active symlinks remain green release `20260914_110313`; rollback blue `20260914_073912` and database revision `0126_payroll_sewing_access` are unchanged. Production, origin/main and `deploy/production-base.json` were reconciled before the dedicated worktree was created. The legacy checkout was preserved.
- Verified pre-change PostgreSQL backup on backend VM: `/home/admilana/erp-roster-backups/employee_roster_20260914_121923.dump`, mode 0600, 53,354,714 bytes, 1,171 restore objects, SHA-256 `f2a9815b83e0ecc6897f4adea5e8fb2801dc76abaa38d99317aa6c72f6c9c578`. Restore list: 83,832 bytes, mode 0600, SHA-256 `2205b966f6a80ccf6ffec1bd6681f0d14a89eeeab7899566947156939a09b404`.
- Validation: read-only dry run; exact pre-write employee/department snapshot checks; unique employee numbers; same-factory manager/reference validation; atomic application of the primary change set; full post-write comparison of all 426 changed profiles and 46 untouched profiles. Audit records `19653` through `20080` cover the change set and the final multi-section master update; the new hash-chain segment verifies. This does not repair the historical audit-chain defect. All four internal/public health/login checks passed. HR browser QA was limited because the existing browser session is Payroll and correctly receives access denied on `/hr/employees`; no permission changes were made to bypass that restriction.
- Dedicated worktree `C:/ERP/.codex-work/employee-roster-20260914`, branch `codex/employee-roster-20260914`. This branch records context only; operational scripts and personal employee data remain local. Reconciliation workbook: `outputs/employee-roster-20260914/Milana-employee-reconciliation-2026-09-14.xlsx`; local extraction, snapshots, reviewed plan and audit evidence: `tmp/roster/`. The workbook contains the complete reconciliation, missing information/conflicts and unlisted current employees. Context mirrored to Obsidian.
