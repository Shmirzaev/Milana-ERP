...
- Latest reviewed batches: **69/127 findings fixed;24 open;34 partial**. Warehouse/bundle/shipment/inbox/allocation/cost references improved; waste UI retains retries; hidden HR form fetches deferred. Focused regressions independently pass. CI `3f5afa3` passed backend/PostgreSQL/frontend, confirming the prior SQLite packaging failure is corrected. Later BOM-cost/recruitment commits need combined CI. User approved longer identifiers; implementation remains uncommitted pending PostgreSQL concurrency proof and root review. No deployment/production access; main conflicts, historical audit damage, runtime lifecycle and load validation remain open.
