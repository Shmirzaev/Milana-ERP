...
| FN07 (partial) | `test_finance_input_bounds.py`; invoice/payment schema and POST `/api/finance/payments` | Negative/nonfinite/overflow amounts, zero/sub-cent payment, invalid IDs or method length reject before writes. Preserve zero/omitted invoice, valid cents, string coercion and nullable method. 21 focused cases; 29 with payment suite,6 PG skips. Constant scalar validation; monetary precision policy remains open. |
...
| PERF22 (partial) | `test_usluga_list_query_growth.py`; GET `/api/usluga/orders` |1/50/401 orders sharing models:9/9/12 SELECTs;401 distinctmodels16. Full scalar payload parity, status filters, model scope, image metadata and output ordering preserved.400-ID chunks; query count O(ceil(orders/400)+ceil(models/400)), output O(parents+children). Unbounded response remains open. |
