...
from app.models import FinishedGoodsStock, Model, Package, PackageBarcodeAlias

STORAGE_STATUSES = ("received_in_storage", "reserved", "damaged")


def package_snapshots(db, package_ids=None, *, expected_only=False):
    balance = (
        db.query(
...
def resolve_package(db, code):
    ...
    if direct:
        return next(iter(direct)) if len(direct) == 1 else None, len(direct) > 1
    aliases = {
        pid
    ...
    return next(iter(aliases)) if len(aliases) == 1 else None, len(aliases) > 1
...
def row_payload(row, current):
    ...
    return {
        ...
        "scanned_at": (
            ...
            if row.scanned_at and row.scanned_at.tzinfo is None
            else row.scanned_at
        ),
