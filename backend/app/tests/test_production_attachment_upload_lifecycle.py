...
def test_production_document_upload_persists_valid_file(tmp_path, monkeypatch):
    monkeypatch.setattr(production.settings, "SALES_ORDER_FILES_DIR", str(tmp_path))
    content = b"%PDF-1.4\nsynthetic attachment"
    upload = UploadFile(file=BytesIO(content), filename="artwork.pdf")

    try:
        response = asyncio.run(production.upload_production_printing_attachment(upload, object()))
    finally:
...
def test_production_document_cancellation_shields_new_file_cleanup(tmp_path, monkeypatch):
    ...
    async def run():
        with CancelScope() as scope:
            ...
            monkeypatch.setattr(production, "sign_path", cancel_after_file_write)
            with pytest.raises(asyncio.CancelledError):
                await production.upload_production_printing_attachment(upload, object())
...
def test_production_image_cancellation_shields_new_file_cleanup(tmp_path, monkeypatch):
...
def test_production_document_collision_preserves_preexisting_file(tmp_path, monkeypatch):
    collision = tmp_path / "po_print_collision.pdf"
    collision.write_bytes(b"keep collision attachment")
    monkeypatch.setattr(production.settings, "SALES_ORDER_FILES_DIR", str(tmp_path))
    monkeypatch.setattr(production, "uuid4", lambda: SimpleNamespace(hex="collision"))
    upload = UploadFile(file=BytesIO(b"%PDF-1.4\nreplacement"), filename="artwork.pdf")

    try:
        with pytest.raises(FileExistsError):
            asyncio.run(production.upload_production_printing_attachment(upload, object()))
    finally:
        asyncio.run(upload.close())
