...
@pytest.mark.parametrize("prefix", ["/storage/model-files/", "/storage/model-files/thumb/"])
@pytest.mark.parametrize("name", ["%2E%2E%5Cprivate.png", "%2E%2E%2Fprivate.png", "missing.png"])
def test_model_file_paths_stay_confined(client, file_user, model_file_urls, prefix, name):
    assert client.get(prefix + name).status_code == 404


def test_thumbnail_generation_releases_auth_connection(client, file_user, model_file_urls, monkeypatch):
    from app.services import image_storage

    original = image_storage.ensure_webp_thumbnail

    def checked_thumbnail(**kwargs):
        assert test_engine.pool.checkedout() == 0
        return original(**kwargs)

    monkeypatch.setattr(image_storage, "ensure_webp_thumbnail", checked_thumbnail)
    assert client.get(model_file_urls[1]).status_code == 200
