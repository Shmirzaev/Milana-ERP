"""Add audited payroll reversals and remove the unused piecework workspace.

Revision ID: 0093_payroll_reversal
Revises: 0092_sewing_role_access
"""
...
revision = "0093_payroll_reversal"
down_revision = "0092_sewing_role_access"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.add_column(
        "payroll_adjustments",
        sa.Column("source_payroll_record_id", sa.Integer(), nullable=True),
    )
    op.create_foreign_key(
        "fk_payroll_adjustments_source_payroll_record_id",
        "payroll_adjustments",
        "payroll_records",
    ...
    op.create_unique_constraint(
        "uq_payroll_adjustments_source_record",
        "payroll_adjustments",
        ["source_payroll_record_id"],
    )
    op.create_index(
    ...
    # are disposable, but any assignment or acceptance requires manual review.
    op.execute(
        """
        LOCK TABLE piecework_acceptances, piecework_assignments, piecework_shifts
        IN ACCESS EXCLUSIVE MODE;
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM piecework_assignments)
               OR EXISTS (SELECT 1 FROM piecework_acceptances) THEN
                RAISE EXCEPTION
                    'Piecework removal blocked: assignments or acceptances exist';
            END IF;
        END
        $$;
        """
...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("accepted_good_quantity + accepted_defect_quantity <= planned_quantity", name="ck_piecework_assignments_accepted_within_plan"),
        ...
        sa.CheckConstraint("status IN ('planned', 'in_progress', 'completed', 'cancelled')", name="ck_piecework_assignments_status"),
    )
    for column in (
