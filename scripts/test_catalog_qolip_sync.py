...
class CatalogPlanTests(unittest.TestCase):
    def test_compound_and_text_pattern_numbers_are_preserved(self):
        for value in ('KJ13028 /4351', 'Д-03', '4152 tek razmer'):
            with self.subTest(value=value):
                plan=build([master(q=value)],[],[new()])
                self.assertEqual(plan['qolip_updates'][0]['qolip_no'],value)
    def test_model_code_only_source_stays_blank(self):
        plan=build([master(q='XJ-3062')],[],[new()])
