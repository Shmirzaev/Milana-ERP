...
def run(bundle, expected_hash, apply=False):
    ...
    with SessionLocal() as db:
        validate_revision(manifest, db.execute(text('select version_num from alembic_version')).scalar())
    old_erp = manifest.get('source_kind') == 'old_erp_model_original'
    existing_family = manifest.get('source_kind') == 'existing_model_family_photo'
    ...
    for photo in manifest['photos']:
        validate_photo_source(photo, manifest.get('source_kind', 'telegram_original'))
        name = photo['stored_name']
        data = (root/'files'/name).read_bytes()
        assert sha(data) == photo['sha256'] and len(data) == photo['stored_bytes']
        if existing_family:
            from validate_old_erp_photo import validate_existing
            validate_existing(photo, data)
            assert (storage/name).is_file(), 'Existing family image is missing from storage'
        elif old_erp:
    ...
    with SessionLocal() as db:
        ...
        source_ids = [p['source']['id'] for p in manifest['photos']] if existing_family else []
        db.query(Model.id).filter(Model.id.in_(sorted(set(ids+source_ids)))).order_by(Model.id).with_for_update().all()
        ...
        if existing_family:
            ...
            sources = load_models(db, source_ids)
            for photo in manifest['photos']:
                source = sources[photo['source']['id']]
                assert source.catalog_scope == 'standard' and source.code == photo['source']['code']
                assert norm(((source.details_json or {}).get('general') or {}).get('model_no')) == photo['model_no']
                assert model_preview_image_url(source) == photo['source']['file_url']
        ...
        for photo in manifest['photos']:
            ...
            for target in photo['models']:
                ...
                if old_erp or existing_family:
                    from validate_old_erp_photo import validate_blank_target
                    validate_blank_target(model.code, (model.details_json or {}).get('general'),
                                          photo['model_no'], model_preview_image_url(model))
        ...
        result = {'mode': 'apply' if apply else 'dry_run', 'manifest_sha256': expected_hash,
                  'source_kind': manifest.get('source_kind', 'telegram_original'),
        ...
        if apply and pending:
            action = 'existing_family_model_photos_20260908' if existing_family else 'old_erp_model_photos_20260908' if old_erp else 'telegram_model_photos_20260905'
            audit = log_action(db, db.get(User,1), action, 'ModelImage', None,
