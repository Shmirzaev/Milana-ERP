...
def meaningful_qolip(row):
    value = str(row.get('qolip_no') or '').strip()
    return bool(value and any(c.isdigit() for c in value) and norm(value) != norm(row['model_no']))
...
def build(masters, variants, production):
    ...
    def source_for(base, oldrows):
        ...
        return linked if any(meaningful_qolip(r) for r in linked) else candidates
    def qolip(sources):
        vals={r['qolip_no'] for r in sources if meaningful_qolip(r)}
        return next(iter(vals)) if len(vals)==1 else None
    for key, rows in new_by_identity.items():
        sources=source_for(key[0],old_by_identity.get(key,[]))
        q=qolip(sources)
        if not q:
            if sources and len({r['qolip_no'] for r in sources if meaningful_qolip(r)})>1:
